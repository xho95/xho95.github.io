...
함수는 모든 인-아웃 매개 변수에 대한 장기 쓰기 액세스 권한을 갖습니다. 인-아웃 매개 변수에 대한 쓰기 액세스는 모든 비-인-아웃 매개 변수가 평가 된 후 시작되어 해당 함수 호출의 전체 기간 동안 지속됩니다. 입력 매개 변수가 여러 개인 경우 매개 변수가 나타나는 순서대로 쓰기 액세스가 시작됩니다.

이 장기 쓰기 액세스의 결과 중 하나는 범위 지정 규칙 및 액세스 제어에서 허용하는 경우에도 인-아웃으로 전달 된 원래 변수에 액세스 할 수 없다는 것입니다. 원본에 액세스하면 충돌이 발생합니다. 예를 들면 다음과 같습니다.
