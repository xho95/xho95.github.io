...
프로토콜 타입에 대한 접근 수준을 명시적으로 할당하고 싶으면, 프로토콜을 정의하는 시점에 그렇게 하도록 합니다. 이러면 지정된 접근 영역 내에서만 채택할 수 있는 프로토콜을 생성할 수 있습니다.

프로토콜 정의 내의 각 '필수 조건 (requirement)' 의 접근 수준은 자동적으로 그 프로토콜의 접근 수준과 같도록 설정됩니다. 프로토콜의 '필수 조건' 에는 그것이 지원하는 프로토콜과 다른 접근 수준을 설정할 수 없습니다. 이것은 프로토콜을 채택하는 타입이 어떤 것이든 프로토콜의 모든 '필수 조건' 을 볼 수 있도록 만들어 줍니다.

> 'public (공용)' 프로토콜을 정의한 경우, 이 프로토콜의 '필수 조건' 을 구현하면 그 '필수 조건' 은 반드시 '공용 (public)' 접근 수준이어야 합니다. 이러한 행동은 다른 타입들과는 다른 것으로, 다른 'public (공용)' 타입을 정의하면 그 타입 멤버는 'internal (내부)' 접근 수준으로 구현합니다.
