...
튜플 타입의 접근 수준은 해당 튜플에서 사용하는 모든 타입 중 가장 제한적인 접근 수준이어야 합니다. 예를 들어, 두 개의 서로 다른 타입을 가지는 튜플을 구성할 때, 하나는 'internal (내부)' 접근이고 하나는 'private (개인 전용)' 접근이면, 이렇게 구성된 튜플의 접근 수준은 'private (개인 전용)' 이 됩니다.
