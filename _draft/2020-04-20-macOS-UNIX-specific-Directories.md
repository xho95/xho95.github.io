...
'단일 사용자 (single-user)' 환경과 '다중 사용자 (multi-user)' 환경 모두에서 중요한 '명령줄 실행 파일 (command-line binaries)' 을 저장하는 디렉토리입니다. macOS 의 각종 **shell** (쉘) 을 포함하여 **mkdir** 같은 실행 파일들이 여기에 존재합니다.

#### `/cores` : 개발 관련 디렉토리

`/cores` 디렉토리에 대한 설명은 `man hier` 명령으로 확인할 수 없습니다. 왜냐면 이 디렉토리는 일반적인 상황에서는 생기지 않기 때문입니다. `/cores` 디렉토리는 macOS 에서 개발을 하면서 충돌이 발생할 경우, 당시의 '메모리 상태 (the state of the memory)' 를 갖고 있는 파일을 저장하기 위한 공간입니다.[^cores]
...
macOS 에 있는 별도 프로그램들의 설정 파일은 `/etc` 의 하위 디렉토리에 저장되는 경우가 많습니다. 예를 들어, **apache** (아파치) 프로그램의 설정 파일은 `/etc/apache2` 디렉토리에 존재합니다.

> 참고로 macOS 의 `/etc` 디렉토리는 실제로는 `/private/etc` 디렉토리의 '심볼릭 링크 (symbolic link)' 입니다. 즉, `/etc/apache2` 디렉토리는 `/private/etc/apache2` 디렉토리와 같은 디렉토리입니다.
>
> `/private` 디렉토리에 대해서는 아래의 [`/private` : 개인 사용자 디렉토리](#private-개인-사용자-디렉토리) 에서 설명하도록 합니다.

#### `/home` : 유닉스 시스템의 홈 디렉토리 - macOS 에서는 사용하지 않음

보통 `/home` 디렉토리는 유닉스 파일 시스템의 기본 '홈 디렉토리 (home directory)' 로 사용되는 곳입니다. 하지만 macOS 에서는 '홈 디렉토리' 로 `/home` 대신 `/Users/username` 디렉토리를 사용합니다.

> macOS 의 '홈 디렉토리' 에 대해서는 아래의 [홈 디렉토리](#--macos-의-홈-디렉토리) 에서 다시 설명하겠습니다.
...
'홈 (home) 디렉토리' 는 다중 사용자 운영 체제에서의 파일 시스템 디렉토리를 말합니다. 여기서 다중 사용자라는 말에는 로그인한 사용자마다 별도의 홈 디렉토리가 생긴다는 의미가 담겨 있고, 파일 시스템이라는 말에는 사용자가 만들고 사용하는 파일들을 관리한다는 의미가 담겨 있습니다.

터미널에서 홈 디렉토리로 이동하려면 아래와 같은 명령을 사용하면 됩니다.

```
$ cd ~
```

macOS 의 '홈 디렉토리' 는 조금 특별합니다. macOS 는 '홈 디렉토리' 로 '유닉스-고유 디렉토리' 인 `/home` 을 사용하지 않고, '표준 디렉토리' 중 하나인 `/Users` 디렉토리를 활용하여 `/Users/username` 디렉토리를 사용합니다.

macOS 에서 `/Users/username` 디렉토리를 '홈 디렉토리' 로 사용하는 이유는, macOS 에서 유닉스 파일 시스템을 도입하기 이전인, **Mac OS 9** 시절부터 이미 `/Users/username` 디렉토리를 사용하고 있었기 때문이라고 합니다.[^macOS-home] 이것은 기존 '맥 운영 체제' 와 '유닉스' 의 서로 다른 파일 시스템을 절충한 것으로 볼 수 있을 것 같습니다.

> 사실 이것이 이 문서를 작성하게 된 동기라고 할 수 있습니다. 이 문서는 macOS 를 사용하면서 혼동을 일으키는 '표준 디렉토리 (standard directories)' 와 '유닉스-고유 디렉토리 (UNIX-specific directories)' 의 기능을 구분하고 정리할 목적으로 작성된 것입니다.

터미널에서 홈 디렉토리로 이동하는 방법에는 아래와 같이 환경 설정 변수를 사용하는 방법도 있습니다.

```
$ cd $HOME
```

이것은 `$HOME` 이라는 환경 설정 변수에 `/Users/username` 이 설정되어 있기 때문입니다. 환경 설정 변수를 확인하려면 다음과 같이 하면 됩니다.[^GUI-environment]
...
[^macOS-home]: macOS 에서 `/Users/username` 을 '홈 디렉토리' 로 사용하는 이유는 [Why are home folders in Mac OS X located in /Users, and not /home?](https://apple.stackexchange.com/questions/50633/why-are-home-folders-in-mac-os-x-located-in-users-and-not-home) 라는 글을 참고하기 바랍니다. 해당 글을 보면 유닉스의 '홈 디렉토리' 로 `/home` 디렉토리를 사용하는 것은 기본 설정이지 필수 사항은 아니기 때문에 `/Users/username` 을 사용하는 것에는 아무런 문제가 없다고 합니다.

[^GUI-environment]: 'GUI' 에서도 `$HOME` 환경 설정 변수를 확인할 수 있는데, 이 방법은 [Where do I find my user folder in the OS X folder hierarchy?](https://apple.stackexchange.com/questions/51280/where-do-i-find-my-user-folder-in-the-os-x-folder-hierarchy) 라는 글을 참고하기 바랍니다.
