장고(Django)는 MVC 패턴에 대응되는 MTV 패턴에 따라 개발을 진행하는 웹 서버 개발용 파이썬 웹 프레임웍입니다. 제공하는 기능이 풍부하고 쉽고 빠른 웹 개발이 가능해서 사용자가 가장 많습니다.[^kyobobook_1]  [^hanbit_1]  [^kyobobook_2]
...
> anaconda를 사용하지 않을 경우는 pip와 virtualenv 조합을 사용하게 되는데 인터넷에 관련 자료가 많이 있는 것 같습니다. [^hannal]  [^djangogirls]  [^hubsite]  [^tryhelloworld]
