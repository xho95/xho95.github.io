...
접근이 충돌하는 상황에서 고려해야 할 메모리 접근의 성질에는 세 가지가 있습니다: 그 접근이 읽기이거나 쓰기인지, 접근의 지속 기간, 메모리에서 접근이 이뤄지는 위치가 그것입니다. 특히, 다음의 조건을 모두 충족하는 접근이 두 개가 되면 충돌이 발생합니다:

* 적어도 하나는 '쓰기 접근 (write access)' 입니다.
* 이들은 메모리의 동일한 위치에 접근합니다.
* 이들의 지속 기간이 겹칩니다.

읽기 및 쓰기 액세스의 차이점은 일반적으로 명백합니다. 쓰기 액세스는 메모리의 위치를 변경하지만 읽기 액세스는 그렇지 않습니다. 메모리의 위치는 액세스중인 대상 (예 : 변수, 상수 또는 속성)을 나타냅니다. 메모리 액세스 기간은 순간적이거나 장기적입니다.

액세스가 시작된 후 종료되기 전에 다른 코드를 실행할 수없는 경우 액세스가 즉시 이루어집니다. 그 특성상 두 가지 즉각적인 액세스는 동시에 발생할 수 없습니다. 대부분의 메모리 액세스는 즉각적입니다. 예를 들어 아래 코드 목록의 모든 읽기 및 쓰기 액세스는 즉각적입니다.
