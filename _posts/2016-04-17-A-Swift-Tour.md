...
문자열에 값을 포함시키는 데는 더 간단한 방법도 있습니다. 괄호 안에 값을 쓰고, 괄호 앞에 '역슬래시 기호 (backshlash; `\`)' 를 쓰면 됩니다. 예를 들면 다음과 같습니다:
...
`if` 문에서, '조건절 (the conditional)' 은 반드시 '불린 표현식 (Boolean expression)' 이어야 합니다-이것의 의미는 가령 `if score { ... }` 와 같은 코드는 에러이지, '0' 으로 암시적인 비교를 하는게 아니라는 것입니다.
...
> 실험
>
> 가장 큰 수가 뭔지 추적한 것처럼, 다른 변수를 추가하여 가장 큰 수가 무슨 종류인지를 추적해 봅시다.

`while` 문을 사용하면 조건이 바뀔 때까지 어떤 '코드 블럭 (block of code)' 을 반복합니다. '반복문 (loop)' 의 조건을 맨 끝에 둘 수도 있는데, 이러면 반복문의 실행을 최소 한 번은 보장합니다.
...
`..<` 를 사용하여 '색인 범위 (range of indexes)' 를 만들면 반복문 내에서 '색인 (index)' 을 유지할 수 있습니다.
...
`..<` 을 사용하면 최상단 값을 생략하는 범위를 만들게 되고, `...` 를 사용하면 양 끝단 값을 모두 포함하는 범위를 만들게 됩니다.
...
함수는 '중첩될 (nested)' 수 있습니다. '중첩 함수 (nested functions)' 는 외부 함수에서 선언한 변수에 접근할 수 있습니다. '중첩 함수' 를 사용하여 함수에 있는 길고 복잡한 코드를 정돈할 수 있습니다.
...
함수는 실제로는, 나중에 호출할 수 있는 코드 블럭을 말하는, '클로저 (closures)' 의 특수한 한 경우에 해당합니다. 클로저에 있는 코드는 클로저가 생성된 영역의 범위에서 사용 가능한 변수와 함수 같은 것들에 접근하게 되는데, 이는 클로저를 실행될 때는 다른 영역에 있더라도 상관없습니다-이에 대한 예제는 '중첩 함수' 에서 이미 보았습니다. 이름없는 클로저를 만들려면 코드 주변을 중괄호 (`{}`) 로 감싸면 됩니다. 이 때 `in` 을 사용하여 인자와 반환 값을 본문과 구분합니다.
...
위에서 열거체의 '`hearts` 경우 값 (case)' 을 참조할 때 두 가지 다른 방법을 썼음에 주목하기 바랍니다: `hearts` 상수에 값을 할당할 때는, 열거체 경우 값 `Suit.hearts` 이 전체 이름으로 참조됐는데 이는 이 상수에 명시적인 타입이 지정된 것이 없기 때문입니다. 'switch (스위치)' 구문 내부에서는, 열거체의 기본 설정 값이 `.hearts` 라는 단축된 양식으로 참조되었으며 이는 `self` 값이 이미 'suit' 인 것을 알고 있기 때문입니다. 이처럼 값의 타입을 이미 알고 있을 때는 언제든 단축된 양식을 사용할 수 있습니다.

열거체가 원시 값을 가질 경우, 이 값은 선언의 일부로써 결정되며, 이는 특정한 '열거체 경우 값 (enumeration case)' 의 모든 인스턴스는 항상 같은 '원시 값' 을 가진다는 것을 의미합니다. '열거체 경우 값' 에 대한 또 다른 선택은 해당 '경우 값' 이 '결합되어 있는 값' 을 가지는 것입니다-이 값은 인스턴스를 만들 때 결정되므로, '열거체 경우 값' 의 각각의 인스턴스마다 다를 수 있습니다. '결합된 값 (associated values)' 은 '열거체 경우 값 인스턴스 (enumeration case instance)' 의 '저장 속성' 처럼 동작한다고 생각하면 됩니다. 예를 들어, 서버에서 일출 시간과 일몰 시간을 요청하는 경우를 고려해 봅시다. 서버의 응답은 요청한 정보일 수도 있고, 또는 뭔가 잘못됐을 경우에 대한 설명일 수도 있습니다.
