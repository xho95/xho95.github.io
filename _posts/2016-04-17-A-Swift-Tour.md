...
여러 개의 `catch` 블럭을 제공하여 지정한 에러들을 처리할 수 있습니다. 패턴은 **switch** 문의 `case` 뒤에 하듯 `catch` 뒤에 작성합니다.
...
에러를 처리하는 또 다른 방법은 `try?` 로 결과를 옵셔널로 변환하는 겁니다. 함수가 에러를 던지면, 지정된 에러를 버리고 결과는 `nil` 이 됩니다. 그 외 경우, 결과는 함수 반환 값을 담은 옵셔널입니다.
