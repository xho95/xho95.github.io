...
특정 에러를 처리하는 여러 개의 `catch` 블럭을 제공할 수 있습니다. **switch 문** 의 `case` 뒤에 하듯 `catch` 뒤에 패턴을 작성합니다.
...
에러를 처리하는 또 다른 방법은 `try?` 를 써서 결과를 옵셔널로 변환하는 겁니다. 함수가 에러를 던지면, 그 특정 에러를 버려서 결과가 `nil` 입니다. 그 외 경우, 결과는 함수 반환 값을 담은 옵셔널입니다.
...
`defer` 를 사용하여, 함수 반환 직전, 함수 안의 다른 모든 코드 뒤에 실행할 코드 블럭을 작성합니다. 함수가 에러를 던지는 거엔 상관없이 코드를 실행합니다. `defer` 를 사용하면, 서로 다른 시간에 실행할 필요가 있는 설정 및 정리 코드도, 나란히 작성할 수 있습니다.
