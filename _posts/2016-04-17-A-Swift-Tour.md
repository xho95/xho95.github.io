...
'상위 클래스' 의 구현을 '재정의 (override)' 하는 '하위 클래스' 의 메소드는 `override` 로 표시합니다—우연히, `override` 없이, 메소드를 재정의하면, 컴파일러가 에러라고 감지합니다. 컴파일러는 실제로는 '상위 클래스' 의 어떤 메소드도 재정의하지 않으면서 `override` 라고 한 메소드도 감지합니다.
