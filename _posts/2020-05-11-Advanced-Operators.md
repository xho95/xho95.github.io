...
연산자 메소드는 `Vector2D` 에 대한 타입 메소드로 정의하며, 메소드 이름은 중복 정의한 연산자 (`+`) 와 일치합니다. 덧셈은 벡터의 핵심 동작이 아니기 때문에, `Vector2D` 구조체의 주 선언부 보단 `Vector2D` 의 익스텐션에서 타입 메소드를 정의합니다. 산술 덧셈 연산자는 이항 연산자이기 때문에, 이 연산자 메소드는 `Vector2D` 타입의 입력 매개 변수를 두 개 취하며, 역시 `Vector2D` 타입인, 단일 출력 값을 반환합니다.
