...
'빨간색 성분' 은 `0xCC6699` 와 `0xFF0000` 라는 수를 '비트 곱' 하여 구합니다. `0xFF0000` 에 있는 `0` 들은 실제로 `0xCC6699` 의 두 번째와 세 번째 '바이트 (bytes)' 를 "숨겨서 (mask)", `6699` 는 무시하고 결과적으로 `0xCC0000` 만 남도록 합니다.
