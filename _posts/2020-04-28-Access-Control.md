...
> 접근 제어를 적용할 수 있는 (속성, 타입, 함수, 등과 같은) 코드의 다양한 측면은, 간결함을 위해, "개체 (entities)" 라고 합니다.
...
_소스 파일 (source file)_ 은 모듈 안의 단일 스위프트 소스 코드 파일 (사실상, 앱이나 프레임웍 안의 단일 파일) 입니다. 비록 개별 타입을 별도의 소스 파일에 정의하는 것이 일반적으로 공통일지라도, 단일 소스 파일에 여러 타입, 함수, 등등의 정의를 담을 수 있습니다.
