...
_접근 제어 (Access Control)_ 는 다른 소스 파일 및 모듈에 있는 코드에서 자신의 코드에 접근하는 걸 제한합니다. 이런 특징은 코드의 자세한 구현은 숨기면서, 더 좋은 인터페이스를 정해서 그 코드에 접근하고 쓸 수 있게 해줍니다.
...
언어의 서로 다른 측면으로 정해지는 이 지침의 의미는 아래에서 자세히 다룹니다:
...
단순한 단일-대상 앱을 작성할 땐, 앱 코드가 앱 안에서 그 자체로-동작하는게 일반적이라 앱 모듈 밖에서 쓸 수 있게 할 필요가 없습니다. 기본 접근 수준인 내부 (internal) 는 이미 이런 필수 조건과 일치합니다. 그러므로, 자신이 접근 수준을 지정할 필요가 없습니다. 하지만, 일부 코드를 파일 전용 (file private) 이나 개인 전용 (private) 으로 표시하여 앱 모듈 안의 다른 코드로부터 세부 구현을 숨기고 싶을 수도 있습니다.
...
단위 테스트 대상으로 앱을 작성할 땐, 테스트를 위해서 그 모듈에서 앱 코드를 쓸 수 있게 할 필요가 있습니다. 기본적으로, 공개 (open) 나 공용 (public) 으로 표시한 개체만 다른 모듈에서 접근 가능합니다. 하지만, 제품 모듈의 불러오기 (import) 선언을 `@testable` 특성으로 표시하고 그 제품 모듈을 테스트 가능 (test enabled) 으로 컴파일하면, 단위 테스트 대상이 어떤 내부 (internal) 개체에든 접근할 수 있습니다.
...
{% include footer_swift_book.md %} 이 장의 원문은 [Access Control](https://docs.swift.org/swift-book/LanguageGuide/AccessControl.html) 에서 볼 수 있습니다.
