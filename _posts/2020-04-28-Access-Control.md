...
('앱 번들 (app bundle)' 이나 '프레임웍' 같은) 'Xcode' 의 각 '제작 대상 (build target)' 은 스위프트에서 별도의 모듈 처럼 취급합니다. 앱 코드의 각 측면들을-아마도 해당 코드를 은닉하여 여러 응용 프로그램에서 재사용하려고-독립된 프레임웍으로 그룹지으면 해당 프레임웍 안에서 정의한 모든 것들은 별도의 모듈의 일부인 것처럼, 앱에서 불러오고 사용되거나 다른 프레임웍에서 사용됩니다.
