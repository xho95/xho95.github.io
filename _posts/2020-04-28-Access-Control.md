...
접근 제어를 다양한 수준으로 부여하는 것 외에도, 스위프트는 일반적인 '상황 (scenarios)' 에서는 '기본 접근 수준 (default access levels)' 을 제공하여 접근 제어 수준을 명시적으로 지정할 필요를 줄여줍니다. 실제로, 'single-target (단일-대상)' 앱을 작성할 경우, 접근 제어 수준을 명시적으로 지정할 필요가 전혀 없을 겁니다.
