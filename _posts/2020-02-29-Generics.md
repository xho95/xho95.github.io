...
결합된 타입에 '제네릭 `where` 구절 (generic `where` 구절; 일반화된 `where` 구절)' 을 포함시킬 수 있습니다. 예를 들어, `Container` 의 버전 중 하나로, 표준 라이브러리에 있는 `Sequence` 프로토콜이 사용하고 있는 것과 같은, '반복자 (iterator)' 를 포함시키고 싶다고 가정해 봅시다. 다음은 이를 작성하는 방법입니다:
...
### Generic Subscripts (제네릭 첨자 연산; 일반화된 첨자 연산)
