...
한 번 '타입 매개 변수' 를 지정하고 나면, (`swapTwoValues(_:_:)` 함수의 `a` 와 `b` 매개 변수 처럼) 함수의 '매개 변수 타입' 이나, 함수의 '반환 타입' 으로, 또는 함수 본문에서 '타입 보조 설명'[^type-annotation] 으로 정의하는데 사용할 수 있습니다. 각각의 경우에서, '타입 매개 변수' 는 함수를 호출할 때마다 _실제 (actual)_ 타입으로 대체됩니다. (위 `swapTwoValues(_:_:)` 예제에 있는, `T` 는 함수를 첫 번째로 호출할 때는 `Int` 로 대체되고, 두 번째로 호출할 때는 `String` 으로 대체됩니다.)
