...
자신의 타입 구속 조건은 자신만의 일반화 타입을 생성할 때 정의할 수 있으며, 이 구속 조건들이 일반화 프로그래밍[^generic-programming] 의 대부분의 강력함을 제공합니다. `Hashable` 같은 추상적인 개념 (abstract concepts) 은 타입을, 그것의 고정 타입 (concrete type) 보단, 그것의 개념적인 성질 (conceptual characteristics) 로 성격짓습니다.[^conceptual-characteristics]
...
[^conceptual-characteristics]: '타입을, 자신의 고정 타입 보단, 자신의 개념적인 성질로 성격짓는다' 는 건 프로그래밍에서의 '덕 타이핑 (duck typing)' 과 비슷한 개념입니다. **덕 타이핑** 에 대한 더 자세한 정보는, 위키피디아의 [Duck typing](https://en.wikipedia.org/wiki/Duck_typing#Templates_or_generic_types) 항목과 [덕 타이핑](https://ko.wikipedia.org/wiki/덕_타이핑) 항목을 참고하기 바랍니다.
