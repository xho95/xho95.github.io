...
'첨자 연산 (subscript)' 은 '제네릭' 일 수 있고, '제네릭 `where` 절' 을 포함할 수 있습니다. `subscript` 뒤의 꺾쇠 괄호 안에 '자리 표시용 타입 이름 (placeholder type name)' 을 작성하고, 첨자 연산 본문의 여는 중괄호 바로 전에 '제네릭 `where` 절' 을 작성합니다. 예를 들면 다음과 같습니다:
