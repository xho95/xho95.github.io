...
[Type Constraints (타입 제약 조건)](https://docs.swift.org/swift-book/LanguageGuide/Generics.html#ID186) 에서 설명한 대로, '타입 제약 조건'을 사용하면 일반화된 함수, 일반화된 첨자 연산자 또는 일반화된 타입들과 관련된 타입 매개 변수들의 요구 사항을 정의 할 수 있습니다.

여기에 더해서 관련된 타입 (associated type) 에 대한 요구 사항을 정의할 때도 유용하게 쓸 수 있는데, 이 때는 _일반화된 where 구절_ 을 정의하면 됩니다. 일반화된 `where` 구절을 사용하면 '관련된 타입'이 반드시 특정 프로토콜을 준수해야 한다거나, 특정 '타입 매개 변수'와 '관련된 타입'이 같아야 한다는 요구 사항들을 덧붙일 수 있습니다. 일반화된 `where` 구절은 `where` 키워드로 시작해서, 뒤이어서 관련된 타입에 대한 제약 조건이나 '타입'과 '관련된 타입' 간의 동등 관계 제약 조건 등이 따라옵니다. 일반화된 `where` 구절은 타입이나 함수 본문의 시작 괄호 바로 앞에 적습니다.
...
두 컨테이너를 검사할 때 컨테이너의 타입이 같을 필요는 없지만 (물론 같더라도 상관없습니다), 각 요소들의 타입은 같아야 합니다. 이 요구 사항은 '타입 제약 조건 (type constraints)' 과 '일반화된 where 구절' 을 조합하여 표현합니다.
