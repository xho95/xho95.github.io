...
이러한 필수 조건을 정의하려면, `Container` 프로토콜이 컨테이너가 보유할 원소 타입을, 특정 컨테이너서 해당 타입이 무엇인지를 모르는 체로, 참조할 방법이 필요합니다. `Containter` 프로토콜은 `append(_:)` 메소드에 전달된 어떤 값이든 컨테이너의 원소 타입과 반드시 똑같은 타입이어야 하고, 컨테이너의 첨자 연산이 반환하는 값이 컨테이너의 원소 타입과 반드시 똑같은 타입일 것을, 지정할 필요가 있습니다.
