...
이 함수는 위에서 쓴 대로 컴파일되지 않습니다. 문제는 같음 비교 검사인, "`if value == valueToFind`" 에 있습니다. 스위프트에 있는 모든 타입을 같음 비교 연산자 (`==`) 로 비교할 수 있는게 아니기 때문입니다. 예를 들어, 복잡한 데이터 모델을 나타내고자 자신만의 클래스나 구조체를 생성한다면, 그 클래스나 구조체에서의 "같음 (equal to)" 의 의미는 스위프트가 추측할 수 있는 어떤 게 아닙니다. 이 때문에, 가능한 _모든 (every)_ 타입 `T` 에 대해 이 코드가 일할거라는 보증이 불가능하여, 코드를 컴파일하려 할 때 적절한 에러를 보고합니다.
