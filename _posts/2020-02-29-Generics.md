...
이 새로운 `isTop(_:)` 메소드는 먼저 스택이 비었는지 검사한 다음, 주어진 항목을 스택의 가장 윗 항목과 비교합니다. 이걸 일반화 `where` 절 없이 하려고 하면, 문제가 생기는데: `isTop(_:)` 구현에서 `==` 연산자를 쓰고 있지만, `Stack` 의 정의에선 항목의 같음 비교가 가능해야 한다고 요구하지 않아서, `==` 연산자를 쓰는 결과가 컴파일-시간 에러가 될거라는 겁니다. 일반화 `where` 절을 쓰는 건 새로운 필수 조건을 익스텐션에 추가해서, 스택 안에 있는 항목이 비교 가능할 때만 익스텐션이 `isTop(_:)` 메소드를 추가하도록 해줍니다.
...
`isTop(_:)` 메소드를 원소의 같음 비교가 불가능한 스택에 대해서 호출하려고 하면, 컴파일-시간 에러를 갖게 됩니다:
...
일반화 `where` 절을 프로토콜의 익스텐션에 쓸 수도 있습니다. 아래 예제는 이전 예제에 있던 `Container` 프로토콜을 확장하여 `startWith(_:)` 메소드를 더합니다.
...
`startsWith(_:)` 메소드는 먼저 컨테이너에 적어도 하나의 항목이 있는지 확인한 다음, 컨테이너의 첫 번째 항목과 주어진 항목이 들어 맞는지 검사합니다. 이 새로운 `startsWith(_:)` 메소드는, 위에서 쓴 스택과 배열을 포함하여, `Container` 프로토콜을 따르는 어떠한 타입과도 쓸 수 있는데, 컨테이터 항목이 비교 가능하기만 하면 됩니다.
...
위 예제의 일반화 `where` 절은 `Item` 이 어떤 한 프로토콜을 따르길 요구하지만, 일반화 `where` 절에서 `Item` 이 특정한 타입이길 요구하게 할 수도 있습니다. 예를 들면 다음과 같습니다:
...
이 예제는 자신의 `Item` 타입이 `Double` 인 컨테이너에 `average()` 메소드를 추가합니다. 컨테이너 안에 있는 항목들을 반복하면서 더하고, 컨테이너에 있는 횟수로 나누어 평균을 계산합니다. 횟수를 `Int` 에서 `Double` 로 변환하는 걸 명시하여 부동-소수점 나누기가 가능하도록 합니다.

익스텐션 부분의 일반화 `where` 절도 여러 개의 필수 조건을 포함할 수 있는데, 이건 다른 곳에서 작성한 일반화 `where` 절에서 그럴 수 있는 것과 같습니다. 목록에서 각각의 필수 조건을 구분하는 건 쉼표로 합니다.

### Contextual Where Clauses (상황별 where 절)

이미 일반화 타입인 상황 속에서 작업할 땐, 자신만의 일반화 타입 구속 조건이 없는 선언 부분에서 일반화 `where` 절을 쓸 수 있습니다. 예를 들어, 일반화 타입의 첨자에 대해서 또는 일반화 타입의 익스텐션에 있는 메소드에 대해서 일반화 `where` 절을 쓸 수 있습니다. 아래 예제에서, `Container` 구조체는 일반화된 것이고, `where` 절은 어떤 타입 구속 조건을 만족해야 이 새로운 메소드를 컨테이너에서 쓸 수 있는 지를 지정합니다.
...
이 예제는 항목이 정수일 땐 `Container` 에 `average()` 메소드를 추가하고, 항목이 비교 가능한 것일 땐 `endsWith(_:)` 메소드를 추가합니다. 두 함수 다 일반화 `where` 절을 포함하고 있어서 원본 `Container` 선언의 일반화 타입 매개 변수인 `Item` 에 타입 구속 조건을 추가합니다.

이런 코드를 상황별 `where` 절을 쓰지 않고 작성하려면, 각각의 일반화 `where` 절마다 하나씩, 두 개의 익스텐션을 작성합니다. 위에 있는 예제와 아래에 있는 예제는 동작이 똑같습니다.
...
이 예제에서 상황별 `where` 절을 쓴 버전은, `average()` 와 `endsWith(_:)` 의 구현이 모두 똑같은 익스텐션 안에 있는데 각각의 메소드에 있는 일반화 `where` 절이 그 메소드를 쓰려면 만족해야 할 필수 조건을 알려주기 때문입니다. 이 필수 조건들을 익스텐션의 일반화 `where` 절로 옮기게 되면 똑같은 상황에서 메소드를 쓸 수 있긴 하지만, 필수 조건마다 하나의 익스텐션이 필요하게 됩니다.

### Associated Types with a Generic Where Clause (일반화 where 절이 있는 결합 타입)

결합 타입에 일반화 `where` 절을 포함시킬 수 있습니다. 예를 들어 어떤 `Container` 버전에선, `Sequence` 프로토콜이 표준 라이브러리에서 쓰는 것 같은, 반복자[^iterator] 를 포함시키고 싶다고 가정해 봅시다. 그걸 써보면 이렇습니다:
...
`Iterator` 에 있는 일반화 `where` 절은 반드시 반복자가, 반복자의 타입과는 상관없이, 컨테이너의 항목과 똑같은 항목 타입의 원소 사이를 오고 가길 요구합니다. `makeIterator()` 함수는 컨테이너 반복자로의 접근을 제공합니다.

또 다른 프로토콜을 상속한 프로토콜에서, 상속한 결합 타입에 구속 조건을 추가하려면 프로토콜 선언에 일반화 `where` 절을 포함시키면 됩니다. 예를 들어, 다음 코드에서 선언한 `ComparableContianer` 프로토콜은 `Item` 이 `Comparable` 을 따르기를 요구합니다:
...
첨자도 일반화할 수 있으며, 일반화 `where` 절을 포함시킬 수도 있습니다. 자리 표시용 타입 이름은 `subscript` 뒤의 꺾쇠 괄호 안에 작성하며, 일반화 `where` 절은 첨자 본문을 여는 중괄호 바로 앞에 작성합니다. 예를 들면 다음과 같습니다:
...
이번 익스텐션에서 `Container` 프로토콜에 추가한 첨자는 일렬로 나열된 색인을 입력 받아 주어진 색인 위치에 있는 항목을 담은 배열을 반환합니다. 이 일반화 첨자의 구속 조건은 다음과 같습니다:

* 꺽쇠 괄호 안에 있는 `Indices` 일반화 매개 변수[^generic-parameter] 는 표준 라이브러리에 있는 `Sequence` 프로토콜을 따르는 타입이어야 합니다.
* 첨자가 입력 받는 단 하나의 매개 변수인, `indices` 는, 그 `Indices` 타입의 인스턴스입니다.
* 일반화 `where` 절은 시퀀스의 반복자가 반드시 `Int` 타입의 원소 사이를 오고 가길 요구합니다. 이는 시퀀스의 색인이 컨테이너에서 쓰는 색인과 똑같은 타입임을 보장합니다.

함께 묶어보면, 이 구속 조건들이 의미하는 건 `indices` 매개 변수에 전달된 값이 일렬로 나열된 정수들이라는 겁니다.

### 다음 장

[Opaque Types and Boxed Types (불투명 타입과 상자친 타입) >]({% link docs/swift-books/swift-programming-language/opaque-types.md %})
