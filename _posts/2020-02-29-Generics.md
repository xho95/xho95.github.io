...
이 새로운 `isTop(_:)` 메소드는 첫 번째로 스택이 비었는지 검사한 다음, 주어진 항목과 스택의 가장 윗 항목을 비교합니다. 일반화 `where` 절 없이 이를 하려 하면, 문제가 있을건데: `isTop(_:)` 구현은 `==` 연산자를 사용하지만, `Stack` 정의는 자신의 항목이 같음 비교가 가능하길 요구하지 않아서, `==` 연산자의 사용이 컴파일-시간 에러가 되버린다는 겁니다. 일반화 `where` 절의 사용은 익스텐션에 새로운 필수 조건을 추가하게 해줘서, 스택 항목이 비교 가능할 때만 익스텐션이 `isTop(_:)` 메소드를 추가합니다.
