...
`startsWith(_:)` 메소드는 먼저 컨테이너에 적어도 하나의 항목을 있다는 걸 확실히 한 다음, 컨테이너의 첫 번째 항목과 주어진 항목이 일치하는 지 검사합니다. 이 새로운 `startsWith(_:)` 메소드는, 위에서 사용한 '스택' 과 '배열' 을 포함하여, '컨테이터' 의 항목이 '비교 가능' 하기만 하다면, `Container` 프로토콜을 준수하는 어떤 타입과도 사용할 수 있습니다.
