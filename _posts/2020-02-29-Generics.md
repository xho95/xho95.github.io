...
위의 예제에서, `Stack` 에 결합된 타입인 `Suffix` 도 또 `Stack` 이므로, `Stack` 에 대한 '접미사 연산 (suffix operation)' 은 또 다른 `Stack` 을 반환합니다. 다른 방법으로는, `SuffixableContainer` 를 준수하는 타입이 그 자신과는 다른 `Suffix` 타입을 가져서-'접미사 연산' 이 다른 타입을 반환할 수 있다는 것을 의미하도록-만들 수도 있습니다. 예를 들어, 다음은 제네릭이-아닌 `IntStack` 타입에, '접미사 타입 (suffix type)' 으로 `IntStack` 대신 `Stack<Int>` 를 사용하는, `SuffixableContainer` 준수성을 추가하는 '익스텐션' 입니다:
