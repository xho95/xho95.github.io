...
제네릭 타입의 '익스텐션' 은 확장된 타입의 인스턴스가 새로운 기능을 획득하기 위해 반드시 만족해야 하는 '필수 조건 (requirements)' 을 포함할 수도 있는데, 이는 아래의 [Extensions with a Generic Where Clause (제네릭 where 구절을 가지는 익스텐션; 일반화된 where 구절을 가지는 확장)](#extensions-with-a-generic-where-clause-제네릭-where-구절을-가지는-익스텐션-일반화된-where-구절을-가지는-확장) 에서 설명합니다.
...
이 프로토콜에서, `Suffix` 는, 위의 `Containter` 에 있는 `Item` 타입과 같은, 결합된 타입입니다. `Suffix` 는 두 개의 구속 조건을 가집니다: (지금 정의하고 있는 프로토콜인) `SuffixableContainer` 프로토콜을 반드시 준수해야 한다는 것과, 이의 `Item` 타입은 컨테이너의 `Item` 타입과 반드시 같아야 한다는 것입니다. `Item` 의 구속 조건은 '제네릭 `where` 구절 (generic `where` clause; 일반화된 where 구절)' 이며, 이는 아래의 [Associated Types with a Generic Where Clause (제네릭-일반화된 where 구절을 가지는 결합된 타입)](#associated-types-with-a-generic-where-clause-제네릭-일반화된-where-구절을-가지는-결합된-타입) 에서 설명합니다.
...
'결합된 타입 (associated type)' 에 대한 '필수 조건' 을 정의하는 것 또한 유용할 수 있습니다. 이렇게 하려면 '_제네릭 where 구절 (generic where clause; 일반화된 where 구절)_' 을 정의하면 됩니다.

'제네릭 `where` 구절' 은 '결합된 타입' 이 정해진 프로토콜을 반드시 준수하도록 필수 조건을 요구할 수 있게 해주기도 하고, 아니면 정해진 '타입 매개 변수' 와 '결합된 타입' 반드시 같아야 한다는 필수 조건을 요구할 수 있게 해줍니다. 제네릭 `where` 구절은 `where` 키워드로 시작한 다음, 그 뒤에 '결합된 타입' 에 대한 '구속 조건' 이나 아니면 타입과 결합된 타입 사이의 '동등 관계 (equality relationships)' 등이 따라옵니다. 제네릭 `where` 구절은 타입 또는 함수 본문의 시작 괄호 바로 앞에 작성합니다.

아래 예제는 `allItemsMatch` 라는 '제네릭 함수' 를 정의하는데, 이는 두 `Container` 인스턴스가 같은 항목을 같은 순서로 담고 있는 지를 확인하기 위해 검사합니다. 이 함수는 모든 항목이 일치하면 `true` 라는 '불린 (Boolean)' 값을 반환하고 그렇지 않으면 `false` 값을 반환합니다.

검사할 두 컨테이너가 (비록 가능하다 할지라도) 같은 타입의 컨테이너일 필요는 없지만, 같은 타입의 항목을 보유하고 있는 것들 이어야 합니다. 이런 '필수 조건' 은 '타입 구속 조건' 과 '제네릭 `where` 구절' 의 '혼합 (combination)' 을 통해서 표현됩니다:
...
첫 번째와 두 번째 필수 조건은 함수의 '타입 매개 변수 목록 (type parameter list)' 에서 정의하며, 세 번째와 네 번째 필수 조건은 '제네릭 (generic; 일반화된) `where` 구절' 에서 정의하고 있습니다.
...
위 예제는 `String` 값을 저장하는 `Stack` 인스턴스를 생성하고, 세 개의 문자열을 이 '스택' 에 밀어 넣습니다. 이 예제는 `Array` 인스턴스도 생성하여 스택과 똑같은 세 개의 문자열를 담는 '배열 글자 표현 (array literal; 배열 리터럴)' 으로 초기화합니다. '스택' 과 '배열' 은 서로 다른 타입이지만, 이들 모두 `Container` 프로토콜을 준수하며[^conform-to-the-container], 모두 같은 타입의 값을 가지고 있습니다. 그러므로 이 두 컨테이너를 인자로 넘겨서 `allItemsMatch(_:_:)` 함수를 호출할 수 있습니다. 위에 있는 예제에서는, `allItemsMatch(_:_:)` 함수가 두 컨테이터에 있는 모든 항목들이 일치한다고 올바르게 보고하고 있습니다.

### Extensions with a Generic Where Clause (제네릭 where 구절을 가지는 익스텐션; 일반화된 where 구절을 가지는 확장)

'제네릭 `where` 구절 (generic `where` clause; 일반화된 `where` 구절)' 을 '익스텐션 (extension; 확장)' 에서 사용할 수도 있습니다. 아래 예제는 이전 예제의 '제네릭 `Stack` 구조체' 를 확장하여 `isTop(_:)` 메소드를 추가합니다.
...
이 새로운 `isTop(_:)` 메소드는 스택이 빈 것이 아닌지를 먼저 검사한 다음, 주어진 항목을 스택의 최상위 항목과 비교합니다. 이걸 '제네릭 `where` 구절' 없이 하려고 하면, 문제가 생기게 됩니다: `isTop(_:)` 의 구현은 `==` 연산자를 사용하지만, `Stack` 의 정의는 자신의 항목이 '같음 비교가 가능할 (equatable)' 것을 필수로 요구하지 않으므로, `==` 연산자를 사용하는 것은 '컴파일-시간 에러' 로 끝나게 됩니다. '제네릭 `where` 구절' 을 사용하는 것은 '익스텐션 (extension)' 에 새로운 '필수 조건' 을 추가하도록 하는 것이므로, 이 '익스텐션 (extension)' 은 스택에 있는 항목이 비교 가능할 때에만 `isTop(_:)` 메소드를 추가합니다.
...
'제네릭 `where` 구절' 은 프로토콜에 대한 '익스텐션 (extension)' 에서 사용할 수 있습니다. 아래 예제는 이전 예제에 있는 `Container` 프로토콜을 확장하여 `startWith(_:)` 메소드를 추가합니다.
...
위 예제에 있는 '제네릭 `where` 구절' 이 필수로 요구하는 것은 `Item` 이 프로토콜을 준수해야 한다는 것이지만, `Item` 이 지정된 타입일 것을 필수로 요구하는 '제네릭 `where` 구절' 을 작성할 수도 있습니다. 예를 들면 다음과 같습니다:
...
'익스텐션 (extension)' 에 있는 '제네릭 `where` 구절' 은 여러 개의 필수 조건을 포함할 수 있는데, '제네릭 `where` 구절' 을 다른 곳에서 작성할 때 그렇게 하는 것과 같다고 보면 됩니다. 목록에 있는 각각의 필수 조건은 쉼표로 구분하면 됩니다.

### Contextual Where Clauses (상황별 where 구절)

이미 제네릭 타입인 상황에서 작업하는 중이라면, 자신이 제네릭 타입 구속 조건을 가지고 있지 않은 선언에서도 제네릭 `where` 구절을 작성할 수 있습니다. 예를 들어, 제네릭 타입의 첨자 연산에 대해서 또는 제네릭 타입에 대한 '확장 (extension)' 에 있는 메소드에 대해서 제네릭 `where` 구절을 작성할 수 있습니다.

`Container` 구조체는 '제네릭' 이며, 아래 예제에 있는 `where` 구절은 컨테이너에서 이 새로운 메소드를 사용 가능하게 만들려면 무슨 타입 구속 조건을 만족해야 하는 지를 지정합니다.
...
이 예제는 항목이 정수일 때는 `Container` 에 `average()` 메소드를 추가하며, 항목이 '비교 가능 (equatable)' 할 때는 `endsWith(_:)` 메소드를 추가합니다. 두 함수 모두 `Container` 의 원래 선언에 있는 제네릭 `Item` 타입 매개 변수에 타입 구속 조건을 추가하는 '제네릭 `where` 구절' 을 포함하고 있습니다.

이러한 코드를 '상황별 `where` 구절 (contextual `where` clause)' 를 사용하지 않고 작성하려면, 각각의 '제네릭 `where` 구절' 에 대해 하나씩, 두 개의 '익스텐션 (extensions)' 을 작성해야 합니다. 위 예제와 아래 예제는 동일한 동작을 합니다.
...
이 예제에서 '상황별 `where` 구절' 을 사용하는 버전은, `average()` 와 `endsWith(_:)` 둘 다 같은 '익스텐션 (extension; 확장)' 안에서 구현하는데 이는 각 메소드의 '제네릭 `where` 구절' 이 해당 메소드를 사용 가능하게 만들기 위한 '필수 조건' 을 알려주기 때문입니다. 이러한 필수 조건을 '익스텐션' 의 '제네릭 `where` 구절' 로 옮기는 것은 동일한 상황에서 메소드를 사용 가능하게 하기 위해서, 각 '필수 조건' 마다 '익스텐션' 을 하나씩 만들어야 하는 것을 의미합니다.

### Associated Types with a Generic Where Clause (제네릭-일반화된 where 구절을 가지는 결합된 타입)

결합된 타입에 '제네릭 `where` 구절 (generic `where` 구절; 일반화된 `where` 구절)' 을 포함시킬 수 있습니다. 예를 들어, `Container` 의 한 버전에서, 표준 라이브러리에 있는 `Sequence` 프로토콜이 사용하는 것 같은, '반복자 (iterator)' 를 포함시키고 싶다고 가정해 봅시다. 이를 작성하는 방법은 다음과 같습니다:
...
`Iterator` 에 대한 '제네릭 `where` 구절' 은 '반복자' 가, '반복자' 의 타입과는 관계없이, 반드시 컨테이너 항목과 항목 타입이 같은 원소들 사이를 왕래할 수 있어야 함을 필수로 요구합니다. `makeIterator()` 함수는 컨테이너의 반복자에 대한 접근을 제공합니다.

다른 프로토콜을 상속하는 프로토콜에서, 상속받은 결합된 타입에 구속 조건을 추가하려면 프로토콜 선언에 '제네릭 `where` 구절' 을 포함하면 됩니다. 예를 들어, 다음 코드는 `Comparable` 을 준수하는 `Item` 을 필수로 요구하는 `ComparableContianer` 프로토콜을 선언합니다:
...
'첨자 연산 (subscript)' 은 '제네릭' 일 수 있고, '제네릭 `where` 구절' 을 포함할 수 있습니다. `subscript` 뒤의 꺾쇠 괄호 안에 '자리 표시용 타입 이름 (placeholder type name)' 을 작성하고, 첨자 연산 본문의 시작 중괄호 바로 전에 '제네릭 `where` 구절' 을 작성합니다. 예를 들면 다음과 같습니다:
...
* '제네릭 `where` 구절' 은 이 수열에 대한 반복자가 반드시 타입이 `Int` 인 원소들 사이를 왕래해야 함을 필수로 요구합니다. 이는 수열에 있는 색인들이 컨테이너에서 사용되는 색인들과 같은 타입임을 보장합니다.

다같이 취합해보면, 이 구속 조건들이 의미하는 것은 `indices` 매개 변수로 전달되는 값은 정수 수열이라는 것입니다.
