...
일반화 타입의 익스텐션은, 밑에 있는 [Extensions with a Generic Where Clause (일반화 where 절을 가진 익스텐션)]({% post_url 2020-02-29-Generics %}#extensions-with-a-generic-where-clause-일반화-where-절을-가진-익스텐션) 에서 논의한 것처럼, 필수 조건을 포함하여 확장한 타입의 인스턴스가 이를 반드시 만족해야 새로운 기능을 얻도록 할 수도 있습니다.
