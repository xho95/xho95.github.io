...
다음은 위의 [Generic Types (일반화 타입)](#generic-types-일반화-타입) 에 있는 제네릭이-아닌 `IntStack` 타입이, `Container` 프로토콜을 준수하도록 채택한 것입니다:
...
다음은 위 [Generic Types (일반화 타입)](#generic-types-일반화-타입) 의 `Stack` 타입에 `SuffixableContainer` 프로토콜에 대한 준수성을 추가하는 '익스텐션 (extension)' 입니다:
