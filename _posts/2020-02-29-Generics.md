...
기존 유형을 확장하여 연관된 유형 지정
확장에 프로토콜 적합성 추가에 설명 된대로 기존 유형을 확장하여 프로토콜에 적합성을 추가 할 수 있습니다. 여기에는 연관된 유형의 프로토콜이 포함됩니다.

Swift의 Array 유형은 이미 append (_ :) 메소드, count 속성 및 Int 인덱스가있는 첨자를 제공하여 해당 요소를 검색합니다. 이 세 가지 기능은 컨테이너 프로토콜의 요구 사항과 일치합니다. 이는 Array가 프로토콜을 채택한다고 선언함으로써 컨테이너 프로토콜에 맞게 Array를 확장 할 수 있음을 의미합니다. 확장을 사용하여 프로토콜 채택 선언에 설명 된대로 빈 확장으로이 작업을 수행합니다.

확장 배열 : 컨테이너 {}
Array의 기존 append (_ :) 메소드와 아래 첨자는 Swift가 위의 일반 스택 유형과 마찬가지로 Item에 사용할 적절한 유형을 유추 할 수 있도록합니다. 이 확장을 정의한 후에는 모든 배열을 컨테이너로 사용할 수 있습니다.
