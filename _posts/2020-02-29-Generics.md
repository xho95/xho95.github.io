...
`findIndex(ofString:in:)` 의 일반화 버전인, `findIndex(of:in:)` 라고, 예상할 법한 걸 작성하면 이렇습니다. 함수가, 배열에 있는 옵셔널 값이 아닌, 옵셔널 색인 번호를 반환하기 때문에, 이 함수의 반환 타입이 여전히 `Int?` 라는 걸 기억하기 바랍니다. 그렇지만, 경고 하건데-이 함수는, 예제 뒤에 설명한 이유로 인해, 컴파일되지 않습니다:
...
이 함수는 위에 적었다시피 컴파일되지 않습니다. 문제는, "`if value == valueToFind`" 라는, 같음 비교 검사에 있습니다. 스위프트의 모든 타입이 같음 비교 연산자 (`==`) 로 비교할 수 있는 건 아닙니다. 예를 들어, 자신만의 클래스나 구조체를 생성하여 복잡한 데이터 모델을 나타냈다면, 그 클래스나 구조체에 대한 "같음 (equal to)" 의 의미는 스위프트가 추측할 수 있는 것이 아닙니다. 이 때문에, 이 코드가 가능한 _모든 (every)_ 타입 `T` 와 작업할 거라는 보증이 불가능하며, 코드를 컴파일하려고 할 때 적절한 에러를 보고합니다.
