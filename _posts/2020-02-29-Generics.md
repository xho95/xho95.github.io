...
_일반화된 코드 (generic code)_ 는, 자신이 정의한 '필수 조건'[^requirements] 을 전제로, 어떤 타입과도 작업할 수 있는 유연하고, 재사용 가능한 함수와 타입을 작성하도록 해줍니다. 중복은 피하고 의도는 명확하고, 추상적인 방식으로 나타내는 코드를 작성할 수 있습니다.

'일반화 (generic)' 는 스위프트의 가장 강력한 특징 중의 하나이며, 대부분의 스위프트 표준 라이브러리는 '일반화된 (generic) 코드' 로 제작되었습니다. 사실, 알아차리지 못했겠지만, _언어 설명서 (Language Guide)_[^language-guide] 전반에 걸쳐 '일반화 (generics)' 를 계속 사용하고 있는 중입니다. 예를 들어, 스위프트의 `Array` 와 `Dictionary` 타입은 둘 다 '일반화된 집합체 (genenric colletion)' 입니다. `Int` 값을 쥐고 있는 배열, 또는 `String` 값을 쥐고 있는 배열, 아니면 진짜로 스위프트에서 생성할 수 있는 다른 어떤 타입의 배열이든 생성할 수 있습니다. 이와 비슷하게, 어떤 특정 타입의 값이든 저장하는 딕셔너리를 생성할 수 있는데, 해당 타입이 무엇인지에 대해서는 제한이 없습니다.

### The Problem That Generics Solve ('일반화' 로 푸는 문제)
...
`swapTwoInts(_:_:)`, `swapTwoStrings(_:_:)`, 및 `swapTwoDoubles(_:_:)` 함수 본문은 모든 점에서 똑같다는 것을 알아차렸을 것입니다. 유일한 차이점은 이들이 받아들이는 값이 (`Int`, `String`, 및 `Double`) 타입이라는 것입니다.

_어떤 (any)_ 타입의 두 값이든 서로 교환할 수 있는 단일 함수를 작성하는 것이, 더 유용하고, 또 더 유연합니다. '일반화된 (generic) 코드' 는 그런 함수를 작성하도록 해줍니다. (이 함수들을 일반화한 버전은 아래에 정의합니다.)
...
> 타입 매개 변수는, 값이 아니라, _타입 (type)_ 에 대한 자리를 표시한다는 것을 지시하기 위하여 (`T` 와 `MyTypeParamter` 같이) 항상 '낙타 모양 대문자 (upper camel case)' 이름을 부여하도록 합니다.
...
### Extending a Generic Type (일반화된 타입 확장하기)
...
제네릭 타입의 '익스텐션' 은 확장된 타입의 인스턴스가 새로운 기능을 획득하기 위해 반드시 만족해야 하는 '필수 조건 (requirements)' 을 포함할 수도 있는데, 이는 아래의 [Extensions with a Generic Where Clause (일반화된 'where' 절을 가진 익스텐션)]({% post_url 2020-02-29-Generics %}#extensions-with-a-generic-where-clause-일반화된-where-절을-가진-익스텐션) 에서 설명합니다.

### Type Constraints (타입 구속 조건)

`swapTwoValues​​(_:_:)` 함수와 `Stack` 타입은 어떤 타입이든 같이 작업할 수 있습니다. 하지만, 제네릭 (일반화된) 함수 및 제네릭 (일반화된) 타입과 같이 사용할 수 있는 타입에 대해 지정된 _타입 구속 조건 (type constraints)_ 을 강제하는 것이 유용할 때가 있습니다. '타입 구속 조건' 은 타입 매개 변수가 반드시 지정된 클래스를 상속받도록 지정하거나, 특정한 프로토콜 또는 특정한 '프로토콜 합성 (protocol composition)'[^protocol-composition] 을 준수하도록 지정합니다.

예를 들어, 스위프트의 `Dictionary` 타입은 딕셔너리의 키로 사용할 수 있는 타입에 대한 제한을 둡니다. [Dictionaries (딕셔너리)]({% post_url 2016-06-06-Collection-Types %}#dictionaries-딕셔너리) 에서 설명한 것처럼, 딕셔너리의 키 타입은 반드시 '_해시 가능 (hashable)_' 해야 합니다. 즉, 스스로를 유일하게 표현할 수 있는 방법을 반드시 제공해야 합니다. `Dictionary` 의 키가 '해시 가능 (hashable)' 해야 특정한 키에 대한 값을 이미 담고 있는 지 검사할 수 있습니다. 이러한 '필수 조건 (requirement)' 이 없다면, `Dictionary` 는 특정한 키에 대한 값을 집어 넣어야 할지 교체해야 할지 알 수가 없고, 주어진 키에 대한 값이 '딕셔너리' 에 이미 있는지 찾을 수도 없을 것입니다.
...
사용자 정의 제네릭 타입을 생성할 때 자신만의 타입 구속 조건을 정의할 수 있으며, 이러한 구속 조건이 '제네릭 프로그래밍 (genenric programming)' 이 가진 강력함의 대부분을 제공하는 것입니다. `Hashable` 같은 '추상적인 개념 (abstract concepts)' 은, '구체적으로 고정된 타입 (concrete type)' 이 아니라, '개념적인 성질 (conceptual characteristics)' 로써 타입의 성질을 규정합니다.
...
게다가, `IntStack` 은 이 `Container` 의 구현에서, `Item` 으로 사용하기 적절한 것은 `Int` 타입임을 지정합니다. `typealias Item = Int` 라고 정의하는 것은 `Container` 프로토콜의 구현에 대한 `Item` 이라는 '추상적인 타입 (abstract type)' 을 `Int` 라는 '구체적으로 고정된 타입 (concrete type)' 으로 바꿉니다.
...
기존 타입을 확장하면, [Adding Protocol Conformance with an Extension (익스텐션으로 프로토콜 준수성 추가하기)]({% post_url 2016-03-03-Protocols %}#adding-protocol-conformance-with-an-extension-익스텐션으로-프로토콜-준수성-추가하기) 에서 설명한 것처럼, 어떤 프로토콜에 '준수성 (conformance)' 을 추가할 수 있습니다. 이는 결합된 타입을 가지고 있는 프로토콜도 포함합니다.

스위프트의 `Array` 타입은 이미 `append(_:)` 메소드, `count` 속성, 그리고 원소를 가져오기 위한 `Int` 색인을 가지는 첨자 연산을 제공합니다. 이렇게 이미 보유한 세 개의 능력은 `Container` 프로토콜의 필수 조건과 일치합니다. 이것의 의미는 `Array` 를 확장하여 `Container` 프로토콜을 준수하도록 하려면 단순히 `Array` 가 이 프로토콜을 채택한다고 선언하기만 하면 된다는 것입니다. [Declaring Protocol Adoption with and Extension (확장으로 프로토콜을 채택한다고 선언하기)]({% post_url 2016-03-03-Protocols %}#declaring-protocol-adoption-with-and-extension-확장으로-프로토콜을-채택한다고-선언하기) 에서 설명한 것처럼, 이렇게 하려면 비어 있는 '익스텐션 (extension)' 을 사용합니다:
...
#### Using a Protocol in Its Associated Type's Constraints (프로토콜을 자신에게 결합된 타입의 구속 조건에서 사용하기)
...
검사할 두 컨테이너가 (비록 가능하다 할지라도) 같은 타입의 컨테이너일 필요는 없지만, 같은 타입의 항목을 보유하고 있는 것들 이어야 합니다. 이런 '필수 조건' 은 '타입 구속 조건' 과 '제네릭 `where` 절' 의 조합을 통해 표현됩니다:
...
세 번째와 네 번째 필수 조건을 조합하면 `anotherContainer` 에 있는 항목 _또한 (also)_ `!=` 연산자로 검사할 수 있다는 의미가 되는데, 왜냐면 이는 `someContainer` 에 있는 항목과 정확하게 똑같은 타입이기 때문입니다.
...
### Extensions with a Generic Where Clause (일반화된 'where' 절을 가진 익스텐션)
...
이미 '일반화된 (generic) 타입' 인 상황에서 작업하고 있을 때는, 자신만의 '일반화된 (generic) 타입 구속 조건' 을 가지고 있지 않은 선언에서도 '일반화된 (generic) `where` 절' 을 작성할 수 있습니다. 예를 들어, '일반화된 (generic) 타입' 의 첨자 연산에 대한 또는 '일반화된 (generic) 타입' 의 '익스텐션 (extension)' 에 있는 메소드에 대한 '일반화된 (generic) `where` 절' 을 작성할 수 있습니다. `Container` 구조체는 '일반화된 것 (generic)' 으로, 아래 예제에 있는 `where` 절은 이 새로운 메소드를 '컨테이너 (container)' 에서 사용 가능하게 만들기 위해 무슨 타입 구속 조건을 만족해야 하는 지를 지정합니다.
...
이 예제는 항목이 정수일 때는 `Container` 에 `average()` 메소드를 추가하며, 항목이 '비교 가능 (equatable)' 할 때는 `endsWith(_:)` 메소드를 추가합니다. 두 함수 모두 원래의 `Container` 선언에 있는 '일반화된 (generic) `Item` 타입 매개 변수' 에 타입 구속 조건을 추가하는 '일반화된 (generic) `where` 절' 을 포함합니다.

이러한 코드를 '상황별 `where` 절' 를 사용하지 않고 작성하려면, 각각의 '일반화된 (generic) `where` 절' 마다 하나씩, 두 개의 '익스텐션 (extensions)' 을 작성합니다. 위 예제와 아래 예제는 작동 방식이 같습니다.
...
이 예제에서 '상황별 `where` 절' 을 사용하는 버전은, `average()` 와 `endsWith(_:)` 의 구현 모두 같은 '익스텐션 (extension)' 안에 있는데 왜냐면 해당 메소드를 사용 가능하게 만들기 위해 만족해야 하는 '필수 조건' 을 각 메소드의 '일반화된 (generic) `where` 절' 이 알려주기 때문입니다. 이러한 필수 조건을 '익스텐션' 의 '일반화된 (generic) `where` 절' 로 옮기는 것은 메소드를 사용 가능하게 만드는 상황은 같지만, 한 '익스텐션' 마다 '필수 조건' 을 하나씩 요구합니다.

### Associated Types with a Generic Where Clause (일반화된 (generic) where 절을 가지는 결합된 타입)

결합된 타입에 '일반화된 (generic) `where` 절' 을 포함시킬 수 있습니다. 예를 들어, `Container` 의 한 버전에서, 표준 라이브러리에 있는 `Sequence` 프로토콜이 사용하는 것 같은, '반복자 (iterator)' 를 포함시키고 싶다고 가정해 봅시다. 이를 작성하는 방법은 다음과 같습니다:
...
`Iterator` 에 대한 '일반화된 (generic) `where` 절' 은 '반복자' 가, '반복자' 의 타입과는 관계없이, 반드시 컨테이너 항목과 항목 타입이 같은 원소들 사이를 왕래할 수 있어야 함을 필수로 요구합니다. `makeIterator()` 함수는 컨테이너의 반복자에 대한 접근을 제공합니다.

다른 프로토콜을 상속하는 프로토콜에서, 상속받은 결합된 타입에 구속 조건을 추가하려면 프로토콜 선언에 '일반화된 (generic) `where` 절' 을 포함하면 됩니다. 예를 들어, 다음 코드는 `Comparable` 을 준수하는 `Item` 을 필수로 요구하는 `ComparableContianer` 프로토콜을 선언합니다:

```swift
protocol ComparableContainer: Container where Item: Comparable { }
```

### Generic Subscripts (일반화된 (generic) 첨자 연산)

'첨자 연산 (subscript)' 은 '일반화 (generic)' 일 수 있고, '일반화된 (generic) `where` 절' 을 포함할 수 있습니다. `subscript` 뒤의 꺾쇠 괄호 안에 '자리 표시용 타입 이름 (placeholder type name)' 을 작성하고, 첨자 연산 본문의 여는 중괄호 바로 전에 '일반화된 (generic) `where` 절' 을 작성합니다. 예를 들면 다음과 같습니다:
...
`Container` 프로토콜에 대한 이 '익스텐션 (extension)' 은 색인으로 된 수열을 받아서 각각의 주어진 색인에 있는 항목을 담은 배열을 반환하는 첨자 연산을 추가합니다. 이 '일반화된 (generic) 첨자 연산' 은 다음과 같이 구속되어 있습니다:

* 꺽쇠 괄호 안에 있는 '일반화된 (generic) 매개 변수'[^generic-parameter] 인 `Index` 는 표준 라이브러리에 있는 `Sequence` 프로토콜을 준수하는 타입이어야 합니다.
* 첨자 연산이 받아 들이는 단일 매개 변수인, `index`는, 이 `Indices` 타입의 인스턴스입니다.
* '일반화된 (generic) `where` 절' 은 이 수열에 대한 반복자가 반드시 타입이 `Int` 인 원소들 사이를 왕래해야 함을 필수로 요구합니다. 이는 수열에 있는 색인들이 컨테이너에서 사용되는 색인들과 같은 타입임을 보장합니다.
...
[^protocol-composition]: '프로토콜 합성 (protocol composition)' 에 대해서는 [Protocol Composition (프로토콜 합성)]({% post_url 2016-03-03-Protocols %}#protocol-composition-프로토콜-합성) 부분을 참고하기 바랍니다.
