...
아래 예제는 `allItemsMatch` 라는 '제네릭 함수' 를 정의하는데, 이는 두 `Container` 인스턴스가 같은 항목을 같은 순서로 담고 있는 지를 확인하는 검사를 수행합니다. 이 함수는 모든 항목이 일치하면 `true` 라는 '불린 (Boolean)' 값을 반환하며 그렇지 않은 경우 `false` 값을 반환합니다.
