...
'포장 (wrapper)' 은 반드시 '`wrappedValue` 인스턴스 속성' 을 정의해야 합니다. 속성의 _포장 값 (wrapped value)_ 은 '이 속성의 획득자 (getter) 와 설정자 (setter) 가 노출하는 값' 입니다. 대부분의 경우, `wrappedValue` 는 '계산 값 (computed value)' 이지만, 대신 '저장 값 (stored value)' 일 수도 있습니다. '포장' 은 '자신의 포장 값에 필요한 어떤 실제 저장 공간' 도 정의하고 관리합니다. 컴파일러는 '포장 타입의 인스턴스에 대한 저장 공간' 을 '포장 속성의 이름 앞에 밑줄 (`_`) 접두사를 붙임' 으로써 통합합니다-예를 들어, `someProperty` 의 포장은 `_someProperty` 로 저장합니다. '포장을 위한 통합 저장 공간' 은 '`private` 접근 제어 수준' 입니다.
