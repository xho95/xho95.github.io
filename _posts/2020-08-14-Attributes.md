...
라이브러리 진화 모드에서, 동결 아닌 구조체 및 열거체 멤버와 상호 작용하는 코드는 미래 버전의 라이브러리가 그 타입의 일부 멤버를 추가, 삭제, 또는 재배치하더라도 재-컴파일 없이 작업을 계속 허용하는 방식으로 컴파일합니다. 컴파일러는 실행 시간에 정보 찾아보기 및 간접 계층 추가하기 같은 기술을 써서 이를 가능하게 합니다. 구조체나 열거체를 동결로 만드는 건 이런 유연함을 포기하면서 성능을 얻는 것인데: 미래 버전의 라이브러리에서 타입을 바꾸는 데는 제한이 걸리지만, 컴파일러가 타입 멤버와 상호 작용하는 코드를 추가로 최적화할 수 있게 됩니다.
...
이 특성을 익스텐션에 적용하하는 건 '`objc` 특성을 명시하지 않은 모든 해당 익스텐션 멤버에 이를 적용한 것' 과 똑같은 효과를 가집니다.
...
이 특성을 익스텐션에 적용하는 건 '`nonobjc` 특성을 명시하지 않은 모든 해당 익스텐션 멤버에 이를 적용한 것' 과 똑같은 효과를 가집니다.
...
결과 제작자 타입을 생성하는 건 '동일한 이름을 가진 사용자 정의 특성' 을 생성합니다. 그러한 특성을 다음 장소에 적용할 수 있습니다:

* 함수 선언에서, 결과 제작자는 함수 본문을 제작합니다.
* 획득자를 포함한 변수나 첨자 연산 선언에서, 결과 제작자는 획득자 본문을 제작합니다.
* 함수 선언에 있는 매개 변수에서, 결과 제작자는 해당 인자로 전달한 클로저 본문을 제작합니다.

결과 제작자 특성을 적용하는 건 'ABI 호환성' 에 큰 충격을 주지 않습니다. 결과 제작자 특성을 매개 변수에 적용하는 건 그 특성을 함수 인터페이스의 일부분이 되도록 하여, 소스 호환성에 효과를 줄 수 있습니다.
