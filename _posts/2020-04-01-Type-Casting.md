...
스위프트는 지정하지 않은 타입과의 작업을 위해 두 개의 특수한 타입을 제공합니다:

* `Any` 는 아예, 함수 타입을 포함한, 어떤 타입의 인스턴스든 표현할 수 있습니다.
* `AnyObject` 는 어떤 클래스 타입의 인스턴스든 표현할 수 있습니다.

`Any` 와 `AnyObject` 는 이들이 제공하는 작동 방식과 보유 능력이 명시적으로 필요할 때에만 사용합니다. 코드에서 작업할 것으로 예상하는 타입은 이를 지정하는 것이 항상 더 좋습니다.

다음은, '함수 타입' 과 '비-클래스 타입' 을 포한한, 서로 섞인 다른 타입들과 작업하기 위해 `Any` 를 사용하는 예제입니다. 이 예제는, `Any` 타입의 값을 저장할 수 있는, `things` 라는 배열을 생성합니다:
