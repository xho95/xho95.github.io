...
* `Any` 는, 함수 타입을 포함한, 어떤 타입의 인스턴스든 나타낼 수 있습니다.
* `AnyObject` 는 어떤 클래스 타입의 인스턴스든 표현할 수 있습니다.

`Any` 와 `AnyObject` 는 이들의 동작과 보유 능력이 명시적으로 필요할 때만 사용합니다. 코드에서 작업하고자 하는 타입을 지정하는 게 항상 더 좋습니다.

함수 타입 및 클래스 아닌 타입을 포함하여, 서로 다른 타입이 섞인 걸 작업하기 위해 `Any` 를 사용한 예제는 이렇습니다. 예제는, `Any` 타입의 값을 저장할 수 있는, `things` 라는 배열을 생성합니다:
