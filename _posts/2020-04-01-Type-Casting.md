...
타입 변환을 써서 타입이 프로토콜을 따르고 있는지 검사할 수도 있는데, 이는 [Checking for Protocol Conformance (프로토콜을 따르는지 검사하기)]({% link docs/swift-books/swift-programming-language/protocols.md %}#checking-for-protocol-conformance-프로토콜을-따르는지-검사하기) 에서 설명합니다.
...
이 예제는 현재 `item` 을 `Movie` 로 내림 변환하려는 걸로 시작합니다. `item` 은 `MediaItem` 의 인스턴스기 때문에, `Movie` _인 게 (might)_ 가능하며; 그와 같이, `Song` 이거나, 또는 심지어 기초 (타입인) `MediaItem` 인 것도 가능합니다. 이런 불확실함 때문에, `as?` 형식의 타입 변환 연산자는 하위 클래스 타입으로 내림 변환을 시도할 때 _옵셔널 (optional)_ 값을 반환합니다. `item as? Movie` 의 결과는 `Movie?`, 또는 "옵셔널 `Movie`" 타입입니다.

`Movie` 로의 내림 변환이 라이브러리 배열의 `Song` 인스턴스에 적용될 땐 실패입니다. 이걸 대처하려고, 위 예제는 옵셔널 연결을 써서 옵셔널 `Movie` 에 실제로 값이 담겼는지 검사합니다 (즉, 내림 변환이 성공인지 알아냅니다.) 이 옵셔널 연결을 쓰면 "`if let movie = item as? Movie`" 인데, 다음 처럼 읽을 수 있습니다:

“`item` 을 `Movie` 로 접근해 봅니다. 이게 성공하면, 반환된 옵셔널 `Movie` 안에 저장된 값을 `movie` 라는 새로운 임시 상수에 설정합니다.”

내림 변환이 성공하면, `movie` 의 속성을 써서 그 `Movie` 인스턴스의 설명을 인쇄하며, 여기엔 `director` 이름도 포함됩니다. 비슷한 원리로 `Song` 인스턴스도 검사하여, 라이브러리에서 `Song` 을 찾을 때마다 (`artist` 이름이 포함된) 적절한 설명을 인쇄합니다.

> 변환 (casting) 은 실제로 인스턴스를 수정하거나 값을 바꾸는게 아닙니다. 밑에 놓인 인스턴스는 (그대로) 똑같이 남아 있으며; 단순히 변환된 타입의 인스턴스로 취급하고 접근하는 것 뿐입니다.

### Type Casting for Any and AnyObject (Any 와 AnyObject 의 타입 변환)

스위프트는 정해지지 않은 불특정 타입과 작업하기 위해 두 개의 특수한 타입을 제공합니다:

* `Any` 는 아예 어떠한 타입의 인스턴스도 나타낼 수 있으며, 여기엔 함수 타입도 포함됩니다.
* `AnyObject` 는 어떠한 클래스 타입의 인스턴스라도 나타낼 수 있습니다.

`Any` 와 `AnyObject` 는 이들이 보유한 동작과 능력이 필요할 때만 쓰도록 합니다. 코드에서 작업이 예상되는 타입은 지정하는게 항상 더 좋습니다.

함수 타입과 클래스 아닌 타입을 포함하여, 서로 다른 타입이 섞인 걸 `Any` 로 작업하는 예제는 이렇습니다. 예제에선 `things` 라는 배열을 생성하는데, 이는 `Any` 타입의 값을 저장할 수 있습니다:
