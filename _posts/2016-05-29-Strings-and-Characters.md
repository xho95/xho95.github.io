...
두 `String` 값 (또는 두 `Character` 값) 은 자신들의 '확장된 자소 덩어리' 가 _법적으로 같은 값 (canonically equivalent)_[^canonically] 이면 같은 것으로 고려합니다. 확장된 자소 덩어리는, 속을 들여다보면 
서로 다른 유니코드 크기 값으로 구성한 경우에도, 언어의 (linguistic) 의미와 형태' 가 똑같으면 법적으로 동치' 입니다.
