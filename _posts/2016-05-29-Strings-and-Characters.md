...
_유니코드 (Unicode)_ 는 서로 다른 문자 체계에서 텍스트를 인코딩[^encoding] 하고, 나타내며, 가공하는 국제 표준입니다. 이는 '어떤 언어에 대해서든 거의 대부분의 문자를 표준 형식으로 표현' 하도록, 그리고 '텍스트 파일이나 웹 페이지 같은 외부 소스에서 해당 문자를 읽고 쓸 수' 있도록 해줍니다. 여기서 설명할 것처럼, 스위프트의 `String` 과 `Character` 타입은 온전히 유니코드를-따릅니다.
