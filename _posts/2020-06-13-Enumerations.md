...
대안으로, 다른 언어의 '공용체 (unions) 나 가변체 (variants)' 가 많이 하는 것처럼, 서로 다른 각각의 case 값과 나란히 저장할, _어떤 (any)_ 타입의 '결합 값 (associated values)' 이든 열거체 case 가 지정할 수 있습니다. 관련된 case 들의 공통 집합을, 제각각 자신과 결합된 적절한 타입의 서로 다른 값 집합을 가진, 한 열거체의 일부분로 정의할 수 있습니다.
