...
C 언어에 익숙하다면, C 열거체는 정수 값 집합에 관련된 이름을 할당한다는 걸 알고 있을 겁니다. 스위프트 열거체는 훨씬 더 유연해서, 열거체의 각 case 에 값을 제공하지 않아도 됩니다. 각각의 열거체 case 에 (_원시 값 (raw value)_ 이라는) 값을 제공한다면, 이 값은 문자열이나, 문자, 아니면 어딴 정수 또는 부동-소수점 타입의 값이든 될 수 있습니다.

대안으로, 다른 언어에서 '공용체 (unions) 나 가변체 (variants) 가 하는 것' 과 거의 같게, 서로 다른 각각의 case 값과 나란히 저장할, _어떤 (any)_ 타입의 '결합 값 (associated values)' 이든 열거체 case 가 지정할 수 있습니다. 관련된 case 들의 공통 집합을, 제각각 자신과 결합된 적절한 타입의 서로 다른 값 집합을 가진, 한 열거체의 일부분로 정의할 수 있습니다.
