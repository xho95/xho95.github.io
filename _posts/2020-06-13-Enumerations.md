...
예를 들어, 재고 추적 시스템이 제품 추적을 하는데 서로 다른 두 가지 타입의 바코드가 필요하다고 가정해 봅시다. 일부 제품의 이름표는 **UPC** 양식의 1-차원 바코드로, `0` 부터 `9` 까지의 수를 사용합니다. 각각의 바코드엔 시스템 숫자, 뒤에 제조사 코드 숫자 다섯 개와 제품 코드 숫자 다섯 개가 있습니다. 그 뒤엔 검사 숫자가 있어서 코드 스캔이 올바로 됐는지 밝혀냅니다:
