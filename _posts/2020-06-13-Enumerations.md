...
> 원시 값은 결합 값과 똑같지 _않습니다 (not)_. 원시 값은, 위의 ASCII 코드 세 개 같이, 코드에서 최초로 열거체를 선언할 때 미리 채울 값으로 설정합니다. 특정 열거체 case 의 원시 값은 항상 똑같습니다. 결합 값은 열거체 case 중 하나를 기초로 새로운 상수나 변수를 생성할 때 설정하며, 그럴 때마다 서로 다를 수 있습니다.
