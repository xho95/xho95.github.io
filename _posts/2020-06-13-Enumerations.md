...
C 언어에 익숙하다면, C 열거체는 정수 값 집합에 관련 이름을 할당한다는 걸 알고 있을 겁니다. 스위프트 열거체는 훨씬 더 유연해서, 열거체의 각 case 에 값을 제공하지 않아도 됩니다. 각각의 열거체 case 에 (_원시 값 (raw value)_ 이라는) 값을 제공한다면, 이 값은 '문자열 (string) 이나, 문자 (character), 아니면 어떤 정수 (integer) 또는 부동-소수점 (float-point) 타입의 값' 이든 될 수 있습니다.
