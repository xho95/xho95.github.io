...
스위프트의 모든 기본 타입 (가령 `String`, `Int`, `Double` 그리고 `Bool`) 은 기본적으로 해시 가능하므로, '셋 (set)' 의 값 타입과 '딕셔너리 (dictionary)' 의 키 (key) 타입으로 사용할 수 있습니다. '관련 값 (associated values)' 이 없는 열거체[^enumeration]의 경우 값 (enumeration case value; [Enumerations]() 에 설명되어 있습니다) 역시 기본적으로 해시 가능합니다.
...
'배열 글자표현' 으로 '셋 (set)' 을 초기화할 수도 있는데, 이는 '셋 컬렉션 (set collection)' 에 하나 이상의 값을 할당하는 '약칭법 (shorthand way)' 입니다.
