...
* 타입에서 첨자를 사용하는 것에 대한 정보를 [Type Subscripts (타입 첨자)]({% post_url 2020-03-30-Subscripts %}#type-subscripts-타입-첨자) 부분에 추가함.
...
* `Any` 타입인 값이 예상될 때 옵셔널 값을 사용하는 것에 대하여 [Type Casting for Any and AnyObject (Any 및 AnyObject 의 타입 변환)]({% post_url 2020-04-01-Type-Casting %}#type-casting-for-any-and-anyobject-any-및-anyobject-의-타입-변환) 에 기록을 추가함.
...
* '여러 개의 패턴 (multiple patterns)' 을 가진 'switch 문 case 절' 에 대한 정보를 [Control Flow (제어 흐름)]({% post_url 2020-06-10-Control-Flow %}) 장의 [Switch (Switch 문)]({% post_url 2020-06-10-Control-Flow %}#switch-switch-문) 부분과 [Statements (구문)]({% post_url 2020-08-20-Statements %}) 장의 [Switch Statement ('switch' 문)]({% post_url 2020-08-20-Statements %}#switch-statement-switch-문) 부분에 추가함.
...
* '옵셔널 사슬 (optional chaining)' 를 통한 할당 작동 방법을 분명하게 밝기기 위해 [Accessing Properties Through Optional Chaining (옵셔널 사슬을 통하여 속성 접근하기)]({% post_url 2020-06-17-Optional-Chaining %}#accessing-properties-through-optional-chaining-옵셔널-사슬을-통하여-속성-접근하기) 부분을 갱신함.
...
* 새롭게 `guard` 문에 대한 정보를 [Control Flow (제어 흐름)]({% post_url 2020-06-10-Control-Flow %}) 장의 [Early Exit (때 이른 탈출문)]({% post_url 2020-06-10-Control-Flow %}#early-exit-때-이른-탈출문) 부분과 [Statements (구문)]({% post_url 2020-08-20-Statements %}) 장의 [Guard Statement ('guard' 문)]({% post_url 2020-08-20-Statements %}#guard-statement-guard-문) 부분에 추가함.
...
* 새롭게 '옵셔널 패턴 (optional pattern)' 에 대한 정보를 [Patterns (패턴; 유형)]({% post_url 2020-08-25-Patterns %}) 장의 [Optional Pattern (옵셔널 패턴)]({% post_url 2020-08-25-Patterns %}#optional-pattern-옵셔널-패턴) 부분에 추가함.
...
* `Any` 타입인 상수와 변수는 이제 함수 인스턴스를 가질 수 있음. `switch` 문 내에서 함수 타입을 검사하고 변환하는 방법을 보여주기 위해 [Type Casting for Any and AnyObject (Any 및 AnyObject 의 타입 변환)]({% post_url 2020-04-01-Type-Casting %}#type-casting-for-any-and-anyobject-any-및-anyobject-의-타입-변환) 에 있는 예제를 갱신함.
