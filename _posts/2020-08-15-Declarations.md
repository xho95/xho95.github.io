...
던지는 함수나 메소드의 호출은 반드시 `try` 나 `try!` 표현식 (즉, `try` 나 `try!` 연산자 영역) 안에 포장해야 합니다.

`throws` 키워드는 함수 타입 내에 있는 것으로, '던지지 않는 (nonthrowing) 함수' 는 '던지는 함수' 의 하위 타입입니다. 그 결과, '던지지 것' 을 예상하고 있는 곳에서 '던지지 않는 함수' 를 사용할 수 있습니다.
...
'던지는 메소드' 는 '던지지 않는 메소드' 를 '재정의 (override)' 할 수 없으며, '던지는 메소드' 는 '던지지 않는 메소드' 를 위한 '프로토콜 필수 조건' 을 만족할 수 없습니다. 그렇다 하더라도, '던지지 않는 메소드' 는 '던지는 메소드' 를 재정의할 수 있으며, '던지지 않는 메소드' 는 '던지는 메소드' 를 위한 '프로토콜 필수 조건' 을 만족할 수 있습니다.
...
`async` 키워드는 함수 타입 내에 있는 것으로, '동기 (synchronous) 함수' 는 '비동기 함수' 의 하위 타입입니다. 그 결과, '비동기 함수' 를 예상하고 있는 곳에서 '동기 함수' 를 사용할 수 있습니다. 예를 들어, '비동기 메소드' 를 '동기 메소드' 로 재정의할 수 있으며, '비동기 메소드' 를 요구하는 '프로토콜 필수 조건' 을 '동기 메소드' 로 만족할 수도 있습니다. 
...
`AnyObject` 라는 '필수 조건' 으로 표시한 프로토콜을 상속한 프로토콜은 어떤 것이든 그 역시 마찬가지로 클래스 타입만이 채택할 수 있습니다.
