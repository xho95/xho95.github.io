...
_익스텐션 선언 (extension declaration)_ 은 기존 타입의 동작을 확장하도록 합니다. '익스텐션 선언' 은 `extension` 키워드로 선언하며 형식은 다음과 같습니다:

&nbsp;&nbsp;&nbsp;&nbsp;extension `type name-타입 이름` where `requirements-필수 조건` {<br />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`declarations-선언`<br />
&nbsp;&nbsp;&nbsp;&nbsp;}

익스텐션 선언의 본문은 0개 이상의 _선언 (declarations)_ 들을 담습니다. 이 _선언 (declarations)_ 들은 계산 속성, 계산 타입 속성, 인스턴스 메소드, 타입 메소드, 초기자, 첨자 연산 선언, 그리고 심지어 클래스, 구조체, 및 열거체 선언을 포함할 수 있습니다. '익스텐션 선언' 은 정리자나 프로토콜 선언, 저장 속성, 속성 관찰자, 또는 '다른 익스텐션 선언' 을 담을 순 없습니다. '프로토콜 익스텐션' 에 있는 선언은 `final` 로 표시할 수 없습니다. 다양한 종류의 선언을 포함하고 있는 '익스텐션' 에 대한 논의와 여러 예제들은, [Extensions (익스텐션; 확장)]({% post_url 2016-01-19-Extensions %}) 을 참고하기 바랍니다.
...
'일반화 (generic) 타입' 이나 '결합 타입을 가진 프로토콜' 을 확장하는 '익스텐션 선언' 은 _필수 조건 (requirements)_ 을 포함할 수 있습니다. '확장한 타입 또는 확장한 프로토콜을 준수하는 타입' 의 인스턴스가 _필수 조건 (requirements)_ 을 만족하면, 선언에서 지정한 동작을 인스턴스가 얻습니다.

'익스텐션 선언' 은 '초기자 선언' 을 담을 수 있습니다. 그렇다 하더라도, 확장하려는 타입이 다른 모듈에서 정의된 것이면, 초기자 선언이 해당 타입의 멤버를 알맞게 초기화함을 보장하도록 해당 모듈에 이미 정의한 초기자로 반드시 위임해야 합니다.

기존 타입의 속성, 메소드, 및 초기자를 해당 타입의 '익스텐션' 에서 재정의할 순 없습니다.

'익스텐션 선언' 은 기존 클래스, 구조체, 및 열거체에 _채택한 프로토콜 (adopted protocols)_ 을 지정함으로써 '프로토콜 준수성 (protocol conformance)' 을 추가할 수 있습니다:

&nbsp;&nbsp;&nbsp;&nbsp;extension `type name-타입 이름`: `adopted protocols-채택한 프로토콜` where `requirements-필수 조건` {<br />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`declarations-선언`<br />
&nbsp;&nbsp;&nbsp;&nbsp;}

'익스텐션 선언' 은 기존 클래스에 '클래스 상속' 을 추가할 순 없으며, 따라서 _타입 이름 (type name)_ 과 콜론 뒤에는 '프로토콜 목록' 만 지정할 수 있습니다.
