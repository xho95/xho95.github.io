...
_실패 가능 초기자 (failable initializer)_ 는 초기자를 선언한 타입에 대한 '옵셔널 인스턴스' 나 '암시적으로 포장을 푸는 (implicitly unwrapped) 옵셔널 인스턴스' 를 만들어 내는 타입의 초기자입니다. 그 결과로, '실패 가능 초기자' 는 초기화가 실패한 것을 나타내기 위해 `nil` 을 반환할 수 있습니다.
