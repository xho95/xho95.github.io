...
편의 초기자는 또 다른 편의 초기자나 클래스의 지명 초기자로 초기화 과정을 맡길 수 있습니다. 그렇더라도, 초기화 과정은 반드시 지명 초기자 호출로 끝나야 결국 클래스 속성을 초기화하게 됩니다. 편의 초기자는 상위 클래스 초기자를 호출할 수 없습니다.
...
_실패할 수 있는 초기자 (failable initializer)_ 는 초기자를 선언한 타입의 옵셔널 인스턴스나 암시적으로 포장 푸는 옵셔널 인스턴스를 만들어 내는 타입의 초기자입니다. 그 결과, 실패 가능 초기자는 `nil` 을 반환하여 초기화 실패를 지시할 수 있습니다.
...
실패 가능 초기자에 대한 더 많은 정보와 예제를 보려면, [Failable Initializers (실패할 수 있는 초기자)]({% link docs/swift-books/swift-programming-language/initialization.md %}#failable-initializers-실패할-수-있는-초기자) 부분을 보기 바랍니다.
...
{% include footer_swift_book.md %} 이 장의 원문은 [Declarations](https://docs.swift.org/swift-book/ReferenceManual/Declarations.html) 에서 볼 수 있습니다.
