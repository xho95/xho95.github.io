...
_임무 (task)_ 는 프로그램에서 비동기로 실행할 수 있는 작업의 단위입니다. 모든 비동기 코드는 어떠한 임무의 일부로써 실행합니다. 이전 부분에서 설명한 `async`-`let` 구문은 자식 임무를 생성합니다. 임무 그룹 (task group) 을 생성하고 그 그룹에 자식 임무를 추가할 수도 있는데, 이는 우선권 (priority) 과 취소 (cancellation) 에 대해 더 많은 제어권을 주며, 동적 개수의 임무를 생성하도록 해줍니다.
