...
이 예제에선, `logger.max` 로의 접근이 잠시 멈춤 가능 지점입니다. 왜냐면 '행위자' 는 한번에 자신의 '변경 가능 상태' 에 오직 한 '임무' 의 접근만을 허용해서, 또 다른 '임무' 코드가 '기록자 (logger)' 와 이미 상호 작용 중이면, 속성에 접근하길 기다리면서 이 코드를 멈춰달기 때문입니다.
