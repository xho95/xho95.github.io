...
스위프트는 비동기 및 병렬 코드를 구조적인 방식으로 작성하는 기능을 내장하고 있습니다. _비동기 코드 (asynchronous code)_ 는, 한번에 한 조각의 프로그램만 실행하긴 하지만, 이를 멈춰 매달고 나중에 다시 시작할 수 있습니다. 프로그램 코드를 멈춰 매달고 다시 시작하는 것은 네트워크 너머로 자료를 가져오거나 파일 구문을 해석하는 '장기 연산' 을 계속하는 동시에 'UI 갱신' 같은 '단기 연산' 도 계속 진행하게 해줍니다. _병렬 코드 (parallel code)_ 는 여러 코드 조각을 동시에 실행한다-예를 들어, 4-개의 코어 프로세서를 가진 컴퓨터는, 각 코어마다 하나의 임무를 실시하여, 동시에 코드 조각 네 개를 실행할 수 있다-는 의미입니다. 병렬 및 비동기 코드를 사용하는 프로그램은 한번에 여러 개의 연산을 실시하며; 외부 시스템을 기다리는 연산은 멈춰 매달고, 이 코드를 더 쉽게 메모리-안전한 방식으로 작성하도록 해줍니다.
...
이 장 나머지에서 _동시성 (concurrency)_ 이라는 용어는 이런 비동기 및 병렬 코드의 일반적인 조합을 언급하기 위해 사용합니다. 

> 이전에 동시성 코드를 작성해봤으면, 쓰레드 작업이 익숙할지도 모릅니다. 스위프트의 동시성 모델은 쓰레드 상에서 제작된 것이지만, 이들과 직접 상호 작용하진 않습니다. 스위프트의 비동기 함수는, 첫 번째 함수를 차단함과 동시에 해당 쓰레드에서 또 다른 비동기 함수를 실행하게 하도록, 현재 실행 중인 쓰레드를 포기할 수 있습니다.

스위프트 언어의 지원 기능을 사용하지 않고도 동시성 코드를 작성하는게 가능하긴 하지만, 그런 코드는 이해하기 더 어려운 경향이 있습니다. 예를 들어, 다음 코드는 사진 이름 목록을 내려받고, 해당 목록의 첫 번째 사진을 내려받은 다음, 사용자에게 사진을 보여줍니다:
...
이 단순한 경우에도, 코드를 '연속된 완료 처리자 (completion handlers)' 로 작성해야 하기 때문에, 중첩 클로저의 작성으로 끝맺게 됩니다. 이런 스타일에서는, 중첩이 깊어져 복잡해지면 코드를 다루기가 급격히 어려워질 수 있습니다. 
