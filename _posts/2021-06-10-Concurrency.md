...
코드에서 `await` 로 표시하여 멈춰달 수 있는 지점은 비동기 함수나 메소드가 반환하길 기다리는 동안 현재 코드 조각이 일시 정지할 지도 모른다고 지시합니다. 이를 _쓰레드 양도하기 (yielding the thread)_ 라고도 하는데, 그 이면을 살펴보면, 스위프트가 현재 쓰레드에 대한 코드 실행을 매달아 멈추고 그 대신 해당 쓰레드에서 다른 코드를 실행하기 때문입니다. `await` 를 가진 코드는 실행을 매달아 멈출 수 있어야 하기 때문에, 프로그램의 정해진 자리에서만 비동기 함수나 메소드를 호출할 수 있습니다:
...
* 두 경우 모두, 필요하다면, 비동기 함수가 반환할 때까지, 실행을 일시 정지할 것을 지시하기 위해 '멈춰달 수 있는 지점' 을 `await` 로 표시합니다.
