...
타입 메소드는, 인스턴스 메소드와 같이, '점 구문 표현 (dot syntax)' 으로 호출합니다. 하지만, 타입 메소드는, 해당 타입의 인스턴스가 아니라, 타입에서 호출합니다. 다음은 `SomeClass` 라는 클래스에서 '타입 메소드' 를 호출하는 방법입니다:
...
`LevelTracker` 구조체는, 아래에 보인 것처럼, 개별 참여자의 진행 상황을 추적하고 갱신하기 위해, `Player` 클래스와 함께 사용됩니다:
