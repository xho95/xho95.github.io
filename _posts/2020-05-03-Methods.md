...
더 일반적으로, 타입 메소드 본문 안에서 사용한 '어떤 규명 안된 메소드와 속성 이름'[^unqualified] 도 다른 타입-수준 메소드와 속성을 참조할 것입니다. 타입 메소드는, 타입 이름을 접두사로 붙일 필요없이, 메소드 이름으로 또 다른 타입 메소드를 호출할 수 있습니다. 이와 비슷하게, 구조체와 열거체의 타입 메소드는 타입 이름 접두사 없이 타입 속성 이름을 사용하여 타입 속성에 접근할 수 있습니다.
