...
위 예제의 `increment()` 메소드는 다음 처럼 작성할 수 있을 것입니다:
...
이 규칙에 대한 주요한 예외는 인스턴스 메소드에 대한 매개 변수 이름이 해당 인스턴스의 속성과 똑같은 이름을 가질 때마다 발생합니다. 이 상황에서는, 매개 변수 이름이 우선권을 가지며, 속성을 참조하려면 더 적당한 방법이 필요해 집니다. `self` 속성을 사용하여 매개 변수 이름과 속성 이름을 구별할 수 있습니다.
