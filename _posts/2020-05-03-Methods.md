...
인스턴스 메소드는 '속성' 처럼 '점 구문 표현 (dot syntax)' 으로 호출합니다:
...
인스턴스 메소드는, 위에서 설명한 것처럼, 특정 타입의 인스턴스에서 호출하는 메소드 입니다. 타입 자체에서 호출하는 메소드 역시 정의할 수 있습니다. 이러한 종류의 메소드를 _타입 메소드 (type methods)_ 라고 합니다. 타입 메소드는 메소드의 `func` 키워드 앞에 `static` 키워드를 작성함으로써 지시합니다. 클래스는, 하위 클래스가 해당 메소드의 상위 클래스 구현을 재정의하는 것을 허용하도록, `class` 키워드를 대신 사용할 수 있습니다.

...
`currentLevel` 속성의 관리를 돕고자, `LevelTracker` 는 `advance(to:)` 라는 인스턴스 메서드를 정의합니다. `currentLevel` 을 갱신하기 전에, 이 메소드는 요청한 새 레벨이 이미 잠금 해제되었는지 검사합니다. `advance(to:)` 메소드는 '불리언 (Boolean)' 값을 반환하여 실제로 `currentLevel` 로 설정할 수 있는 지를 나타냅니다. `advance(to:)` 메소드를 호출하는 코드가 그 반환 값을 무시하는 것을 실수라고 할 수는 없기 때문에, 이 함수를 `@discardableResult` 특성을 써서 표시했습니다. 이 특성에 대한 더 자세한 정보는, [Attributes (특성)]({% post_url 2020-08-14-Attributes %}) 을 참고하기 바랍니다.
