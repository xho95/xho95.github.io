...
위 예제에서 `someFunctionWithEscapingClosure` 함수의 호출은 에러인데 왜냐면 이것이 '변경 메소드 (mutating method)' 안에 있어서, `self` 가 변경 가능해버리게 되기 때문입니다. 이는 '벗어나는 클로저' 가 구조체의 `self` 를 '변경 가능한 참조' 로 붙잡을 수는 없다는 규칙을 위반합니다.
...
[^explicit]: 이 경우에 명시적으로 해야하는 이유는 함수도 참조 타입이라서 클로저와 함수가 서로를 참조하면 '순환 참조' 가 되기 때문입니다. 이 경우 `compleionHandler` 를 벗어 나게 만들려면 명시적으로 `@escaping` 을 붙여야 합니다.
