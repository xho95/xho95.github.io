...
클로저를 벗어날 수 있게 하는 한 가지 방법은 이를 함수 밖에서 정의한 변수에 저장하는 것입니다. 예를 들어 보면, '비동기 (asynchronous)' 연산을 시작시키는 함수들은 '완료 상황 처리자 (completion handler)' 로 클로저 인자를 취하는 경우가 많습니다. 함수는 연산을 시작시키고 나서 반환하지만, 연산이 완료될 때까지 클로저를 호출하진 않습니다-나중에 호출하려면, 클로저를 벗어나게 해야 합니다. 예를 들면 다음과 같습니다:
