...
클로저는, 함수에 인자로 전달된 클로저가, 함수가 반환한 후에 호출될 때, 함수를 _벗어난다 (escape)_ 라고 합니다.[^escape] 클로저를 매개 변수로 받는 함수를 선언할 때, 매개 변수의 타입 앞에 `@escaping` 을 작성하면 이 클로저는 벗어나도록 허용된 것임을 지시할 수 있습니다.

클로저가 벗어날 수 있는 방법 중 하나는 함수 외부에서 정의한 변수에 저장하는 것입니다. 예를 들어 보면, 비동기 연산을 시작하는 많은 함수들이 '완료 상황 처리자 (completion handler)' 로 클로저 인자를 받는다는 것입니다. 함수는 연산을 시작한 다음 반환하지만, 클로저는 연산이 완료되기 전까지 호출되지 않습니다-클로저는, 나중에 호출하려면, 벗어날 필요가 있습니다. 예를 들면 다음과 같습니다:
...
`someFunctionWithEscapingClosure(_:)` 함수는 클로저를 인자로 받아서 이를 함수 외부에서 선언한 배열에 추가합니다. 이 함수의 매개 변수를 `@escaping` 으로 표시하지 않으면, 컴파일-시간에 에러 (compile-time error) 를 갖게 될 것입니다.

'벗어나는 클로저' 가 `self` 를 참조하는데 이 `self` 가 클래스의 인스턴스를 참조하는 경우에는 특별한 주의가 필요합니다. '벗어나는 클로저' 에서 `self` 를 붙잡는 것은 의도하지 않게 '강한 참조 순환 (strong reference cycle)' 을 만들기 쉽습니다. 참조 순환에 대한 정보는, [Automatic Reference Counting (자동 참조 카운팅)]({% post_url 2020-06-30-Automatic-Reference-Counting %}) 을 참고하기 바랍니다.

보통은, 클로저 본문에서 변수를 사용하면 클로저가 이를 암시적으로 붙잡지만, 이 경우에는 명시적으로 할 필요가 있습니다.[^explicit] `self` 를 붙잡고 싶으면, 사용할 때 `self` 를 명시적으로 작성하거나, 아니면 클로저가 '붙잡을 목록 (capture list)' 에 `self` 를 포함시키도록 합니다. `self` 를 명시적으로 작성하는 것은 의도를 밖으로 표현하게 해주며, 참조 순환이 없다는 것을 확정해야 함을 상기시켜 줍니다. 예를 들어, 아래 코드에서, `someFunctionWithEscapingClosure(_:)` 에 전달된 클로저는 `self` 를 명시적으로 참조합니다. 이와는 대조적으로, `someFunctionWithNonescapingClosure(_:)` 에 전달된 클로저는 '벗어나지 않는 클로저 (nonescaping closure)' 로, 이는 이것이 `self` 를 암시적으로 참조할 수 있다는 것을 의미합니다.
...
이번 버전은 클로저가 '붙잡을 목록' 에 포함시키는 것으로써 `self` 를 붙잡는 `doSomething()` 이며, 그런 다음 `self` 를 암시적으로 참조합니다:
...
만약 `self` 가 구조체나 열거체의 인스턴스라면, `self` 를 항상 암시적으로 참조할 수 있습니다. 하지만, `self` 가 구조체나 열거체의 인스턴스일 때는 '벗어나는 클로저' 가 `self` 를 '변경 가능한 참조 (mutable reference)' 로 붙잡을 수는 없습니다. 구조체와 열거체는, [Structures and Enumerations Are Value Types (구조체와 열거체는 값 타입입니다)]({% post_url 2020-04-14-Structures-and-Classes %}#structures-and-enumerations-are-value-types-구조체와-열거체는-값-타입입니다) 에서 설명한 것처럼, '변경 가능성의 공유 (shared mutability)'[^shared-mutability] 를 허용하지 않습니다.
...
위 예제의 `someFunctionWithEscapingClosure` 함수에 대한 호출은 에러인데 왜냐면 이것이 '변경 메소드 (mutating method)' 안에 있으므로, `self` 가 변경 가능하기 때문입니다. 이는 '벗어나는 클로저' 가 구조체의 `self` 를 '변경 가능한 참조' 로 붙잡을 수 없다는 규칙을 위반합니다.
