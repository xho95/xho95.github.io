...
스위프트에서 값을 담을 수 있는 클로저 중에서 가장 간단한 것은 '품어진 함수 (nested function)' 인데, 이는 다른 함수 본문 안에 작성하는 방식입니다. 중첩 함수는 외부 함수의 인수를 캡처 할 수 있으며 외부 함수 내에 정의 된 상수 및 변수도 캡처 할 수 있습니다.

다음은 인크 리 멘터라는 중첩 함수가 포함 된 makeIncrementer라는 함수의 예입니다. 중첩 인크 리 멘탈 () 함수는 주변 컨텍스트에서 runningTotal과 amount의 두 값을 캡처합니다. 이 값을 캡처 한 후, makeIncrementer는이를 호출 할 때마다 runningTotal을 양만큼 증가시키는 클로저로 인크 리 멘터를 리턴합니다.
