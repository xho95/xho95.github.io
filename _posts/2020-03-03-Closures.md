...
위 예제에서 처럼 '끝자리 클로저 문법' 을 사용하면 클로저의 기능을 클로저가 지원하는 함수 뒤에 깔끔하게 '캡슐화 (encapsulates)' 할 수 있으며, 전체 클로저를 `map(_:)` 메소드의 바깥 괄호로 감쌀 필요도 없어집니다.
...
클로저를 클래스 인스턴스의 속성으로 할당하거나, 클로저로 클래스의 인스턴스나 멤버를 가리켜서 클로저가 그 인스턴스를 담게 (capture) 되면, 클로저와 그 인스턴스 사이에는 '강한 참조 순환 (strong reference cycles)' 이 생깁니다. 스위프트는 _담은 목록 (capture lists)_ 를 사용해서 이 강한 참조 순환을 끊어 냅니다. 이에 대한 자세한 내용은 [Strong Reference Cycles for Closures (클로저에 대한 강한 참조 순환)](https://docs.swift.org/swift-book/LanguageGuide/AutomaticReferenceCounting.html#ID56) 에서 확인할 수 있습니다.
