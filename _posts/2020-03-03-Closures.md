...
다음에 예로 든 함수 `makeIncrementer` 는 `incrementer` 라는 '중첩 함수' 를 갖고 있습니다. 그리고 이 중첩 함수인 `incrementer()` 는 자기 주위 영역에 있는 두 값인  `runningTotal` 과 `amount` 를 담습니다. 값을 담은 후, `incrementer` 는  `makeIncrementer` 가 반환하는 클로저이면서, 호출될 때마다 `runningTotal` 을 `amount` 만큼 증가시키는 역할을 합니다.
