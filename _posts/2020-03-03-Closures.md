...
이 예제는 `incrementalByTen` 이라는 상수에 '매 번 호출할 때마다 `runningTotal` 변수에 `10` 을 더하는 증가기 (incrementer) 함수에 대한 참조를 설정' 합니다.  함수를 여러 번 호출하면 이 동작을 실제로 보여줍니다:
...
두 번째 증가기를 생성하면, 이는 자신만 저장한 새로운, 별도의 `runningTotal` 변수에 대한 참조를 가질 것입니다:
