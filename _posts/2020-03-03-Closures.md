...
클로저가 벗어날 수 있게 하는 한 방법은 함수 밖에 정의된 변수에 저장하는 겁니다. 한 예로, 비동기 연산[^asynchronous-operation] 을 시작하는 수많은 함수들은 클로저 인자를 완료 처리자[^completion-handler] 로 입력 받습니다. 연산을 시작한 후 함수는 반환하지만, 연산을 완료하기 전까지 클로저를 호출하지 않습니다-나중에 호출하려면, 클로저가 벗어날 필요가 있습니다. 예를 들면 다음과 같습니다:
