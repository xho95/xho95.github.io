...
`self` 를 참조하는 '벗어나는 클로저' 에서 `self` 가 클래스의 인스턴스를 참조하고 있는 경우 특별한 주의가 필요합니다. 벗어나는 클로저에서 `self` 를 붙잡는 것은 강한 참조 순환을 우연히 만들기 쉽습니다. 참조 순환에 대한 정보는, [Automatic Reference Counting (자동 참조 카운팅)]({% post_url 2020-06-30-Automatic-Reference-Counting %}) 를 참고하기 바랍니다. 
