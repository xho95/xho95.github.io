...
보통은, 클로저가 변수를 암시적으로 붙잡기 때문에 클로저 본문에서 사용하면 되지만, 이 경우에는 명시적이어야 합니다.[^explicit] `self` 를 붙잡고 싶으면, 사용할 때 `self` 를 명시적으로 작성하거나, 아니면 `self` 를 클로저의 '붙잡을 목록 (capture list)' 에 포함시키면 됩니다. `self` 를 명시적으로 작성하는 것은 의도를 표현해주며, 참조 순환이 없다고 확정해야 함을 상기시켜 줍니다.
