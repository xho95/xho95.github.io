...
`makeIncrementer` 의 반환 타입은 `() -> Int` 입니다. 이는, 단순한 값 보단, 하나의 _함수 (function)_ 를 반환한다는 걸 의미합니다. 반환한 함수엔 매개 변수가 없으며, 매 번 호출할 때마다 `Int` 값을 반환합니다. 함수에서 다른 함수를 반환할 수 있는 방법을 배우려면, [Function Types as Return Types (반환 타입으로써의 함수 타입)]({% post_url 2020-06-02-Functions %}#function-types-as-return-types-반환-타입으로써의-함수-타입) 부분을 보기 바랍니다.
