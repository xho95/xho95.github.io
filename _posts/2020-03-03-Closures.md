...
다음 예제는, `incrementer` 라는 중첩 함수를 담고 있는, `makeIncrementer` 라는 함수입니다. 중첩 함수 `incrementer()` 는, 자기 주위의, `runningTotal` 과 `amount` 라는, 두 값을 붙잡습니다. 이 값을 붙잡은 후, `makeIncrementer` 를 호출할 때마다 `runningTotal` 을 `amount` 만큼 증가시키는 클로저인 `incrementer` 를 반환합니다.[^factory-method]
