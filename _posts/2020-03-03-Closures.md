...
위의 [Closure Expression Syntax (클로저 표현식 문법)](#closure-expression-syntax-클로저-표현식-구문-표현) 부분에서 다뤘던 '문자열-정렬 클로저' 를 끝자리 클로저로 고치면 다음과 같이 `sorted(by:)` 메소드 괄호 밖으로 뺄 수 있습니다:
