...
클로저 표현식 안에서 짧게 줄인 인자 이름을 사용하면, 정의에서 클로저 인자 목록을 생략할 수 있습니다. 짧게 줄인 인자 이름의 타입은 예상한 함수 타입으로 추론하며, 짧게 줄인 인자로 사용한 가장 큰 수는 클로저가 취한 인자의 개수를 결정합니다. `in` 키워드도 생략할 수 있는데, 클로저 표현식 전체가 자신의 본문만으로 이루어지기 때문입니다:
