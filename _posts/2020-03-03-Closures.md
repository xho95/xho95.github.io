...
그럼에도 불구하고, 원한다면 여전히 타입을 명시할 수도 있고, 코드가 헷갈리는걸 피해준다면 그러는 걸 권장합니다. `sorted(by:)` 메소드의 경우, '정렬이 일어난다' 는 사실에 의해서 클로저의 목적이 명확하며, 문자열 배열의 정렬을 거들고 있기 때문에, 클로저가 `String` 값과 작업할 것이라 가정해도 안전합니다.
