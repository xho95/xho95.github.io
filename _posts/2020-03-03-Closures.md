...
클로저를 인라인 클로저 표현식으로 함수나 메소드에 전달할 때는 매개 변수 타입과 반환 타입의 추론이 항상 가능합니다. 그 결과, 함수나 메소드 인자로 클로저를 사용할 때는 인라인 클로저를 '완전체 (fullest) 형식' 으로 작성할 필요가 절대 없습니다.
...
연산자 메소드에 대한 더 많은 내용은, [Operator Methods (연산자 메소드)]({% post_url 2020-05-11-Advanced-Operators %}#operator-methods-연산자-메소드) 부분을 참고하기 바랍니다.
...
각각의 배열 원소에 제공한 클로저를 적용한 후, `map(_:)` 메소드는 새로 맵핑한 모든 값을, 원본 배열에서의 관련 값과 똑같은 순서로, 담은 새로운 배열을 반환합니다.
...
`makeIncrementer` 의 반환 타입은 `() -> Int` 입니다. 이는, 단순한 값이라기 보단, 하나의 _함수 (function)_ 를 반환한다는 의미입니다. 반환하는 함수에는 매개 변수가 없으며, 매 번 호출할 때마다 `Int` 값을 반환합니다. 함수가 다른 함수를 반환할 수 있는 방법을 배우려면, [Function Types as Return Types (반환 타입으로써의 함수 타입)]({% post_url 2020-06-02-Functions %}#function-types-as-return-types-반환-타입으로써의-함수-타입) 을 참고하기 바랍니다.
...
클로저가 벗어날 수 있는 한 가지 방법은 함수 밖에서 정의한 변수에 저장하는 것입니다. 한 예로, '비동기 (asynchronous) 연산' 을 시작하는 수많은 함수들이 클로저 인자를 '완료 처리자 (completion handler)'[^completion-handler] 로 취합니다. 함수는 연산 시작 후에 반환하지만, 클로저는 연산을 완료할 때까지 호출되지 않습니다-나중에 호출하려면, 클로저가 벗어날 필요가 있습니다. 예를 들면 다음과 같습니다:
...
벗어나는 클로저가 `self` 를 참조할 때 `self` 가 클래스 인스턴스를 참조하는 경우엔 특별한 주의가 필요합니다. 벗어나는 클로저에서 `self` 를 붙잡는 건 '강한 참조 순환' 을 생성하는 사고를 만들기가 쉽습니다. 참조 순환에 대한 정보는, [Automatic Reference Counting (자동 참조 카운팅)]({% post_url 2020-06-30-Automatic-Reference-Counting %}) 장을 참고하기 바랍니다.
...
위 예제에서 `someFunctionWithEscapingClosure` 함수는 '변경 (mutating) 메소드' 안에 있어서, `self` 가 변경 가능하기 때문에, 호출이 에러입니다. 그건 '벗어나는 클로저는 구조체의 `self` 를 변경 가능한 참조로 붙잡을 수 없다' 는 규칙을 위반하는 것입니다.
...
자동 클로저는, 클로저를 호출할 때까지 그 안의 코드를 실행하지 않기 때문에, '값 평가 (evaluation)' 를 지연시켜 줍니다. 값 평가를 지연하는 것은, 해당 코드를 평가하는 시점을 제어하게 해주므로, 코드에 부작용 (side effect)[^side-effect] 이 있거나 계산 비용이 많이 들 때 유용합니다. 아래 코드는 클로저가 '값 평가' 를 지연하는 방법을 보여줍니다:
...
[^mapped-value]: '대체 맵핑 값 (alternative mapped value)' 에서의 '맵핑' 은 수학에서 말하는 '맵핑' 개념과 같습니다. '맵핑' 을 직역하면 '대응 관계' 정도로 옮길 수 있습니다. '맵핑' 에 대한 더 자세한 정보는, 위키피디아의 [Map (mathematics)](https://en.wikipedia.org/wiki/Map_(mathematics) 항목과 [맵핑 (수학)](https://ko.wikipedia.org/wiki/맵핑_(수학) 항목을 참고하기 바랍니다.

[^implicitly]: 구조체나 열거체 같은 값 타입은 '참조 순환' 이 일어나지 않기 때문에, 이들을 참조하는 클로저 안에서는 `self` 를 붙이지 않아도 됩니다. 이러한 내용은 '스위프트 5.3 버전' 에서 새롭게 추가되었으며, 이에 대한 더 자세한 정보는, 애플에서 공개한 [SE-0269: Increase availability of implicit self in @escaping closures when reference cycles are unlikely to occur](https://github.com/apple/swift-evolution/blob/master/proposals/0269-implicit-self-explicit-capture.md) 문서를 참고하기 바랍니다.
...
[^side-effect]: 컴퓨터 용어로 '부작용 (side effect)' 은, [Expressions (표현식)]({% post_url 2020-08-19-Expressions %}) 맨 앞에 있는 '부작용 (side effect)' 의 주석에서 설명한 것처럼, '부수적인 효과' 정도로 이해하는 것이 좋습니다.
