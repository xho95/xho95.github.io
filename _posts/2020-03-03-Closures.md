...
_클로저 표현식_ 은 '간결하고, 집중된 구문으로 인라인 클로저[^inline-closure] 를 작성하는 방법' 입니다. 클로저 표현식은 분명함과 의도를 잃지 않고도 클로저를 줄임 형식으로 작성하기 위한 여러가지 구문 최적화를 제공합니다. 아래의 클로저 표현식 예제는 `sort(by:)` 메소드라는 단일한 예제를 여러번 반복해서, 매 번 똑같은 기능을 좀 더 간결한 방식으로 표현하기 위한, 개량을 함으로써 이 최적화를 보여줍니다.
