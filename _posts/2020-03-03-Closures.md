...
클로저는 자신의 정의 주위에 있는 상수와 변수를 _붙잡을 (capture)_ 수 있습니다. 그러면 클로저가, 상수와 변수를 정의한 원본 영역이 더 이상 존재하지 않은 경우에도, 해당 상수와 변수를 참조하고 수정할 수 있습니다.[^capture-scope]
...
다음 예제는, `incrementer` 라는 중첩 함수를 담고 있는, `makeIncrementer` 라는 함수입니다. 중첩 함수 `incrementer()` 는, 자기 주위의, `runningTotal` 과 `amount` 라는, 두 값을 붙잡습니다. 이 값을 붙잡은 후, `makeIncrementer` 의 매 호출 때마다 `runningTotal` 을 `amount` 만큼 증가시키는 클로저인 `incrementer` 를 반환합니다.[^factory-method]
...
`makeIncrementer` 의 반환 타입은 `() -> Int` 입니다. 이는, 단순한 값이라기 보단, 하나의 _함수 (function)_ 를 반환한다는 의미입니다. 반환하는 함수에는 매개 변수가 없으며, 매 호출 때마다 `Int` 값을 반환합니다. 함수가 어떻게 다른 함수를 반환할 수 있는 지 배우려면, [Function Types as Return Types (반환 타입으로써의 함수 타입)]({% post_url 2020-06-02-Functions %}#function-types-as-return-types-반환-타입으로써의-함수-타입) 을 참고하기 바랍니다.

`makeIncrementer(forIncrement:)` 함수는, 반환할 '증가기 (incrementer)' 의 현재 총 누적 값을 저장하는, `runningTotal` 이라는 정수 변수를 정의합니다. 이 변수를 `0` 이라는 값으로 초기화합니다.
...
[^capture-scope]: 이렇게 원래 영역이 사라져도 사용할 수 있는 것을 가지고 '붙잡는다 (capture)' 라고 합니다. 클로저는 `class` 와 같은 참조 타입이라 '자유 저장소 (free store; 또는 heap)' 에 생성되는데 이 때 해당 상수와 변수를 복사하여 자신의 생애 주기 동안 참조하게 됩니다.
