...
#### Inferring Type From Context (상황으로부터 타입 추론하기)
...
클로저를 '인라인 클로저 표현식' 으로 함수나 메소드에 전달할 때 매개 변수 타입과 반환 타입은 항상 추론 가능합니다. 그 결과, 함수나 메소드 인자로 클로저를 사용할 때는 '인라인 클로저' 를 '완전한 형식 (fullest form)' 으로 작성할 일이 절대 없습니다.
