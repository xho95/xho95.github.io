...
> 가로챈다는 개념에 익숙하지 않아도 걱정할 필요가 없습니다. 뒤에 나오는 [Capturing Values (값 붙잡기)](#capturing-value) 에서 더 자세히 알아볼 겁니다.
...
* '품어진 함수 (nested functions)' 는 이름을 갖고 있으며 자신을 감싸고 있는 함수의 값을 붙잡을 (capture) 수 있는 클로저입니다.
...
[Nested Functions (품어진 함수)](https://docs.swift.org/swift-book/LanguageGuide/Functions.html#ID178) 부분에서 소개한 '품어진 함수'는, 큰 함수 내에서 '독립된 코드 블럭'을 정의하고 이름 붙이는 나름 편리한 방법입니다. 그러나 일일이 선언하고 이름 붙일 필요 없이 좀 더 간단하게 함수 같은 것을 만들 수 있다면 더 좋을 겁니다. 이는 특히 함수나 메소드의 인자로 함수들을 넘겨줘야할 때 더 그렇습니다.
...
하지만 솔직히 필요한 것은 한 문장으로 된 함수 (`a > b`) 일 뿐인데, 이 방법은 좀 지루하다고 할 수 있습니다. 이제부터 '클로저 표현식 구문 표현' 을 활용해서 삽입된 (inline)[^inline-closure] '정렬 클로저' 를 만드는 더 좋은 방법을 알아보겠습니다.
...
연산자 메소드에 대한 더 자세한 내용은 [Operator Methods (연산자 메소드)](https://docs.swift.org/swift-book/LanguageGuide/AdvancedOperators.html#ID42) 를 참조하십시오.
...
위의 [Closure Expression Syntax (클로저 표현식 문법)](#closure-expression-syntax) 부분에서 다뤘던 '문자열-정렬 클로저' 를 끝자리 클로저로 고치면 다음과 같이 `sorted(by:)` 메소드 괄호 밖으로 뺄 수 있습니다:
...
스위프트에서 값을 붙잡을 수 있는 클로저 중에서 가장 간단한 것은 '품어진 함수 (nested function)' 인데, 이는 다른 함수 본문 안에 작성하는 방식입니다. 품어진 함수는 자기 외부 함수의 인자는 어느 것이든 붙잡을 수 있으며 외부 함수 내에 정의되어 있으면 상수와 변수도 어느 것이든 붙잡을 수 있습니다.
...
`makeIncrementer` 의 반환 타입은 `() -> Int` 입니다. 이것은 단순히 값이 아니라 하나의 _함수 (function)_ 를 반환한다는 것을 의미합니다. 이 함수가 '반환하는 함수' 는 매개 변수를 갖지 않으며, 호출 될 때마다 `Int` 값을 반환합니다. 함수로 다른 함수를 반환하는 방법을 알고 싶으면 [Function Types as Return Types (반환 타입으로 사용되는 함수 타입)](https://docs.swift.org/swift-book/LanguageGuide/Functions.html#ID177) 을 보기 바랍니다.
...
클로저를 클래스 인스턴스의 속성으로 할당하거나, 클로저로 클래스의 인스턴스나 멤버를 가리켜서 클로저가 그 인스턴스를 담게 (capture) 되면, 클로저와 그 인스턴스 사이에는 '강한 참조 순환 (strong reference cycles)' 이 생깁니다. 스위프트는 _담은 목록 (capture lists)_ 를 사용해서 이 강한 참조 순환을 끊어 냅니다. 이에 대한 자세한 내용은 [Strong Reference Cycles for Closures (클로저에서의 강한 참조 순환)](https://docs.swift.org/swift-book/LanguageGuide/AutomaticReferenceCounting.html#ID56) 에 있습니다.
