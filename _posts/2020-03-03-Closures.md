...
함수나 클로저를 상수나 변수에 할당할 때마다, 실제로는 해당 상수나 변수가 함수나 클로저의 _참조 (reference)_ 가 되도록 설정하고 있는 것입니다. 위 예제에서, 클로저의 내용 자체가 아니라, `incrementByTen` 이 _참조하는 (refers to)_ 것이 상수라서, 클로저를 선택한 것입니다.[^refer-to-constant]
...
[^refer-to-constant]: 즉, 클로저가 A 를 참조할 때 B 를 참조하도록 바꿀 수는 없지만, A 안의 내용은 바꿀 수 있다는 의미입니다.
