...
`someFunctionWithEscapingClosure(_:)` 함수는 자신의 인자로 클로저를 취하고 함수 밖에서 선언한 배열에 이를 추가합니다. 이 함수의 매개 변수를 `@escaping` 이라고 표시하지 않으면, 컴파일-시간 에러를 가질 것입니다.

`self` 를 참조하는 벗어나는 클로저는 `self` 가 클래스 인스턴스를 참조할 경우에 특별한 주의가 필요합니다. 벗어나는 클로저에서 `self` 를 붙잡는 건 '강한 참조 순환' 을 생성하는 사고를 만들기가 쉽습니다. 참조 순환에 대한 정보는, [Automatic Reference Counting (자동 참조 카운팅)]({% post_url 2020-06-30-Automatic-Reference-Counting %}) 장을 참고하기 바랍니다.
