...
클로저는 자신이 정의된 곳 주위의 상수와 변수를 _붙잡을 (capture)_ 수 있습니다. 그러면, 상수와 변수를 정의한 원본 영역이 더 이상 존재하지 않은 경우에도, 클로저가 해당 상수와 변수를 참조하고 수정할 수 있습니다.[^capture-scope]
...
[^capture-scope]: 스위프트의 클로저는 기본적으론 상수와 변수를 참조로 붙잡지만, 최적화에 따라 복사하기도 합니다. 따라서, 원본 영역과는 상관없이 이를 참조하고 수정할 수 있습니다. 클로저는 클래스 처럼 참조 타입이어서 메모리 안의 자유 저장소 (free storage; 또는 heap) 영역에 생성됩니다. 
