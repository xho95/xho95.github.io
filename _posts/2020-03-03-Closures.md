...
클로저 표현식을 매 번 호출할 때마다 `output` 이라는 문자열을 제작합니다. 이는 나머지 연산자 (`number % 10`) 를 써서 `number` 의 마지막 숫자를 계산하고, 이 숫자로 `digitNames` 딕셔너리에서 적절한 문자열을 찾아 봅니다. 클로저는 0보다 큰 어떤 정수든 문자열 표현을 생성할 수 있습니다.
