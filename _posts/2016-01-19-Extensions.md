...
`Int` 값에 요청한 색인만큼의 숫자가 충분치 않다면, 마치 수치 값 왼쪽에 0 이 덧대진 것처럼, 첨자 구현이 `0` 을 반환합니다:

```swift
746381295[9]
// 마치 아래를 요청한 것과 같이, 0을 반환함:
0746381295[9]
```

### Nested Types (중첩 타입)

익스텐션은 기존의 클래스와, 구조체, 및 열거체에 새로운 중첩 타입을 추가할 수 있습니다:
...
이번 예제는 `Int` 에 새로운 중첩 열거체를 추가합니다. 이, `Kind` 라는, 열거체는 한 특별한 정수로 나타난 수의 종류를 표현합니다. 특히, 그 수가 음수인지, 0 인지, 양수인지를 표현합니다.

이 예제는 `Int` 에, `kind` 라는, 새로운 계산 인스턴스 속성도 추가하는데, 이는 그 정수에 대한 적절한 `Kind` 열거체를 반환합니다.

이제 중첩된 열거체를 어떤 `Int` 값에서도 쓸 수 있습니다:
...
이, `printIntegerKinds(_:)` 함수는, `Int` 값 배열을 입력 받아서 그 값들을 차례대로 반복합니다. 배열 안의 각 정수마다, 함수가 그 정수의 `kind` 계산 속성을 고려하여, 적절한 설명을 인쇄합니다.

> `number.kind` 는 이미 `Int.Kind` 타입인 걸 알고 있습니다. 이 때문에, 모든 `Int.Kind` case 값들은 `switch` 문 안에서 짧게 줄인 형식인, 즉 `Int.Kind.negative` 보단 `.negative` 같이, 쓸 수 있습니다.
