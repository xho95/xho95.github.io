...
이 계산 속성들 (computed properties) 을 사용하면 `Double` 값이 길이 단위를 가진 것처럼 표현할 수 있습니다. 비록 계산 속성으로 구현되었더라도, 속성의 이름을 '부동-소수점 문자표현 값 (floating-point literal value)'[^literal] 뒤에 점을 찍고 붙여서, 거리 변환을 수행하여 사용할 수 있습니다.   
