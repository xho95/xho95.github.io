...
* 새로운 '숨겨진 타입 (nested types)' 정의하고 사용하기
...
익스텐션을 써서 이미 존재하는 클래스, 구조체, 그리고 열거체에 '숨겨진 타입 (nested type)'[^nested-type]을 새로 추가할 수 있습니다:
...
이 예제는 `Int` 에 '숨겨진 열거체'를 새로 추가합니다. 이 열거체는 `Kind` 라고 하는데 정수가 나타내는 수의 종류를 표시합니다. 특히 이 수가 음수인지, 0 인지, 양수인지를 표시합니다.

이 예제는 또 `Int` 에 `kind` 라는 새로운 '계산 인스턴스 속성'을 추가해서, 이 정수에 맞는 적당한 `Kind` 열거체의 사례 값[^case]을 반환합니다.

이제 숨겨진 열거체는 모든 `Int` 값에서 사용할 수 있습니다:
...
[^nested-type]: 'nested types' 은 '중첩 타입' 으로 옮길 수 있는데, 실제로는 중첩되어 있다는 말이 한 타입 내에 정의되어 있다는 말로, 그 때문에 외부로는 타입이 드러나지 않는다는 느낌이 있습니다. 중첩 타입보다는 '숨겨진 타입'으로 옮기도록 합니다.
