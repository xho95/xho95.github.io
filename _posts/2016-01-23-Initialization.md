...
* 일단 사슬망 맨 위에 닿아서, 사슬망의 최후 클래스가 모든 저장 속성에 값이 있다는 걸 보장하고 나면, 인스턴스의 메모리 전체가 초기화된 걸로 고려하며, 단계 1 이 완성됩니다.

**Phase 2 (단계 2)**

* 사슬망 맨 위에서부터 밑으로 다시 내려가면서, 사슬망에 있는 각각의 지명 초기자가 인스턴스를 한 번 더 자신에게 맞게 고칠 기회를 가집니다. 초기자는 이제 `self` 에 접근할 수 있어서 자신의 속성을 수정하는 것과, 자신의 인스턴스 메소드를 호출하는 것, 등을 계속 할 수 있습니다.
* 최후엔, 사슬망의 어떤 편의 초기자도 인스턴스를 자신에게 맞게 고치고 `self` 작업을 할 옵션을 가지게 됩니다.

단계 1 에서 가상의 하위 클래스와 상위 클래스에 대한 초기화 호출은 이렇게 보입니다:
