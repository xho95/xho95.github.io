...
상위 클래스의 _지명 (designated)_ 초기자와 일치하는 하위 클래스 초기자를 작성할 때는, 사실상 '해당 지명 초기자의 재정의 (override)' 를 제공하고 있는 겁니다. 그러므로, 하위 클래스의 초기자 정의 앞에 반드시 `override` 수정자를 작성해야 합니다. 이는, [Default Initializers (기본 초기자)](#default-initializers-기본-초기자) 에서 설명한 것처럼, 자동으로 제공된 '기본 초기자' 를 재정의하고 있는 경우에도 그렇습니다.
