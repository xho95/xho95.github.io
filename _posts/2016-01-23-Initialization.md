...
초기화가 실패할 수도 있는 클래스, 구조체, 또는 열거체를 정의하는 것이 때로는 좋을 것입니다. 이러한 실패는 무효한 초기화 매개 변수 값에 의해서거나, 필수적인 외부 소스가 없어서 이거나, 아니면 어떤 다른 조건이 초기화가 성공하는 것을 막고 있기 때문에 발생할 수 있을 겁니다.
