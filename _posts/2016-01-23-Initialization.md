...
위의 네 가지 안전성 검사를 기반으로, 2-단계 초기화를 끝까지 마무리하면 이렇습니다:
...
가상의 하위 클래스와 상위 클래스에 대해서 1 단계의 초기화 호출을 보면 이렇습니다:
...
클래스나, 구조체, 또는 열거체에서 실패할 수 있는 초기자는 같은 클래스나, 구조체, 또는 열거체의 또 다른 실패할 수 있는 초기자로 일을 (옆으로) 맡길 수 있습니다. 이와 비슷하게, 하위 클래스의 실패할 수 있는 초기자는 상위 클래스의 실패할 수 있는 초기자로 일을 (위로) 맡길 수 있습니다.

어느 경우든, 또 다른 초기자로 일을 맡겨서 초기화를 실패하게 하면, 전체 초기화 과정이 곧바로 실패하며, 더 이상 초기화 코드를 실행하지 않습니다.

> 실패할 수 있는 초기자는 실패할 수 없는 초기자로도 일을 맡길 수 있습니다. 이런 접근법은 이미 있던 실패하지 않을 초기자에 실패할 수도 있는 상태를 추가해야할 때 사용합니다.

아래 예제는 `Product` 의 하위 클래스인 `CartItem` 을 정의합니다. `CartItem` 클래스는 온라인 장바구니 안의 항목을 모델링합니다. `CartItem` 은 상수 저장 속성인 `quantity` 를 도입하고 이 속성엔 항상 적어도 `1` 이라는 값이 있도록 보장합니다:
...
일반적으로 적절한 타입의 옵셔널 인스턴스를 생성하는 실패할 수 있는 초기자를 정의하려면 `init` 키워드 뒤에 물음표를 (`init?` 처럼) 붙이면 됩니다. 대안으로, 정의한 초기자가 적절한 타입의 암시적으로 풀리는 옵셔널 인스턴스를 생성하도록 할 수도 있습니다. 이렇게 하려면 `init` 키워드 뒤에 물음표 대신 느낌표를 (`init!` 처럼) 붙이면 됩니다.

`init?` 에서 `init!` 로 그리고 그 반대로도 일을 맡길 수 있으며, `init?` 를 `init!` 로 그리고 그 반대로도 재정의할 수 있습니다. `init` 에서 `init!` 로 일을 맡길 수도 있지만, 그렇게 하면 `init!` 초기자가 초기화를 실패하게 할 경우 단언문 (asssertion) 을 발생시킬 겁니다.
...
### Setting a Default Property Value with a Closure or Function (기본 속성 값을 클로저나 함수로 설정하기)

저장 속성의 기본 값을 자신에게 맞게 고치거나 설정하는게 필요할 경우, 클로저나 전역 함수를 쓰면 그 속성에 자신만의 기본 값을 제공할 수 있습니다. 속성이 속한 타입의 새 인스턴스를 초기화할 때마다, 클로저나 함수를 호출하며, 그 반환 값이 속성의 기본 값으로 할당됩니다.

이런 종류의 클로저나 함수는 일반적으로 속성과 똑같은 타입의 임시 값을 생성하고, 그 값이 원하는 초기 상태가 되도록 맞춘 다음, 그 임시 값을 반환하여 속성의 기본 값으로 씁니다.

클로저로 기본 속성 값을 제공하기 위한 뼈대는 이렇습니다:  

```swift
class SomeClass {
  let someProperty: SomeType = {
    // someProperty 의 기본 값을 이 클로저 안에서 생성함 
    // someValue 는 반드시 SomeType 과 똑같은 타입이어야 함
    return someValue
  }()
}
```

클로저의 닫는 중괄호 뒤에 빈 괄호 쌍이 있다는 걸 알아두기 바랍니다. 이는 스위프트에게 클로저를 곧바로 실행하라고 말하는 겁니다. 이 괄호를 생략하면, 클로저의 반환 값이 아니라, 클로저 그 자체를 속성에 할당하려고 하는 겁니다.

> 클로저로 속성을 초기화할 경우, 클로저가 실행된 시점에는 인스턴스의 나머지 부분이 아직 초기화된게 아니라는 걸 기억합니다. 이것의 의미는 클로저 안에서는 다른 어떤 속성 값에도 접근할 수 없다는 것으로, 심지어 그 속성에 기본 값이 있어도 그렇습니다. 암시적 `self` 속성을 쓰는 것도, 인스턴스의 어떤 메소드를 호출하는 것도 안됩니다.

아래 예제는 `Chessboard` 라는 구조체를 정의하여, 체스 게임판을 모델링합니다. 체스를 플레이하는 8x8 크기 판은, 검은색과 흰색 정사각형이 번갈아 있습니다.
