...
아래 그림은 네 개의 클래스로 된 좀 더 복잡한 클래스 계층 구조를 보여줍니다. 이는 계층 구조에 있는 지명 초기자가 어떻게 클래스 초기화에 대한 "깔때기 (funnel)" 와 같은, 연쇄망에 있는 클래스 간의 상관 관계를 단순화 하는, 그런 역할을 하게 되는 지를 묘사합니다.

![funnel points](/assets/Swift/Swift-Programming-Language/Initialization-funnel.png)

#### Two-Phase Initialization (2-단계 초기화)

스위프트에서 클래스 초기화는 '2-단계 (two-phase)' 과정을 거칩니다. 첫 번째 단계에서는, 클래스가 자신이 도입한 저장 속성에 초기 값을 할당합니다. 이렇게 한번 모든 저장 속성에 대한 초기 상태를 결정하고 나면, 두 번째 단계가 시작되는데, 이 때 각각의 클래스는 새 인스턴스의 사용 준비를 마치기 전에 자신의 저장 속성을 다시 사용자 목적에 맞게 바꿀 기회를 가집니다.

2-단계 초기화 과정을 사용하면 초기화를 안전하게 하면서도, 클래스 계층 구조에 있는 각 클래스에 여전히 완전한 유연함을 부여합니다. 2-단계 초기화는 속성 값을 초기화하기도 전에 그것에 접근하는 것을 막아 주며, 예상치 않게 다른 초기자가 해당 속성 값에 다른 값을 설정하는 것을 막아줍니다.

> 스위프트의 2-단계 초기화 과정은 오브젝티브-C 언어에 있는 초기화와 비슷합니다. 주요한 차이점은 1 단계 동안, 오브젝티브-C 언어에서는 모든 속성에 '0' 또는 'null' 값 (가령 `0` 이나 `nil` 등) 을 할당한다는 것입니다. 스위프트의 초기화 흐름은 사용자가 자신만의 초기 값을 설정할 수 있으며, `0` 이나 `nil` 이 유효한 기본 설정 값이 아닌 타입에 대처할 수 있다는 점에서 좀 더 유연합니다.

스위프트의 컴파일러는 도움이 될 만한 네 개의 안전-검사를 수행하여 2-단계 초기화가 에러 없이 완료될 수 있도록 확인해 줍니다:
...
위에 있는 네 개의 안전성 검사를 기반으로 하여, 2-단계 초기화를 끝까지 마치는 방법은 다음과 같습니다:
