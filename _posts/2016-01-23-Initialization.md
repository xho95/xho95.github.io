...
아래의 `Rect` 구조체는 세 방법 중 하나로 초기화할 수 있습니다-기본 설정인 '0' 으로 초기화된 `origin` 과 `size` 속성 값을 사용하거나, 원점과 크기 값에 지정된 값을 제공하거나, 아니면 중심점과 크기 값에 지정된 값을 제공하는 것입니다. 이러한 초기화 옵션은 `Rect` 구조체의 정의 부분에 있는 세 개의 사용자 정의 초기자로써 표현됩니다:
