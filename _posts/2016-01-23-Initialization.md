...
이러한 초기화 과정은, 특정 타입에 대한 인스턴스를 새로 생성하기 위해 호출할 수 있는 특수한 함수와도 같은, _초기자 (initializers)_ 를 정의하여 구현합니다. 오브젝티브-C 언어의 초기자와는 다르게, 스위프트의 초기자는 값을 반환하지 않습니다. 이들의 가장 중요한 역할은 새로운 타입의 인스턴스가 맨 처음 사용되기 전에 올바르게 초기화되도록 보장하는 것입니다.
...
스위프트에서 클래스 초기화는 '두-단계 (two-phase)' 과정을 거칩니다. 첫 번째 단계에서는, 클래스가 자신이 도입한 저장 속성에 초기 값을 할당합니다. 이렇게 한번 모든 저장 속성에 대한 초기 상태를 결정하고 나면, 두 번째 단계가 시작되는데, 이 때 각각의 클래스는 새 인스턴스의 사용 준비를 마치기 전에 자신의 저장 속성을 다시 사용자 목적에 맞게 바꿀 기회를 가집니다.

두-단계 초기화 과정을 사용하면 초기화를 안전하게 하면서도, 클래스 계층 구조에 있는 각 클래스에 여전히 완전한 유연함을 부여합니다. 두-단계 초기화는 속성 값을 초기화하기도 전에 그것에 접근하는 것을 막아 주며, 예상치 않게 다른 초기자가 해당 속성 값에 다른 값을 설정하는 것을 막아줍니다.

> 스위프트의 두-단계 초기화 과정은 오브젝티브-C 언어에 있는 초기화와 비슷합니다. 주요한 차이점은 1 단계 동안, 오브젝티브-C 언어에서는 모든 속성에 '0' 또는 'null' 값 (가령 `0` 이나 `nil` 등) 을 할당한다는 것입니다. 스위프트의 초기화 흐름은 사용자가 자신만의 초기 값을 설정할 수 있으며, `0` 이나 `nil` 이 유효한 기본 설정 값이 아닌 타입에 대처할 수 있다는 점에서 좀 더 유연합니다.

스위프트의 컴파일러는 도움이 될 만한 네 개의 안전-검사를 수행하여 두-단계 초기화가 에러 없이 완료될 수 있도록 확인해 줍니다:

**Safety Check 1 (안전성 검사 1)**

  지명 초기자는 위쪽 상위 클래스에 위임하기 전에 반드시 자신의 클래스가 도입한 모든 속성을 초기화했다는 것을 먼저 보장해야 합니다.

이전에 언급한 것처럼, 객체의 메모리는 일단 한번 모든 저장 속성의 초기 상태를 알고 나면 완전히 초기화된 것으로 간주합니다. 이 규칙을 만족하려면, 지명 초기자가 여기서 손을 털고 딴 데로 넘기기 전에 반드시 자신의 모든 속성이 초기화 되었음을 확인해 줘야 합니다.

**Safety Check 2 (안전성 검사 2)**

  지명 초기자는 상속받은 속성에 값을 할당하기 전에 반드시 위쪽 상위 클래스에 위임을 먼저해야 합니다. 그렇게 하지 않으면, 지명 초기자가 할당한 새 값이 상위 클래스의 자체 초기화로 인해 덮어 써지게 될 것입니다.

**Safety Check 3 (안전성 검사 3)**

  편의 초기자는 (해당 클래스에서 정의한 속성도 포함하여) _어떤 (any)_ 속성에든 값을 할당하기 전에 반드시 다른 초기자에 위임을 먼저해야 합니다. 그렇게 하지 않으면, 편의 초기자가 할당한 새 값이 자신의 지명 초기자에 의해 덮어 써지게 될 것입니다.

**Safety Check 4 (안전성 검사 4)**

  초기자는, 첫 단계의 초기화를 완료하기 전까지, 인스턴스 메소드를 호출하거나, 인스턴스 속성의 값을 읽거나, `self` 를 값처럼 참조할 수 없습니다.

클래스 인스턴스는 첫 번째 단계가 끝나기 전에는 완전히 유효한 상태인 것이 아닙니다. 일단 첫 번째 단계 끝에서 클래스 인스턴스가 유효하다고 알게 돼야만, 비로소 속성에 접근할 수 있고, 메소드를 호출할 수 있게 됩니다.

위에 있는 네 개의 안전성 검사를 기반으로 하여, 두-단계의 초기화를 끝까지 마치는 방법은 다음과 같습니다:

**Phase 1 (1 단계)**

* 지명 초기자 또는 편의 초기자가 클래스에서 호출됩니다.
* 해당 클래스의 새 인스턴스에 대한 메모리가 할당됩니다. 이 메모리는 아직 초기화된 것이 아닙니다.
* 해당 클래스에 대한 지명 초기자는 해당 클래스가 도입한 모든 저장 속성들이 값을 가지고 있음을 확인합니다. 이 저장 속성에 대한 메모리는 이제 초기화된 것입니다.
* 지명 초기자는 작업을 넘겨서 상위 클래스의 초기자가 자신의 저장 속성에 대해 같은 작업을 수행하도록 합니다.
* 이것을 클래스 상속 체인을 올라가서 체인의 최상단에 도달할 때까지 계속 합니다.
* 일단, 체인의 최상단에 도달했고, 체인에 있는 '최종 클래스 (final class)' 가 모든 저장 속성들이 값을 가지고 있다고 보장을 했으면, 인스턴스의 메모리는 완전히 초기화된 것으로, 1단계가 완료됩니다.

**Phase 2 (2 단계)**

* 체인의 최상단에서 다시 내려 가면서, 체인 내의 각각의 지명 초기자는 인스턴스를 추가로 사용자 목적에 맞게 변경할 수 있는 선택 기회를 가집니다. 초기자는 이제 `self` 에 접근해서 속성을 수정하거나, 인스턴스 메소드를 호출하는, 등등의 일을 할 수 있습니다.
* 마지막으로, 체인 내의 어떤 편의 초기자도 `self` 를 사용하여 인스턴스를 사용자 목적에 맞게 변경할 선택 기회를 가지게 됩니다.
