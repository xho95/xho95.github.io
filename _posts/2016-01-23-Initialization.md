...
지정된 이니셜 라이저는 안전 점검 1에 따라 모든 서브 클래스의 특성에 값이 있는지 확인합니다. 그런 다음 수퍼 클래스에서 지정된 이니셜 라이저를 호출하여 체인의 초기화를 계속합니다.

수퍼 클래스의 지정된 이니셜 라이저는 모든 수퍼 클래스 속성에 값이 있는지 확인합니다. 초기화 할 수퍼 클래스가 더 이상 없으므로 추가 위임이 필요하지 않습니다.

수퍼 클래스의 모든 속성이 초기 값을 갖 자마자 메모리는 완전히 초기화 된 것으로 간주되고 1 단계가 완료됩니다.
