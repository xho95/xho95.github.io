...
여기선, 상위 클래스가 하나의 지명 초기자와 두 개의 편의 초기자를 가집니다. 하나의 편의 초기자는 다른 편의 초기자를 호출하며, 차례대로 하나의 지명 초기자를 호출합니다. 이는 위의 규칙 2 와 규칙 3 을 만족합니다. 상위 클래스 그 자체는 더 이상 상위 클래스를 가지지 않으므로, 규칙 1 은 적용하지 않습니다.
