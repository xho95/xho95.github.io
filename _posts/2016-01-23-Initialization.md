...
스위프트의 클래스 초기화는 '2-단계 (two-phase) 과정' 입니다. 첫 번째 단계에서는, 각 저장 속성을 도입한 클래스가 초기 값을 할당합니다. 모든 저장 속성에 대한 초기 상태를 한 번 결정하고 나면, 두 번째 단계를 시작하며, 새 인스턴스가 사용할 준비가 됐다고 여겨지기 전에 각각의 클래스가 자신의 저장 속성을 한번 더 사용자 정의할 수 있는 기회가 주어집니다.

'2-단계 초기화 과정' 은, 여전히 클래스 계층 구조의 각 클래스에 완전한 유연성을 부여하면서도, 초기화를 안전하게 만듭니다. 2-단계 초기화는 초기화되기 전에 속성 값에 접근하는 것도 막아 주며, 예상치 않게 다른 초기자에 의해 속성 값이 다른 값으로 설정되는 것도 막아줍니다.
