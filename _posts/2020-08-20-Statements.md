...
'사용 가능성 조건' 은 '쉼표로-구분한 플랫폼 이름 및 버전 목록' 을 취합니다. 플랫폼 이름으로 `iOS`, `macOS`, `watchOS`, 및 `tvOS` 를 사용하며, 관련 '버전 번호' 를 포함합니다. `*` 인자는 필수이며, '사용 가능성 조건' 이 보호하는 '코드 블럭 본문' 이 '대상 (target)' 으로 지정한 '최소 배포 대상' 에서 실행되는, 다른 어떤 플랫폼이든 지정합니다.
