...
스위프트, 구문에는: '단순 (simple) 문', '컴파일러 제어 (compiler control) 문', 그리고 '제어 흐름 (control flow) 문' 이라는 세 종류가 있습니다. 단순문이 일상적으로 가장 흔하며 표현식이나 선언문으로 (이를) 구성합니다. '컴파일러 제어문' 은 프로그램이 컴파일러의 작동을 바꾸도록 허용하며 '조건부 컴파일 블럭 (conditional compilation block)' 과 '라인 제어문 (line control statement)' 을 포함합니다.
