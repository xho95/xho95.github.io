...
`switch` 문과 같이, 컴파일러는 `catch` 절을 다 써버리는지 추론하려 합니다. 그렇다고 결정할 수 있으면, 에러를 처리하는 걸로 고려합니다. 그 외 경우, (자신을) 담은 영역 밖으로 에러를 전파할 수 있는데, 이는 반드시 자신을 둘러싼 `catch` 절이 에러를 처리하거나 반드시 (자신을) 담은 함수를 `throws` 로 선언해야 한다는 걸 의미합니다.
