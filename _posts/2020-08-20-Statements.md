...
비록 '유형-맞춤 (pattern-matching)' 연산의 실제 실행 순서와, 'case 절' 유형의 평가 순서는 특별히, 지정되어 있지 않을지라도, `switch` 문의 '유형 맞춤 (pattern matching)' 은 마치 평가를 소스 순서대로-즉, 소스 코드에 나타나는 순서대로-수행하는 것 같이 동작합니다. 그 결과, 똑같은 값으로 평가되는, 따라서 제어 표현식 값과 일치하는, 유형을 담은 'case 절' 이 여러 개인 경우, 프로그램은 소스 순서에서 맨 처음 일치하는 'case 절' 안의 코드만을 실행합니다.
