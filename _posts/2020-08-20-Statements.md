...
`do` 구문은 새로운 영역을 도입하기 위해 사용하는 것으로, 정의한 에러 조건과의 일치 여부를 맞춰볼 '유형 (pattern)' 을 담고 있는, `catch` 절을 선택 사항으로 하나 이상 가질 수 있습니다. `do` 문의 영역 안에서 선언한 변수와 상수는 해당 영역 내에서만 접근할 수 있습니다.

스위프트의 `do` 문은 C 언어에서 코드 블럭의 경계를 구분하는데 사용하는 '중괄호 (curly; `{}`)' 와 비슷하며, 실행 시간에 성능 비용이 일어나지 않습니다.
...
`do` 코드 블럭에 있는 어떤 구문이든 에러를 던지면, 그 '유형 (pattern)' 이 에러와 일치하는 첫 번째 `catch` 절로 프로그램 제어를 전달합니다. 아무 절도 일치하지 않으면, 에러를 주변 영역으로 전파합니다. 에러가 최상위 수준에서도 처리되지 않으면, 실행 시간 에러와 함께 프로그램 실행을 중지합니다.

`switch` 문과 같이, 컴파일러는 `catch` 절이 '빠짐없이 철저한 (exhaustive)' 지를 추론하려고 합니다. 그렇다고 결정할 수 있으면, 에러는 처리된 것으로 간주합니다. 다른 경우라면, 에러는 담고 있는 영역 밖으로 전파할 수 있으며, 이는 에러가 이를 둘러싼 `catch` 절에서 반드시 처리되거나 아니면 이를 담고 있는 함수가 반드시 `throws` 로 선언되어야 함을 의미합니다.

'다중 유형 (multiple patterns)' 을 가진 `catch` 절은 '유형 (patterns)' 중 어떤 것이라도 에러와 일치하면 에러와 일치합니다. `catch` 절이 '다중 유형 (multipe patterns)' 을 담고 있는 경우, 모든 '유형' 은 반드시 같은 '상수 연결 (bindings)' 이나 '변수 연결 (bindings)' 을 담고 있어야 하며, 연결된 각각의 변수 또는 상수는 반드시 `catch` 절의 모든 '유형 (patterns)' 에서 똑같은 타입을 가져야 합니다.

에러가 처리된다는 것을 보장하려면, 모든 에러와 일치하는 '유형 (patterns)' 을 가진 `catch` 절을 사용하면 되는데, 가령 '와일드카드 패턴 (wildcard pattern; `_`)' 같은 것이 있습니다. `catch` 절이 패턴을 지정하지 않은 경우, 이 `catch` 절은 어떤 에러와도 일치하며 이를 `error` 라는 이름의 지역 상수에 연결합니다. `catch` 절에서 사용할 수 있는 패턴에 대한 더 많은 정보는, [Patterns (패턴; 유형)]({% post_url 2020-08-25-Patterns %}) 을 참고하기 바랍니다.

여러 `catch` 절을 가진 `do` 문을 사용하는 방법에 대한 예제를 보려면, [Handling Errors (에러 처리하기)]({% post_url 2020-05-16-Error-Handling %}#handling-errors-에러-처리하기) 를 참고하기 바랍니다.
