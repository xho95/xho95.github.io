...
음수 글자 값은, `-42` 처럼, 정수 글자 값 앞에 빼기 기호 (`-`) 를 둬서 표현합니다.

가독성을 위해 숫자 사이에 밑줄 (`_`) 도 허용하지만, (값은) 무시하므로 글자 값에는 영향이 없습니다. 정수 글자 값은 `0` 을 선두로 시작할 수 있지만, 이 (값) 또한 무시하여 글자 값의 밑수나 값엔 영향이 없습니다.

달리 지정하지 않는 한, 정수 글자 값의 기본 추론 타입은 스위프트 표준 라이브러리 타입 `Int` 입니다. 스위프트 표준 라이브러리는, [Integers (정수)]({% post_url 2016-04-24-The-Basics %}#integers-정수) 에서 설명한 것처럼, 다양한 크기의 부호 있는 정수와 부호 없는 정수도 정의합니다.
...
16진 부동-소수점 글자 값은 `0x` 접두사와, 그 뒤에 옵션으로 16진 분수 부분 및, 그 뒤의 16진 지수 부분으로 구성됩니다. 16진 분수 부분은 소수점 및 그 뒤에 있는 일련의 16진 숫자로 구성됩니다. '지수' 는 대문자 또는 소문자 `p` 접두사와 `p` 앞의 값을 '2' 의 몇 제곱해야 하는지 지시하는 '일련의 10진 숫자' 들로 구성됩니다. 예를 들어, `0xFp2` 는, `60` 이라고 평가하는, '15 x 2<sup>2</sup>' 를 표현합니다. 이와 비슷하게, `0xFp-2` 는, `3.75` 라고 평가하는, '15 x 2<sup>-2</sup>' 를 표현합니다.
