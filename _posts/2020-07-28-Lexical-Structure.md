...
정규 표현식 안에서의, 역 빗금은 그 정규 표현식의 일부이지, 문자열 글자 값 안에서 같이 벗어난 문자로 이해하지 않습니다. 이는 그 뒤의 특수 문자를 글자 그대로 해석하거나, 그 뒤의 특수하지 않은 문자를 특수한 방식으로 해석하라고 지시합니다. 예를 들어, `/\(/` 는 단일한 왼쪽 괄호와 맞고 `/\d/` 는 단일한 숫자와 맞습니다.
...
[^carriage-return]: 캐리지 반환 문자는 `/r` 을 의미합니다.

[^line-feed]: 줄 먹임 문자는 `/n` 을 의미합니다.
