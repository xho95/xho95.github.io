...
'여러 줄짜리 문자열 글자 값' 에 있는 '줄 끊음 (line breaks)' 은 '줄 바꿈 (line feed) 문자' 를 사용하도록 '정규화 (normalized)' 합니다.[^line-break-to-line-feed] 소스 파일이 '캐리지 반환' 과 '줄 먹임' 을 혼합하여 가지더라도, 문자열에 있는 모든 '줄 끊음' 은 다 똑같을 것입니다.
