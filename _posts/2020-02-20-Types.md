...
(어떤 타입 `T` 에 대한) 함수 타입 `() -> T` 의 매개 변수에 `autoclosure` 특성을 적용하면 자신을 호출한 쪽에서 암시적으로 클로저를 생성할 수 있습니다. 이는 함수를 호출할 때 클로저를 명시하지 않아도 표현식의 평가를 미루는 구문상의 편의를 제공합니다. 자동 클로저 함수 타입 매개 변수의 예제는, [Autoclosures (자동 클로저)]({% link docs/swift-books/swift-programming-language/closures.md %}#autoclosures-자동-클로저) 부분을 보도록 합니다.
