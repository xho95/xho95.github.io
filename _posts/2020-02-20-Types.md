...
_메타타입 타입 (metatype type)_ 은, 클래스 타입, 구조체 타입, 열거체 타입, 그리고 프로토콜 타입을 포함하여, 어떤 타입에 대한 타입이든 참조합니다.

클래스, 구조체, 또는 열거체 타입의 '메타타입 (metatype)' 은 해당 타입의 이름 뒤에 `.Type` 을 붙인 것입니다. 프로토콜 타입의 메타타입-실행 시간에 이 프로토콜을 준수하는 구체적으로 고정된 타입이 아닌 것-은 해당 프로토콜의 이름 뒤에 `.Protocol` 을 붙인 것입니다. 예를 들어, `SomeClass` 라는 클래스 타입의 메타타입은 `SomeClass.Type` 이고 `SomeProtocol` 이라는 프로토콜의 메타타입은 `SomeProtocol.Protocol` 입니다.
...
해당 타입의 메타타입 값으로 타입의 인스턴스를 생성하려면 '초기자 표현식 (initializer expression)' 을 사용합니다. 클래스 인스턴스인 경우라면, 반드시 호출되는 초기자를 `required` 키워드로 표시하거나 아니면 전체 클래스를 `final` 키워드로 표시해야 합니다.
