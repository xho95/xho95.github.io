...
클래스 타입은 상위 클래스는 단 하나만 상속하고 프로토콜은 어떤 개수든 준수할 수 있습니다. 클래스를 정의할 때, 상위 클래스의 이름은 반드시 '타입 식별자 목록' 의 맨 처음에 와야 하며, 클래스가 준수해야 하는 프로토콜은 어떤 개수든 뒤따를 수 있습니다. 클래스가 다른 클래스를 상속하지 않으면, 목록이 프로토콜로 대신 시작할 수 있습니다. 클래스 상속에 대한 확장된 논의와 여러 예제들은, [Inheritance (상속)]({% post_url 2020-03-31-Inheritance %}) 장을 참고하기 바랍니다.
