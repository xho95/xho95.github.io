...
스위프트 언어는 스위프트 표준 라이브러리의 `Array<Element>` 타입에 대해 다음 같이 수월한 구문을 제공합니다:

&nbsp;&nbsp;&nbsp;&nbsp;[`type-타입`]

다른 말로 하면, 다음의 두 선언은 서로 같은 겁니다:
...
두 경우 모두, `someArray` 상수를 문자열 배열이라고 선언합니다. 배열 원소는 대괄호 안에 유효 색인 값을 지정하는 첨자를 통하여 접근할 수 있는데: `someArray[0]` 는 0번 색인의 원소인, `"Alex"` 를, 참조합니다.
