...
클래스나, 구조체, 또는 열거체 타입의 메타타입은 그 타입 이름 뒤에 `.Type` 이 있는 겁니다. 프로토콜 타입-실행 시간에 프로토콜을 준수하는 고정된 타입이 아닌 타입-의 '메타타입' 은 해당 프로토콜의 이름 뒤에 `.Protocol` 이 붙은 것입니다. 예를 들어, `SomeClass` 라는 클래스 타입의 '메타타입' 은 `SomeClass.Type` 이며 `SomeProtocol` 이라는 프로토콜의 '메타타입' 은 `SomeProtocol.Protocol` 입니다.
