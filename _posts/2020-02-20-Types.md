...
스위프트 언어는 접미사 `?` 를, 스위프트 표준 라이브러리에서 정의한, '이름 있는 타입' 인 `Optional<Wrapped>` 에 대한 '수월한 구문 표현' 으로 정의합니다. 다르게 말해서, 다음의 두 선언은 '동치 (equivalent)' 입니다:
...
두 경우 모두, `optionalInteger` 라는 변수가 '옵셔널 정수' 라는 타입을 가진다고 선언합니다. 타입과 `?` 사이에 공백은 전혀 없다는 것에 주목하기 바랍니다.

`Optional<Wrapped>` 타입은, 있을 수도 없을 수도 있는 값을 나타내기 위해 사용하는, `none` 과 `some(Wrapped)` 라는, 두 개의 'case 값' 이 있는 열거체입니다. 어떤 타입이든지 옵셔널 타입이라고 명시적으로 선언 (또는 암시적으로 변환) 할 수 있습니다. 옵셔널 변수나 속성을 선언할 때 초기 값을 제공하지 않으면, 그 값은 기본으로 `nil` 로 자동 설정됩니다.
...
'옵셔널 연쇄 (optional chaining)' 및 '옵셔널 연결 (optional binding)' 을 사용하면 옵셔널 표현식에서 조건부로 연산을 수행할 수도 있습니다. 값이 `nil`이면, 아무런 연산을 수행하지 않으므로 아무런 실행 시간 에러도 일으키지 않습니다.
