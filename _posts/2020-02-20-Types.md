...
다른 '이름 붙은 타입'[^other-named-type] 은 '프로토콜의 목록' 만 상속하거나 준수할 수 있습니다. 프로토콜 타입은 어떤 개수의 다른 프로토콜도 상속할 수 있습니다. 프로토콜 타입이 다른 프로토콜을 상속할 때는, 다른 프로토콜에 있던 '필수 조건 집합' 을 한군데로 모으며, 현재 프로토콜을 상속하는 어떤 타입이든 반드시 해당 필수 조건 모두를 준수해야 합니다.

열거체 정의에 있는 '타입 상속 절' 은 '프로토콜의 목록' 이거나, 자신의 'case 값' 에 원시 값을 할당하는 열거체인 경우, 해당 원시 값의 타입을 지정하는 '단일한, 이름 붙은 타입' 일 수 있습니다. 원시 값의 타입을 지정하기 위해 '타입 상속 절' 을 사용하는 '열거체 정의' 에 대한 예제는, [Raw Values (원시 값)]({% post_url 2020-06-13-Enumerations %}#raw-values-원시 값) 부분을 참고하기 바랍니다.
...
[^other-named-type]: 스위프트에서 '이름 붙은 타입' 은, 맨 처음에 설명한 것처럼, 클래스, 구조체, 열거체, 및 프로토콜을 말합니다. 즉, 여기서 말한 '다른 이름 있는 타입 (other named types)' 은 방금 전에 설명한 클래스를 제외한, '구조체, 열거체, 및 프로토콜' 을 말하는 것입니다.
