...
함수 타입의 _매개 변수 타입 (parameter type)_ 엔 가변 매개 변수가 있을 수 있습니다. 구문상으로, 가변 매개 변수는, `Int...` 처럼, 기초 타입 이름'과 그 바로 뒤의 세 점 (`...`) 으로 구성됩니다. 가변 매개 변수는 기초 타입 이름의 원소를 담은 배열이라고 취급합니다. 구체적인 사례로, 가변 매개 변수 `Int...` 는 `[Int]` 라고 취급합니다. 가변 매개 변수를 사용한 예제는, [Variadic Parameters (가변 매개 변수)]({% post_url 2020-06-02-Functions %}#variadic-parameters-가변-매개-변수) 부분을 참고하기 바랍니다.
