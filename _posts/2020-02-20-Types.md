...
[^Self-and-type-of]: 이 설명만 보면, `type(of:)` 함수를 호출하면 되므로, `Self` 가 필요 없을 것 같습니다. 하지만 `type(of:)` 함수는 컴파일 시간에 호출하지 않습니다. `f()` 함수의 반환 값은 컴파일 시간에 정해야 하는데, 이 때 `type(of:)` 함수를 호출할 순 없습니다. 즉, `Self` 는 컴파일 시간에 현재 타입의 동적 타입을 지정하기 위해 사용하는 겁니다. `type(of:)` 함수에 대한 더 자세한 내용은 Apple 개발자 문서의 [type(of:)](https://developer.apple.com/documentation/swift/2885064-type) 항목을 참고하기 바랍니다.
