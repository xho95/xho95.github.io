...
`Any` 타입은 다른 어떤 타입으로 된 값도 담을 수 있습니다. `Any` 는 다음 타입 중 어떤 것의 인스턴스에 대해서라도 구체적으로 '고정된 타입 (concrete type)' 인 것처럼 사용할 수 있습니다.

* 클래스, 구조체, 또는 열거체
* `Int.sef` 같은, '메타 타입 (metatype)'
* 어떤 타입의 성분이든 이를 가진 튜플
* 클로저 타입 또는 함수 타입
...
`Any` 를 어떤 인스턴스에 대한 '고정된 타입 (concrete type)' 인 것처럼 사용할 때는, 먼저 그 인스턴스를 알려진 타입으로 변환해야만 속성 또는 메소드에 접근할 수 있습니다. '고정된 타입' 이 `Any` 인 인스턴스는 원래의 동적 타입을 유지하고 있으며 해당 타입을-`as`, `as?`, 또는 `as!` 같은, '타입-변환 연산자' 를 사용하여 변환할 수 있습니다. 예를 들어, '혼성 배열 (heterogeneous array)' 에 있는 첫 번째 객체를 `String` 으로 '조건부 내림 변환 (conditionally downcast)' 하려면 아래처럼 `as?` 를 사용합니다:
...
`AnyObject` 프로토콜은 `Any` 타입과 비슷합니다. 모든 클래스는 `AnyObject` 를 암시적으로 준수합니다. `Any` 는, 언어에서 정의하고 있는 것과, 달리, `AnyObject` 는 스위프트 표준 라이브러리에서 정의하고 있습니다. 더 많은 정보는, [Class-Only Protocols (클래스-전용 프로토콜)]({% post_url 2016-03-03-Protocols %}#class-only-protocols-클래스-전용-프로토콜) 과 [AnyObject](https://developer.apple.com/documentation/swift/anyobject) 를 참고하기 바랍니다.
