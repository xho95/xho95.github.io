...
스위프트에는: '이름 붙인 (named) 타입' 과 '복합 (compound) 타입' 이라는 두 가지 종류의 타입이 있습니다. _이름 붙인 타입 (named type)_ 은 정의할 때 특정 이름을 부여할 수 있는 타입입니다. 이름 붙인 타입은 클래스, 구조체, 열거체, 그리고 프로토콜을 포함합니다. 예를 들어, `MyClass` 라는 이름을 붙인 사용자-정의 클래스 인스턴스는 `MyClass` 라는 타입을 가집니다. 사용자가-정의한 '이름 붙인 타입' 에 더하여, 스위프트 표준 라이브러리는, 배열, 딕셔너리, 및 옵셔널 값을 표현하는 것을 포함한, 흔히 사용하는 수많은 '이름 붙인 타입' 을 정의합니다.
