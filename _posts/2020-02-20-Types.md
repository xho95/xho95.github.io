...
자신의 반환 타입으로 불투명 타입을 사용한 함수는 반드시 단일한 실제 타입을 공유하는 값을 반환해야 합니다.[^share-a-single] 반환 타입은 함수의 일반화 타입 매개 변수 부분인 타입을 포함할 수 있습니다. 예를 들어, `someFunction<T>()` 라는 함수는 `T` 나 `Dictionary<String, T>` 타입의 값을 반환할 수 있습니다.
