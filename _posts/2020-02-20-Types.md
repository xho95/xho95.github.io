...
_타입 식별자 (type identifier)_ 는 이름 붙인 타입 또는 이름 붙거나 복합인 타입의 타입 별명을 참조합니다.

대부분의 시간 동안, 타입 식별자는 식별자와 똑같은 이름인 이름 붙인 타입을 직접 참조합니다. 예를 들어, `Int` 는 `Int` 라는 '이름 붙인 타입' 을 직접 참조하는 '타입 식별자' 이며, `Dictionary <String, Int>` 라는 타입 식별자는 `Dictionary <String, Int>` 라는 '이름 붙인 타입' 을 직접 참조합니다.
