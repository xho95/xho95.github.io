...
`Any` 타입은 다른 모든 타입에서의 값을 가질 수 있습니다. `Any` 는 다음의 타입들 중 어떤 것의 인스턴스에 대해서라도 '구체적으로 고정된 타입 (concrete type)' 으로 사용할 수 있습니다.
...
`Any` 를 인스턴스에 대한 '고정된 타입 (concrete type)' 으로 사용할 때는, 먼저 이 인스턴스를 알려진 타입으로 변환해야만 속성 또는 메소드에 접근할 수 있게 됩니다. `Any` 라는 '고정된 타입' 을 가진 인스턴스는 원래의 동적 타입을 유지하고 있어서 해당 타입을-`as`, `as?`, 또는 `as!` 같은, 타입-변환 연산자를 사용하여 변환할 수 있습니다. 예를 들어, 다음처럼 '혼성 배열 (heterogeneous array)' 에 있는 첫 번째 객체를 `String` 으로 '조건부 내림 변환 (conditionally downcast)' 하려면 `as?`를 사용합니다:
...
`AnyObject` 프로토콜은 `Any` 타입과 비슷합니다. 모든 클래스들은 암시적으로 `AnyObject` 를 준수합니다. 언어에서 정의하는, `Any` 와 달리, `AnyObject` 는 스위프트 표준 라이브러리에서 정의합니다. 더 많은 정보는, [Class-Only Protocols (클래스-전용 프로토콜)]({% post_url 2016-03-03-Protocols %}#class-only-protocols-클래스-전용-프로토콜) 및 [AnyObject](https://developer.apple.com/documentation/swift/anyobject) 를 참고하기 바랍니다.
