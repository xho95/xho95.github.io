...
스위프트 언어는, 스위프트 표준 라이브러리에서 정의한, `Optional<Wrapped>` 라는 '이름 붙인 타입' 에 대해, 접근할 때 자동으로 포장을 푸는 추가적인 동작을 가진, 수월한 구문 표현으로 `!` 라는 접미사를 정의합니다. `nil` 값을 가진 '암시적으로 포장이 풀리는 옵셔널' 을 사용하려고 하면, 실행 시간 에러를 가지게 됩니다. 암시적으로 포장을 푸는 동작을 제외하면, 다음의 두 선언은 '동치 (equivalent)' 입니다:
