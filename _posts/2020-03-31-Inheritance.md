...
상속하게 될 성질을 재정의하려면, 재정의할 정의의 접두사로 `override` 키워드를 붙입니다. 그럼으로써 '재정의' 의 제공이 의도한 것이고 일치한 정의가 실수로 제공안된 것은 아닌지 분명하게 밝힙니다. 실수로 재정의하는 것은 예상하지 않은 작동 방식을 유발할 수 있으며, 코드를 컴파일할 때 `override` 키워드가 없는 재정의는 어떤 것이든 에러로 진단합니다.
