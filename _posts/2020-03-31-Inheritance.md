...
상속 속성은 소스 구현이 저장인지 계산 속성인지에 상관없이, _어떤 (any)_ 상속 속성에든 사용자 정의 획득자 (및 적절하다면, 설정자) 를 제공하여 재정의할 수 있습니다. 상속한 속성의 태생이 저장인지 계산인지는 하위 클래스가 알 수 없습니다-이는 상속한 속성이 가진 정해진 이름과 타입만을 압니다. 재정의할 속성의 이름과 타입은, 똑같은 이름과 타입을 가진 상위 클래스 속성과 재정의가 일치하는지 컴파일러가 검사할 수 있도록, 반드시 둘 다 정식으로 알려야 합니다.
