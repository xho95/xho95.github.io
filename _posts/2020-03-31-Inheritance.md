...
다음 예제는 `Car` 라는 새로운 클래스를 정의하는데, 이는 `Vehicle` 의 하위 클래스입니다. `Car` 클래스가 소개하는 `gear` 라는 새로운 저장 속성은, 기본 값이 `1` 입니다. `Car` 클래스는 `Vehicle` 로부터 물려받은 `description` 속성도 재정의하여, 기어의 현 상태를 포함한 자신만의 설명을 제공합니다:
...
속성 재정의를 써서 물려받은 속성에 속성 관찰자를 추가할 수 있습니다. 이는 물려받은 속성의 값을 바꿀 때 알림을 받게 하는데, 이 때 그 속성이 원래 어떻게 구현됐는 지는 상관없습니다. 속성 관찰자에 대한 더 많은 정보는, [Property Observers (속성 관찰자)]({% link docs/swift-books/swift-programming-language/properties.md %}#property-observers-속성-관찰자) 를 보기 바랍니다.

> 속성 관찰자를 상수로 물려받은 저장 속성이나 읽기-전용으로 물려받은 계산 속성에 추가할 수는 없습니다. 이 속성들엔 값을 설정할 수 없어서, 재정의 부분에서 `willSet` 이나 `didSet` 을 구현하는게 적절하지 않습니다.
>
> 재정의한 설정자와 재정의한 속성 관찰자 둘 다를 똑같은 속성에 제공할 수 없다는 것도 알아두기 바랍니다. 속성의 값이 바뀌는 걸 관찰하고 싶은데, 그 속성에 이미 자신만의 설정자를 제공했다면, 어떤 값의 바꿈도 단순히 자신의 설정자에서 관찰할 수 있습니다.

다음 예제는 `AutomaticCar` 라는 새로운 클래스를 정의하는데, 이는 `Car` 의 하위 클래스입니다. `AutomaticCar` 클래스는 자동 변속기가 있는 자동차를 나타내며, 현재 속도를 기반으로 자동으로 사용하기 적절한 기어를 선택합니다:
...
`AutomaticCar` 인스턴스의 `currentSpeed` 속성을 설정할 때마다, 속성의 `didSet` 관찰자가 인스턴스의 `gear` 속성을 새로운 속도에 적절하게 선택된 기어로 설정합니다. 구체적으로 이 경우에, 속성 관찰자가 선택한 건 새로운 `currentSpeed` 값을 `10` 으로 나누고, 가장 가까운 정수로 내림한 후, `1` 을 더한 기어입니다. 속도가 `35.0` 이면 기어는 `4` 를 만들어 냅니다:
...
메소드나, 속성, 또는 첨자가 재정의되는 걸 막으려면 이를 _최종 (final)_ 으로 표시하면 됩니다. 이렇게 하려면 `final` 수정자를 메소드나, 속성, 또는 첨자 도입자 키워드 앞에 (`final var` 와, `final func`, `final class func`, 및 `final subscript` 같이) 쓰면 됩니다.

최종인 메소드나, 속성, 또는 첨자를 하위 클래스에서 재정의하려는 어떤 시도든 컴파일-시간 에러로 보고합니다. 메소드나, 속성, 또는 첨자를 익스텐션 안에서 클래스로 추가한 건 익스텐션의 정의 안에서 최종이라고 표시할 수도 있습니다.

전체 클래스를 최종으로 표시하려면 클래스 정의에 있는 `class` 키워드 앞에 `final` 수정자를 (`final class` 같이) 쓰면 됩니다. 최종 클래스로 하위 클래스를 만들려는 어떤 시도든 컴파일-시간 에러라고 보고합니다.
