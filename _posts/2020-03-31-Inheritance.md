...
메소드나, 속성, 또는 첨자가 재정의되는 걸 막으려면 이를 _마지막 (final)_ 이라고 표시하면 됩니다. 이렇게 하려면 `final` 수정자를 메소드나, 속성, 또는 첨자 도입자 키워드 앞에 (`final var` 와, `final func`, `final class func`, 및 `final subscript` 같이) 쓰면 됩니다.

마지막인 메소드나, 속성, 또는 첨자를 하위 클래스에서 어떻게든 재정의하려고 하면 컴파일-시간 에러를 보고합니다. 클래스에 메소드나, 속성, 또는 첨자를 추가한게 익스텐션이면 익스텐션 정의 안에서 마지막을 표시할 수도 있습니다.

클래스 전체를 마지막으로 표시하려면 `final` 수정자를 클래스 정의의 `class` 키워드 앞에 쓰면 됩니다 (`final class`). 마지막 클래스로 어떻게든 하위 클래스를 만들려고 하면 컴파일-시간 에러를 보고합니다.
