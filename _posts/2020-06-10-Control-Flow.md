...
### Deferred Actions (미뤄둔 행동)

`if` 와 `while` 같은 제어-흐름 생성문이, 코드 일부를 실행할지 또는 몇 번을 실행할지를 제어하게 해주는 것과 달리, `defer` 문은 코드 조각이 _언제 (when)_ 실행될지를 제어합니다. `defer` 블럭으로 코드를 작성하면 나중에, 프로그랩이 현재 시야의 끝에 도달할 때 실행됩니다. 예를 들면 다음과 같습니다:
...
위 예제에서, `defer` 블럭 안의 코드는 `if` 문의 본문을 나가기 전에 실행됩니다. 첫 번째로, `if` 문의 코드를 실행하여, `score` 를 5만큼 증가시킵니다. 그런 다음, `if` 문의 시야를 벗어나기 전에, 미뤄둔 코드를 실행하여, `score` 를 인쇄합니다. 

`defer` 안의 코드는 항상 실행되며, 프로그램이 어떻게 그 시야를 벗어나든 상관하지 않습니다. 거기엔 때 이르게 함수를 탈출하거나, `for` 반복문 끊고 나오기, 또는 에러 던지기와 같은 코드가 포함됩니다. `defer` 의 이런 동작은 행동이 쌍으로 발생하는 걸 보증해야할 연산인-수동으로 메모리 할당하고 풀어주기와, 저-수준의 파일 설명자 열고 닫기 및, 데이터베이스의 트랜잭션 시작하고 끝내기와 같은 것-에 유용한데, 이는 두 행동을 코드 안에 서로 나란히 작성할 수 있기 때문입니다. 예를 들어, 다음 코드는, 코드 뭉치에서 100을 더하고 빼서, 점수에 일시적인 보너스를 줍니다:
...
if score < 100 {
  ...
  // 보너스가 있는 score 를 쓰는 코드는 여기에 둠.
  print(score)
}
// "103" 을 인쇄함
```

똑같은 시야 범위 안에 하나 이상의 `defer` 블럭을 쓰면, 첫 번째로 지정한 걸 마지막에 실행합니다.
...
프로그램이-예를 들어, 런타임 에러나 충돌 때문에-실행을 멈추면, 미뤄둔 코드를 실행하지 않습니다. 하지만, 에러를 던진 후엔 미뤄둔 코드가 실행되며; `defer` 와 에러 처리를 같이 사용하는 것에 대한 정보는, [Specifying Cleanup Actions (치울 행동 지정하기)]({% link docs/swift-books/swift-programming-language/error-handling.md %}#specifying-cleanup-actions-치울-행동-지정하기) 부분을 보기 바랍니다. 
