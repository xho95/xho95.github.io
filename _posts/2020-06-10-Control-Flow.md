...
게임판은 `Int` 값 배열을 가지고 나타냅니다. 크기는 `finalSquare` 라는 상수를 기초로 하며, 배열 초기화 및 예제 나중의 승리 조건 검사에서도 사용합니다. 참가자는, 게임판 밖인, "0 번 정사각형" 에서 시작하기 때문에, 게임판을, `25` 가 아닌, `26` 개의 `Int` 값 0으로 초기화합니다.
...
그런 다음 뱀과 사다리에 맞게 일부 정사각형을 더 특정한 값으로 설정합니다. 사다리 받침을 가진 정사각형은 게임 판을 올라가도록 양수를 가지는 반면, 뱀 머리를 가진 정사각형은 게임판을 다시 내려가도록 음수를 가집니다.
...
3번 정사각형은 11번 정사각형으로 올라가게 만드는 사다리 밑바닥을 담고 있습니다. 이를 나타내도록, `board[03]` 은, (`3` 과 `11` 의 차이인) 정수 값 `8` 과 같은 `+08` 입니다. 값과 구문이 나란하도록, 단항 양수 연산자 (`+i`) 를 명시적으로 단항 음수 연산자 (`-i`) 와 같이 사용하며 `10` 보다 작은 수는 0으로 덧댑니다. (엄밀하게 말해서 어떤 꾸밈 기술도 필요하지는 않지만, 코드를 더 깔끔하게 합니다.)
...
> 이 검사를 하지 않으면, `board[square]` 가 `board` 배열 경계 밖의 값에 접근할 지도 모르는데, 이러면 실행 시간 에러가 발동할 것입니다.

그러면 현재 `while` 반복문 실행이 끝나고, 반복을 다시 할 지 확인하고자 반복문 조건을 검사합니다. 참가자가 `25`번 정사각형 위 또는 너머로 이동했으면, 반복문 조건을 `false` 로 평가하며 게임을 끝냅니다.

이 경우엔 `while` 반복문이 적절한데, `while` 반복문을 시작할 때 게임이 얼마나 길어질 지 명확하지 않기 때문입니다. 그 대신, 특별한 조건을 만족할 때까지 반복문을 실행합니다.
...
정해진 조건을 기초로 서로 다른 코드 조각을 실행하는 것이 유용할 때가 종종 있습니다. 에러가 일어날 때 부가적인 코드 조각을 실행하고 싶거나, 값이 너무 크거나 작아질 때 메시지를 보여주고 싶어질 지도 모릅니다. 이를 하려면, 코드 일부분을 _조건문 (conditional)_ 으로 만듭니다.

스위프트는 코드에 조건 분기를 추가하기 위한: `if` 문과 `switch` 문이라는 두 가지 방법을 제공합니다. `if` 문은, 전형적으로, '적은 결과만 발생 가능한 단순한 조건 평가' 에 사용합니다. `switch` 문은 '여러 순서로 발생 가능한 더 복잡한 조건' 에 적합하며 '패턴 맞춤 (pattern matching) 이 실행할 적절한 코드 분기 선택에 도움이 되는 상황' 에 유용합니다.
...
모든 `switch` 문은 반드시 _빠짐없이 철저 (exhaustive)_[^exhaustive] 해야 합니다. 즉, 가능한 모든 타입의 고려 값은 반드시 '`switch` 문 case 절 하나와 일치' 해야 합니다. '가능한 모든 값에 case 절을 제공' 하는 게 적절치 않다면, 명시하지 않은 어떤 값이든 다루기 위한 '기본적인 (default) case 절을 정의' 할 수 있습니다. 이 기본적인 case 절은 `default` 키워드로 지시하며, 항상 반드시 마지막에 나타나야 합니다.
