...
> 이 검사를 하지 않으면, `board[square]` 가 `board` 배열 경계 밖의 값에 접근할 지도 모르는데, 이러면 실행 시간 에러를 발동할 것입니다.

그러면 현재 `while` 반복문 실행이 끝나고, 반복문을 다시 실행해야 하는지 확인하기 위해 반복문의 조건을 검사합니다. 참가자가 `25` 번 정사각형 위나 그 너머로 이동했으면, 반복문의 조건은 `false` 로 평가되며 게임이 끝납니다.
