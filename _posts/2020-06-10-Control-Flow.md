...
스위프트는 코드에 조건 분기를 추가하기 위한: `if` 문과 `switch` 문이라는 두 가지 방법을 제공합니다. `if` 문은, 전형적으로, '적은 결과만 발생 가능한 단순한 조건 평가' 에 사용합니다. `switch` 문은 '여러 순서로 발생 가능한 더 복잡한 조건' 에 적합하며 '패턴 맞춤 (pattern matching) 이 실행할 적절한 코드 분기 선택에 도움이 되는 상황' 에서 유용합니다.
