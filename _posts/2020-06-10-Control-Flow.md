...
3번 정사각형은 11번 정사각형으로 올라가게 만드는 사다리 밑바닥을 담고 있습니다. 이를 나타내도록, `board[03]` 은, (`3` 과 `11` 의 차이인) 정수 값 `8` 과 같은 `+08` 입니다. 값과 구문이 나란하도록, 단항 양수 연산자 (`+i`) 를 명시적으로 단항 음수 연산자 (`-i`) 와 같이 사용하며 `10` 보다 작은 수는 0으로 덧댑니다. (꾸밈 기술은 엄밀하게 말해서 딱히 필요하진 않지만, 코드를 더 깔끔하게 만듭니다.)
