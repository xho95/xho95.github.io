...
게임판은 `Int` 값 배열로 나타냅니다. 크기는 `finalSquare` 라는 상수에 기반하며, 이를 써서 배열도 초기화하고 예제 나중에 승리 조건도 검사합니다. 참가자는, 게임판 밖인, "0 번 정사각형" 에서 시작하기 때문에, 게임판을, `25` 가 아닌, `26` 개의 `Int` 값 0으로 초기화합니다.
