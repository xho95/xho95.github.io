...
게임판은 `Int` 값 배열을 가지고 나타냅니다. 크기는 `finalSquare` 라는 상수를 기초로 하며, 배열 초기화 및 예제 나중의 승리 조건 검사에서도 사용합니다. 참가자는, 게임판 밖인, "0 번 정사각형" 에서 시작하기 때문에, 게임판을, `25` 가 아닌, `26` 개의 0 이라는 `Int` 값으로 초기화합니다.
...
그런 다음 일부 정사각형은 뱀과 사다리에 맞는 더 특정한 값을 갖도록 설정합니다. 사다리 받침을 가진 정사각형은 게임 판을 올라가기 위해 양수를 가지는 반면, 뱀 머리를 가진 정사각형은 게임판을 다시 내려가기 위해 음수를 가집니다.
