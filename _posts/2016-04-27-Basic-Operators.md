...
한-쪽 범위는, '첨자 연산' 뿐 아니라, 다른 상황에서도 사용할 수 있습니다. 첫 번째 값을 생략한 '한-쪽 범위' 에는 동작을 반복할 수 없는데, 반복 동작을 어디서 시작해야할 지 명확하지 않기 때문입니다. 최종 값을 생략한 '한-쪽 범위' 에는 동작을 반복 _할 수 (can)_ 있습니다; 하지만, 범위가 무한정 계속되기 때문에, 명시적인 종료 조건을 반복문에 확실히 추가해야 합니다. '한-쪽 범위' 는, 아래 코드에서 보인 것처럼, 특정 값을 담고 있는 지도 검사할 수 있습니다.
