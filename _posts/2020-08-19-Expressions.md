...
`await` 표현식은, `async(priority:operation:)` 함수에 전달된 '끝자리 클로저' 와 같이, '비동기인 상황' 에서만 있을 수 있습니다. `defer` 문의 본문, 또는 '동기 함수 타입' 의 '자동 클로저' 안에 있을 수는 없습니다.
...
'함수 호출 표현식' 은 닫는 괄호 바로 뒤에 '클로저 표현식' 형식의 '끝자리 클로저 (trailing closures)' 를 포함할 수 있습니다. 끝자리 클로저는, 마지막 괄호 친 인자 뒤에 추가한, 함수의 인자로 이해합니다. '첫 번째 클로저 표현식' 은 이름표를 붙이지 않으며; 어떤 '추가적인 클로저 표현식' 이든 앞에 '인자 이름표 (argument labels)' 를 붙입니다. 아래 예제는 '끝자리 클로저 구문을 사용한 것과 사용하지 않은 것이 서로 같은 버전의 함수 호출' 을 보여줍니다:
