...
중위 연산자의 왼-쪽 표현식에 `try` 나, `try?`, 또는 `try!` 를 표시할 땐, 중위 표현식 전체에 그 연산자를 적용합니다. 그렇더라도, 괄호를 써서 연산자의 적용 범위을 명시할 수 있습니다.

```swift
sum = try someThrowingFunction() + anotherThrowingFunction()   // 두 함수 호출 모두 try 를 적용함
sum = try (someThrowingFunction() + anotherThrowingFunction()) // 두 함수 호출 모두 try 를 적용함
sum = (try someThrowingFunction()) + anotherThrowingFunction() // 에러: 첫 번째 함수 호출에만 try 를 적용함
```

중위 연산자가 할당 연산자거나 `try` 표현식을 괄호로 테두리친게 아닌 한, 중위 연산자 오른-쪽에 `try` 표현식이 나타날 순 없습니다.
