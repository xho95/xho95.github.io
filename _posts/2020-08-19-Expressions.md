...
클로저 표현식은, 함수 호출에서 클로저를 곧바로 사용할 때 처럼, 변수나 상수에 저장하지 않고도 사용할 수 있습니다. 위 코드에서 `myFunction` 에 전달한 클로저 표현식은 이 '곧바로 사용' 하는 한 예입니다. 그 결과, 클로저 표현식이 '벗어나는 (escaping)' 지 '벗어나지 않는 (nonescaping)' 지는 표현식의 주변 상황에 달려 있습니다. 곧바로 호출하거나 '벗어나지 않는 함수 인자' 로 전달한 경우의 클로저 표현식은 '벗어나지 않는' 것입니다. 그 외의 경우, 클로저 표현식은 '벗어나는' 것입니다.
