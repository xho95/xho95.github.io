...
기본적으로, 클로저 표현식은 주위 영역의 상수와 변수를 그 값에 대한 '강한 참조 (strong references)' 를 가지고 붙잡습니다. _붙잡을 목록 (capture list)_ 을 사용하면 클로저에서 값을 붙잡을 방법을 명시적으로 제어할 수 있습니다.
...
붙잡을 목록에 있는 '엔트리 (entries)' 들은 클로저를 생성할 때 초기화 됩니다. 붙잡을 목록에 있는 각각의 엔트리에 대해서, 주위 영역에 있는 같은 이름을 가진 상수 또는 변수의 값에 대한 하나의 상수를 초기화 합니다. 예를 들어 아래 코드에서, `a` 는 붙잡을 목록에 포함되어 있지만 `b` 는 아니므로, 서로 다른 동작 방식을 부여합니다.
