...
클로저와 클래스 인스턴스 사이의 '강한 참조 순환' 은 클로저의 정의에서 _붙잡는 목록 (capture list)_ 을 정의함으로써 해결합니다. '붙잡는 목록' 은 클로저의 본문에서 하나 이상의 '참조 타입' 을 붙잡을 때 사용할 규칙을 정의합니다. 두 클래스 인스턴스 사이의 '강한 참조 순환' 에서 처럼, 각각의 붙잡을 참조를 '강한 참조' 보다는 '약한 참조' 나 '소유하지 않는 참조' 로 선언합니다. '약한' 또는 '소유하지 않는 참조' 를 적절히 선택하는 것은 다른 코드들 사이의 관계에 달려 있습니다.
