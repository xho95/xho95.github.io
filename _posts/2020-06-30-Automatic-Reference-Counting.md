...
클로저가 매개 변수 목록과 반환 타입을 제공하는 경우 '붙잡을 목록' 은 그 앞에 위치합니다:
...
클로저가 매개 변수 목록이나 반환 타입을 상황으로 추론할 수 있기 때문에 지정하지 않는 경우, '붙잡을 목록' 은 클로저 맨 앞에 위치하며, `in` 키워드가 뒤에 옵니다:[^capture-list-place]
...
[^capture-list-place]: 사실 본문에 있는 두 가지 경우 모두 '붙잡을 목록 (capture list)' 이 클로저 본문 가장 앞에 있습니다. 그러니까 클로저 본문을 '여는 중괄호' 바로 다음에 '붙잡을 목록' 을 작성한다고 생각하면 됩니다. 
