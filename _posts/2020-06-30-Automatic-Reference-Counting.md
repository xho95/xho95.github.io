...
약한 참조는 자신이 참조할 인스턴스를 강하게 들지 않기 때문에, 약한 참조가 여전히 참조하는 동안에도 그 인스턴스가 해제될 가능성이 있습니다. 그러므로, **ARC** 는 참조한 인스턴스가 해제될 때 자동으로 약한 참조를 `nil` 로 설정합니다. 그리고, 약한 참조는 그 값이 실행 시간에 `nil` 로 바뀌는 걸 허용할 필요가 있기 때문에, 항상 옵셔널 타입의, 상수 보단, 변수로 선언합니다.
