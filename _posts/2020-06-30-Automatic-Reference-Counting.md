...
위 [Strong Reference Cycles for Closures (클로저의 강한 참조 순환)](#strong-reference-cycles-for-closures-클로저의-강한-참조-순환) 의 `HTMLElement` 예제에 있는 '강한 참조 순환' 을 해결하는 데 사용하기에는 '소유하지 않는 참조' 가 적절한 방법입니다. 다음은 순환을 피하도록 `HTMLElement` 클래스를 작성하는 방법입니다:
