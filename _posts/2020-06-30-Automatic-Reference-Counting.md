...
이를 가능하게 하려고, 속성이나, 상수, 또는 변수에 클래스 인스턴스를 할당할 때마다, 그 속성이나, 상수, 또는 변수는 인스턴스에 대한 _강한 참조 (strong reference)_ 를 만듭니다. 참조를 "강한 (strong)" 참조라고 하는 것은 해당 인스턴스를 꽉 쥐고 있어서, 이 '강한 참조' 가 남아 있는 한 해제를 허용하지 않기 때문입니다.
...
약한 참조에 있는 값의 존재는, 그냥 다른 어떤 옵셔널 값인 것처럼, 검사할 수 있으며, 절대로 더 이상 존재하지 않는 무효한 인스턴스를 참조하도록 끝맺을 수 없습니다.
