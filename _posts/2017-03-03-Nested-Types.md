...
열거체는 종종 특정 클래스나 구조체의 기능을 지원하고자 생성합니다. 이와 비슷하게, 순전히 더 복잡한 타입 안에서만 사용할 보조 클래스 및 구조체를 정의한다면 편리할 수 있습니다. 이를 해내기 위해, 스위프트는 _중첩 타입 (nested types)_ 을 정의하여, 자신이 지원할 타입 정의 안에 지원용 열거체와, 클래스, 및 구조체를 중첩할 수 있게 합니다.
