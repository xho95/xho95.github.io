...
아파치를 실행하려면 관리자 권한이 필요하므로 명령의 맨 앞에 `sudo` 를 붙여줍니다. 위와 같이하면 비밀번호를 입력하라는 표시가 나오는데 관리자 비밀 번호를 입력하면 됩니다.
...
'기본 제공 index 페이지' 는 **/Library/WebServer/Documents** 디렉토리에 존재하며, 해당 폴더에 가보면 **index.html.en** 파일이 있음을 확인할 수 있습니다.[^macOS-unix]

아파치에서는 `DocumentRoot` 로 지정된 디렉토리에서 웹페이지를 불러오는데, 기본적으로는 이 값이 **/Library/WebServer/Documents** 로 지정되어 있습니다.
