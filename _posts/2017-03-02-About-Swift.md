...
* 변수는 사용 전에 항상 미리 초기화합니다.
* 배열의 색인이 범위를 벗어난 오류가 없는지 검사합니다.
* 정수는 값이 넘치지 않는지 검사합니다.
* '옵셔널 (Optional)' 을 써서 `nil` 값을 명시적으로 처리하도록 합니다.
* 메모리는 자동으로 관리합니다.
* 에러 처리를 통해서 예기치 않은 실패 상황도 제어하고 복구할 수 있습니다.

Swift는 수년 동안 제작 해 왔으며 새로운 기능으로 계속 발전하고 있습니다. 스위프트에 대한 우리의 목표는 야심입니다. 우리는 당신이 그것으로 만든 것을 기다릴 수 없습니다.
...
스위프트는 강력한 타입 추론 및 패턴 탐색 기능에 현대적이고 가벼운 구문 표현을 결합했기에, 복잡한 아이디어도 분명하고 간결하게 표현할 수 있습니다. 그 결과로, 코드를 작성하는 것만 쉬운게 아니라 이해하고 유지 보수하는 것또한 더 편해졌습니다.
