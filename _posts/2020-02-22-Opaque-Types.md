...
불투명 타입은 일반화 타입의 역방향인 것으로 생각할 수 있습니다. 일반화 타입은 함수를 호출하는 코드가 그 함수의 매개 변수와 반환 값 타입을 함수 구현에서 뽑아내는 (abstracted)[^abstract-away] 방식으로 고르게 합니다. 예를 들어, 다음 코드의 함수가 반환할 타입은 자신을 호출한 쪽에 의존합니다:
...
불투명 반환 타입을 가진 함수에선 이 역할들이 역방향입니다. 불투명 타입은 함수 구현부가 반환 값의 타입을 함수를 호출하는 코드에서 뽑아내는 방식으로 고르게 합니다. 예를 들어, 다음 예제에 있는 함수는 해당 도형의 실제 타입을 노출하지 않으면서 '사다리꼴 (trapezoid)' 을 반환합니다.
...
[^abstract-away]: 'abstract' 은 '추상화하다, 발췌하다' 라는 의미를 가지고 있는데, 이는 "원래의 것에서 (핵심을) 뽑아내다" 라는 의미입니다.
