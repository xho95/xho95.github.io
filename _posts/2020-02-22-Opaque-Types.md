...
`max(_:_:)` 를 호출하는 코드가 `x` 와 `y` 의 값을 선택하며, 이 값들의 타입이 `T` 의 고정 타입을 결정합니다. 호출하는 코드는 `Comparable` 프로토콜을 따르는 타입은 어떤 것이든 쓸 수 있습니다. 함수 안에 있는 코드는 일반화된 방식으로 작성되어서 호출하는 쪽에서 어떤 타입을 제공하든 처리할 수 있습니다. `max(_:_:)` 구현부는 모든 `Comparable` 타입에서 공유하는 기능만 씁니다.
