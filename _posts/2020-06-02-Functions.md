...
아래 예제에 있는 함수는 `greet(person:)` 이라고 하는데, 하는 일이 그거기 때문입니다-사람 이름을 입력받아 그 사람에 대한 인사말을 반환합니다.[^what-it-does] 이걸 해내려고, 입력 매개 변수인-`person` 이라는 `String` 값-하나와, 그 사람의 인사말을 담을, `String` 이라는 반환 타입을 정의합니다.
