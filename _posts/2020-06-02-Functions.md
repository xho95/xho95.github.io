...
모든 함수엔 _함수 이름 (function name)_ 이 있는데, 이는 함수가 수행할 임무를 설명합니다. 함수를 사용하려면, 그 함수를 이름으로 "호출 (call)" 하며 함수의 매개 변수 타입과 맞는 (_인자 (arguments)_ 라는) 입력 값을 전달합니다. 함수 인자의 순서는 반드시 항상 함수의 매개 변수 목록과 똑같아야 합니다.

아래 예제의 함수는 `greet(person:)` 이라고 하는데, 하는게 그거기 때문입니다-사람 이름을 입력받아 그 사람의 인사말을 반환합니다.[^what-it-does] 이걸 해내고자, 입력 매개 변수인-`person` 이라는 `String` 값-하나와, 그 사람의 인사말을 담을, `String` 이라는 반환 타입을 정의합니다.
