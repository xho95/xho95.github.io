...
함수를 정의할 땐, 옵션으로, _매개 변수 (parameters)_ 라는, 함수가 입력으로 취하는 이름과 타입이 있는 값을 정의할 수 있습니다. 옵션으로 값의 타입을 정의하여 함수를 마칠 때 출력으로 되돌려줄 수도 있는데, 이를 _반환 타입 (return type)_ 이라고 합니다.
