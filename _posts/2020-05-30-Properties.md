...
_느긋한 저장 속성 (lazy stored property)_ 은 최초로 사용하기 전까진 초기 값을 계산하지 않는 속성입니다. 자신의 선언 앞에 `lazy` 수정자 (modifier) 를 작성하여 느긋한 저장 속성을 지시합니다.

> 느긋한 속성은 반드시 (`var` 키워드를 가진) 변수로 항상 선언해야 하는데, 인스턴스 초기화를 완료하기 전까진 자신의 초기 값을 못가져올 지도 모르기 때문입니다. 상수 속성은 반드시 초기화를 완료하기 _전에 (before)_ 항상 값을 가져야 하므로, 느긋하다고 (lazy) 선언할 수 없습니다.
