...
포장된 값에 더하여, 속성 포장에서 추가 기능을 드러내려면 _내보인 값 (projected value)_ 을 정의하면 됩니다-예를 들어, 데이터베이스로의 접근을 관리하는 속성 포장이라면 `flushDatabaseConnection()` 메소드를 자신이 내보일 값으로 드러낼 수 있습니다. 내보인 값의 이름은, 달러 기호 (`$`) 로 시작한다는 것만 빼면, 포장된 값과 똑같습니다. 코드에선 `$` 로 시작하는 속성을 정의할 수 없기 때문에 내보인 값과 내가 정의한 속성이 간섭할 일은 절대 없습니다.

위의 `SmallNumber` 예제에서, 속성에 너무 큰 수를 설정하려 하면, 저장하기 전에 속성 포장이 그 수를 적당하게 조절합니다. 아래 코드는 `SmallNumber` 구조체에 `projectedValue` 속성을 추가하여 속성에 새 값을 저장하기 전에 속성 포장이 그 값을 조절했는지 추적합니다.
...
`someStructure.$someNumber` 라고 쓰면 포장이 내보인 값에 접근합니다. 4 같은 작은 수를 저장한 후엔, `someStructure.$someNumber` 값이 `false` 입니다. 하지만, 55 같이, 너무 큰 수를 저장하려고 한 후엔 내보인 값이 `true` 입니다.

속성 포장은 어떤 타입의 값이든 자신이 내보인 값으로 반환할 수 있습니다. 이 예제에서, 속성 포장은 단 한 조각의 정보인-수를 적당하게 조절했는지-만을 드러내므로 자신이 내보일 값으로 불리언 값을 드러냅니다. 포장에서 더 많은 정보를 드러내야 한다면 다른 어떤 데이터 타입의 인스턴스를 반환할 수도, `self` 를 반환하여 자신이 내보일 값으로 포장의 인스턴스를 드러낼 수도 있습니다.

타입의 일부인, 속성의 획득자나 인스턴스의 메소드 같은, 코드에서 내보인 값에 접근할 땐, 그냥 다른 속성에 접근하는 것 같이, 속성 이름 앞의 `self.` 를 생략할 수 있습니다. 다음 예제 코드는 `height` 와 `width` 의 포장이 내보인 값을 `$height` 와 `$width` 로 참조합니다:
...
속성 포장 구문은 그냥 획득자와 설정자가 있는 속성의 수월한 구문일 뿐이기 때문에, `height` 와 `width` 로 접근하는 건 다른 어떤 속성에 접근하는 것과 똑같이 동작합니다. 예를 들어, `resize(to:)` 안의 코드는 `height` 와 `width` 의 속성 포장으로 그들에 접근합니다. `resize(to: .large)` 를 호출하면, `.large` 의 switch 문 case 가 직사각형의 높이와 폭을 100 으로 설정합니다. 포장은 그 속성의 값이 12 보다 커지는 걸 막고, 내보인 값을 `true` 로 설정하여, 자신의 값을 적당히 조절한 사실을 기록합니다. `resize(to:)` 끝에서, 반환문이 `$heigh` 와 `$width` 를 검사하여 속성 포장이 `height` 나 `width` 중 어느 것이든 조절했는지 결정합니다. 

## Global and Local Variables (전역 및 지역 변수)

위에서 설명한 속성을 계산하고 관찰하는 능력은 _전역 변수 (global variables)_ 와 _지역 변수 (local variables)_ 에서도 쓸 수 있습니다. 전역 변수는 모든 함수나, 메소드, 클로저, 또는 타입의 밖에서 정의하는 변수입니다. 지역 변수는 함수나, 메소드, 또는 클로저 안에서 정의하는 변수입니다.

이전 장에서 마주쳤던 전역 및 지역 변수들은 모두 _저장 변수 (stored variables)_ 였습니다. 저장 변수는, 저장 속성 같이, 특정 타입의 값에 저장 공간을 제공하며 그 값을 설정하고 가져오는 걸 허용합니다.

하지만, 전역과 지역 어느 곳에서든, _계산 변수 (computed variables)_ 도 정의하고 저장 변수에 관찰자도 정의할 수 있습니다. 계산 변수는, 값을 저장하기 보단, 계산하며, 작성 방식은 계산 속성과 똑같습니다.

{: .note }
> 전역 상수와 변수는, [Lazy Stored Properties (느긋한 저장 속성)](#lazy-stored-properties-느긋한-저장-속성) 과 비슷한 예에 따라, 항상 느긋하게 계산됩니다. 느긋한 저장 속성과 달리, 전역 상수와 변수엔 `lazy` 수정자를 표시할 필요가 없습니다.
>
> 지역 상수와 변수는 절대로 느긋하게 계산하지 않습니다.

속성 포장은 지역 저장 변수에 적용할 수 있지만, 전역 변수나 계산 변수에는 안됩니다. 예를 들어, 아래 코드에선, `myNumber` 가 `SmallNumber` 를 속성 포장으로 사용합니다. 
...
`SmallNumber` 를 속성에 적용할 때 같이, `myNumber` 값을 10 으로 설정하는 건 유효합니다. 속성 포장이 12 보다 높은 값은 허용하기 않기 때문에, `myNumber` 에 24 대신 12 를 설정합니다.

## Type Properties (타입 속성)

인스턴스 속성은 한 특별한 타입의 인스턴스에 속하는 속성입니다. 그 타입의 인스턴스를 새로 생성할 때마다, 다른 어떤 인스턴스와도 구분되는, 자신만의 속성 값 집합을 갖게 됩니다.

그 타입의 어떤 한 인스턴스가 아닌, 타입 그 자체에 속하는 속성을 정의할 수도 있습니다. 그 타입의 인스턴스를 얼마나 많이 만들던 간에, 이 속성들의 복사본은 늘 하나만 있을 겁니다. 이러한 종류의 속성을 _타입 속성 (type properties)_ 이라 합니다.

타입 속성은 한 특별한 타입의 _모든 (all)_ 인스턴스에 보편적인 값을 정의하는데 유용하며, 가령 모든 인스턴스에서 사용할 수 있는 (**C** 의 정적 상수 같은) 상수 속성이나, 그 타입의 모든 인스턴스에 대해 전역인 값을 저장하는 (**C** 의 정적 변수 같은) 변수 속성이 그렇습니다.

저장 타입 속성은 변수나 상수일 수 있습니다. 계산 타입 속성은 항상 변수 속성으로 선언하며, 이는 계산 인스턴스 속성과 똑같은 방식입니다.

{: .note }
> 저장 인스턴스 속성과 달리, 저장 타입 속성엔 반드시 항상 기본 값을 줘야 합니다. 이는 타입 그 자체에는 초기화 시점에 저장 타입 속성에 값을 할당할 초기자가 없기 때문입니다.
>
> 저장 타입 속성은 최초 접근 시점에 느긋하게[^lazily] 초기화됩니다. 여러 쓰레드에서 동시에 접근할 때에도, 단 한 번만 초기화되는 걸 보증하며, `lazy` 수정자로 표시할 필요도 없습니다.

### Type Property Syntax (타입 속성 구문)

**C** 와 **오브젝티브-C** 에선, 타입과 결합된 정적 상수와 변수를 _전역 (global)_ 정적 변수로 정의합니다. 스위프트에선, 하지만, 타입 속성을, 타입의 바깥쪽 중괄호 안인, 타입 정의 부분에다 작성하며, 각 타입 속성의 시야 범위는 자신이 지원하는 타입입니다.
...
{: .note }
> 이 두 검사 중 첫 번째에서, `didSet` 관찰자는 `currentLevel` 을 다른 값으로 설정합니다. 이걸로, 하지만, 관찰자를 다시 호출하진 않습니다.

`AudioChannel` 구조체로 `leftChannel` 과 `rightChennel` 이라는 두 개의 새로운 음향 채널을 생성하여, 스테레오 음향 시스템의 음량을 나타낼 수 있습니다:
...
_왼쪽 (left)_ 채널의 `currentLevel` 을 `7` 로 설정하면, `maxInputLevelForAllChannels` 타입 속성이 `7` 과 같이 갱신되는 걸 볼 수 있습니다:
...
_오른쪽 (right)_ 채널의 `currentLevel` 을 `11` 로 설정하려고 하면, 오른쪽 채널의 `currentLevel` 속성이 최대 값인 `10` 으로 막혀서, `maxInputLevelForAllChannels` 타입 속성이 `10` 과 같이 업데이트되는 걸 볼 수 있습니다:
