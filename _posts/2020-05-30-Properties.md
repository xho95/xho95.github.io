...
> 전역 상수와 전역 변수는, [Lazy Stored Properties (느긋한 저장 속성)](#lazy-stored-properties-느긋한-저장-속성) 과 비슷한 관례대로, 항상 '느긋하게 (lazily)' 계산됩니다. '느긋한 저장 속성' 과는 달리, 전역 상수와 전역 변수는 `lazy` 수정자로 표시할 필요가 없습니다.
>
> 지역 상수와 지역 변수는 절대로 '느긋하게' 계산되지 않습니다.
