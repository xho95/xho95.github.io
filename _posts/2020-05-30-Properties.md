...
위에서 설명한 능력인 속성의 계산과 관찰은 _전역 변수 (global variables)_ 와 _지역 변수 (local variables)_ 에서도 쓸 수 있습니다. 전역 변수는 어떤 함수나, 메소드, 클로저, 또는 타입이든 그 밖에서 정의되어 있는 변수입니다. 지역 변수는 함수나, 메소드, 또는 클로저 안에 정의되어 있는 변수입니다.

이전 장에서 마주쳤던 전역 및 지역 변수들은 모두 _저장 변수 (stored variables)_ 였습니다. 저장 변수는, 저장 속성 같이, 특정 타입의 값에 저장 공간을 제공하며 그 값의 설정과 가져옴을 허용합니다.

하지만, _계산 변수 (computed variables)_ 를 정의하고 저장 변수에 관찰자를 정의하는 것도, 전역 및 지역 범위 중 어느 곳에서든, 할 수 있습니다. 계산 변수는, 값을 저장하기 보단, 계산하며, 쓰는 방식은 계산 속성과 똑같습니다.
