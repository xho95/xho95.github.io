...
속성 포장은 어떤 타입의 값이든 '드러낸 값' 으로 반환할 수 있습니다. 이 예제에 있는, 속성 포장은 단 한 조각의 정보-수를 조정했는지-만을 노출하므로 '불리언 (Boolean) 값' 을 '드러낸 값' 으로 노출합니다. 더 많은 정보를 노출할 필요가 있는 포장은 어떤 다른 자료 타입의 인스턴스를 반환할 수 도 있고, 아니면 '드러낸 값' 으로 포장의 인스턴스를 노출하기 위해 `self` 를 반환할 수도 있습니다.
