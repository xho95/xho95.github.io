...
상수나 변수의 현재 값을 `print(_:separator:terminator:)` 함수로 인쇄할 수 있습니다:
...
스위프트의 주석은 C 의 주석과 매우 비슷합니다. '한-줄짜리 (single-line) 주석' 은 '빗금 두 개 (two forward-slashes; `//`)' 로 시작합니다:

```swift
// 이것은 주석입니다.
```

'여러 줄짜리 (multiline) 주석' 은 빗금 뒤의 별표 (`/*`) 에서 시작하고 별표 뒤의 빗금 (`*/`) 에서 끝납니다:
...
C 에 있는 '여러 줄짜리 주석' 과는 다르게, 스위프트의 '여러 줄짜리 주석' 은 다른 '여러 줄짜리 주석' 을 '중첩할 (nested)' 수 있습니다. '중첩된 주석' 을 작성하려면 '여러 줄짜리 주석 블럭' 을 시작한 다음 첫 번째 블럭 내에서 두 번째 '여러 줄짜리 주석' 을 시작합니다. 그 다음에는 두 번째 블럭을 닫고, 이어서 첫 번째 블럭을 닫습니다:
...
'중첩된 여러 줄짜리 주석' 은, 큰 코드 블럭에 이미 '여러 줄짜리 주석' 이 있는 경우에도, 빠르고 쉽게 주석을 달 수 있게 해줍니다.

### 세미콜론 (Semicolons)

다른 많은 언어들과는 달리, 스위프트는 코드 각 구문 뒤에 세미콜론 (`;`) 을 붙이는 것이 필수가 아닌데, 물론 원한다면 그렇게 해도 되긴 합니다. 하지만, 구분된 여러 구문을 한 줄에 작성하려면 세미콜론 _이 (are)_ 필수입니다:
...
_정수 (integers)_ 는, `42` 와 `-23` 같이, 분수 성분이 없는 수 전부를 말합니다. 정수는 _부호 있는 (signed)_ 것 (양수, 0, 및 음수) 또는 _부호 없는 (unsigned)_ 것 (양수, 및 0) 이 있습니다.

스위프트는 8, 16, 32, 그리고 64 비트 형식의 '부호 있는 정수' 와 '부호 없는 정수' 를 제공합니다. 이 정수는 C 와 비슷한 '작명법 (naming convention)' 을 따라서, 8-비트 부호 없는 정수는 타입이 `UInt8` 이고, 32-비트 부호 있는 정수는 타입이 `Int32` 입니다. 스위프트의 모든 타입처럼, 이 정수 타입의 이름은 대문자로 시작합니다.
...
이 속성 값들은 (위 예제의 `UInt8` 처럼) 적절하게-크기가 조절된 수치 타입이므로 같은 타입의 다른 값들과 나란하게 표현식에서 사용할 수 있습니다.

#### Int

대부분의 경우, 코드에서 사용하는 정수의 크기는 지정할 필요가 없습니다. 스위프트는, `Int` 라는, 추가적인 정수 타입을 제공하는데, 이는 현재 플랫폼의 고유 '워드 (word)'[^word] 크기와 같은 크기를 가집니다:

* 32-비트 플랫폼에 대한, `Int` 는 `Int32` 와 같은 크기입니다.
* 64-비트 플랫폼에 대한, `Int` 는 `Int64` 와 같은 크기입니다.

특정 크기의 정수와 작업해야 하는 것이 아니라면, 코드의 정수 값으로는 항상 `Int` 를 사용합니다. 이는 코드의 '일관성 (consistency)' 과 '상호 호환성 (interoperability)' 을 높입니다. 32-비트 플랫폼에 대한, `Int` 도 `-2,147,483,648` 에서 `2,147,483,647` 에 이르는 값을 저장할 수 있으므로, 대부분의 정수 범위로 충분히 큽니다.

#### UInt

스위프트는, `UInt` 라는, 부호없는 정수 타입도 제공하는데, 이것도 현재 플랫폼의 고유 워드 크기와 같은 크기를 가집니다:

* 32-비트 플랫폼에 대한, `UInt` 는 `UInt32` 와 같은 크기입니다.
* 64-비트 플랫폼에 대한, `UInt` 는 `UInt64` 와 같은 크기입니다.
...
> `Double` 은 최소 '15 자리 유효 숫자 (15 decimal digits)' 의 정밀도를 가지는 반면, `Float` 의 정밀도는 좀 더 적은 '6 자리 유효 숫자 (6 decimal digits)' 입니다. 사용하기에 적당한 부동-소수점 타입은 코드에서 작업할 값의 본질과 범위에 달려 있습니다. 어느 타입이든 적당한 상황이라면, `Double` 이 더 좋습니다.
...
다른 정수 타입은 특별히 지금 당장 필요한 작업일 때만, 즉 외부 소스에서 크기가 명시적으로 정해진 자료, 또는 성능 및, 메모리 사용량을 위해서, 아니면 다른 최적화가 필요하기 때문인 경우에만 사용합니다. 이런 상황에서는 크기가 명시적으로 정해진 타입을 사용하는 것이 예기치 않은 어떤 '값 넘침 (overflows)' 을 잡아내는 것을 도와주며 사용하고 있는 자료의 본질을 암시적으로 문서화[^nature]하도록 합니다.
...
위 코드에서, 옵셔널 값 `assumedString` 은 `implicitString` 에 값을 할당하기 전에 강제로-포장이 풀리는데 이는 `implicitString` 의 타입이 명시적인, 옵셔널이-아닌 `String` 이기 때문입니다. 아래 코드의, `optionalString` 은 명시적인 타입을 가지고 있지 않으므로 이는 '일상적인 (ordinary) 옵셔널' 입니다.
...
'암시적으로 포장이 풀리는 옵셔널' 이 `nil` 인 경우 포장된 값에 접근하려고 하면, '실행 시간 에러 (runtime error)' 를 발생시킬 것입니다. 이 결과는 마치 값을 담지 않은 보통의 옵셔널 뒤에 느낌표를 붙인 경우에서와 정확히 똑같습니다.

'암시적으로 포장이 풀리는 옵셔널' 이 `nil` 인지 검사하는 방식은 보통의 옵셔널 검사와 똑같습니다:
...
'암시적으로 포장이 풀리는 옵셔널' 과 '옵셔널 연결' 을 같이 사용하여, 단일 구문에서 값을 검사하고 포장을 풀 수도 있습니다:
