...
단언문은 스위프트 표준 라이브러리의 [assert(_:_:file:line:)](https://developer.apple.com/documentation/swift/1541112-assert) 함수를 호출하여 작성합니다. 이 함수에 `true` 나 `false` 로 평가할 표현식과 조건 결과가 `false` 면 보여줄 메시지를 전달합니다. 예를 들면 다음과 같습니다:
