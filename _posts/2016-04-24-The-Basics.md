...
스위프트는 8, 16, 32, 그리고 64 비트 양식의 부호 있는 정수와 부호없는 정수를 제공합니다. 이 정수들은 C 언어의 '작명법 (naming convention)' 에 따라, 8-비트 부호없는 정수는 타입이 `UInt8` 이고, 32-비트 부호 있는 정수는 타입이 `Int32` 입니다. 스위프트의 모든 타입들 처럼, 이 정수 타입들의 이름은 대문자로 시작합니다.
...
스위프트는 부호없는 정수 타입으로, `UInt` 도 제공하며, 이는 현 플랫폼의 고유한 워드 크기와 같은 크기를 가집니다:
...
'타입 추론 장치' 가 더 유용한 순간은 상수나 변수를 선언할 때 기본 설정 값이 있는 경우입니다. 이는 보통 상수나 변수를 선언하는 시점에 _글자 (표현) 값 (literal value)_ (또는 그냥 '_글자 값 (literal)_') 을 할당하는 것으로 이루어 집니다. ('글자 값 (literal value)' 은 소스 코드 상에서 직접 드러나는 값을 말하는 것으로, 아래 예제에 있는 `42` 와 `3.14159` 같은 것들이 이에 해당합니다.)

예를 들어, 글자 값 `42` 를 새로운 상수에 할당하면서 타입을 알리지 않으면, 스위프트는 원하는 상수가 `Int` 라고 추론하며, 이는 정수 처럼 보이는 수로 초기화를 했기 때문입니다:
...
마찬가지로, '부동 소수점 글자 값 (floating-point literal)' 에 대해 타입을 지정하지 않으면, 스위프트는 원하는 타입이 `Double` 이라고 추론합니다:
...
한 표현식에 정수 글자 값과 부동-소수점 글자 값이 결합되어 있으면, 문맥을 통해서 `Double` 타입으로 추론합니다:
...
글자 값 `3` 이 명시적으로 타입을 갖는 것도 아니고 그 자신이 타입인 것도 아니니, 더하기 연산 중에 '부동-소수점 수 글자 값' 이 있는 것을 보고 결과 타입은 `Double` 이 알맞다고 추론하게 됩니다.

### Numeric Literals (수치 글자 값)

'정수 글자 값 (integer literals)' 의 작성법은 다음과 같습니다:
...
아래의 모든 '정수 글자 값' 은 10진수 값으로 `17` 을 가집니다:
...
```
'부동-소수점 글자 값' 은 (접두사가 없는) 10진 (decimal) 수 , 나 (접두사가 `0x` 인) 16진 (hexadecimal) 수 일 수 있습니다. 소수점 양 쪽에는 항상 숫자 (또는 16진 숫자) 가 있어야 합니다. 10진 부동 (소수점 수) 는 _지수 (exponent)_ 를 가질 수도 아닐 수도 있는데, 대문자 또는 소문자 `e` 로 이를 지시합니다; 16진 부동 (소수점 수) 는 반드시 지수를 가져야 하며, 대문자 또는 소문자 `p` 로 이를 지시합니다.
...
아래의 모든 '부동-소수점 글자 값' 은 10진수 값으로 `12.1875` 를 가집니다:
...
'수치 글자 값' 은 여분의 '양식 (formatting)' 을 써서 더 쉽고 이해하기 편하게 만들 수 있습니다. 정수와 부동 (소수점 수) 모두 여분의 0 을 채우고 '밑줄 (underscores)' 을 포함해서 가독성을 높일 수 있습니다. 어떤 종류의 양식도 '글자 값' 의 실제 값에는 영향을 미치치 않습니다:
...
[^private-use-Unicode-scalar-values]: '사용자 영역 유니코드 크기 값' 이란 '유니코드 평면 (Unicode planes)' 에서 '사용자 영역 (private-use areas)' 에 있는 값을 말합니다. 유니코드에는 '5번 평면 (`F0000 ~ FFFFF`)' 과 '16번 평면 (`100000 ~ 10FFFF`)', 이렇게 두 개의 '사용자 영역 (private-use areas)' 이 있습니다. 더 자세한 내용은 위키피디아의 [Plane (Unicode)](https://en.wikipedia.org/wiki/Plane_(Unicode)) 및 [유니코드 평면](https://ko.wikipedia.org/wiki/유니코드_평면) 항목을 참고하기 바랍니다.
