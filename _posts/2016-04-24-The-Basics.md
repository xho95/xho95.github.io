...
스위프트는 '_타입-안전 (type-sefe)_ 언어' 이며, 이는 '코드에서 작업할 값의 타입이 명확하도록 언어가 도와준다' 는 의미입니다. 코드가 `String` 을 요구하면, '타입 안전 장치 (type safety)' 가 `Int` 를 전달하는 실수를 막아줍니다. 마찬가지로, 타입 안전 장치는 '옵셔널-아닌 `String` 을 요구하는 코드' 에 '옵셔널 `String` 을 전달하는 사고' 도 막아줍니다. 타입 안전 장치는 개발 과정에서 에러를 가능한 빨리 잡아내고 고칠 수 있게 도와줍니다.
