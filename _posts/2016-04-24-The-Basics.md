...
`UInt` 는 특히 크기는 플랫폼의 워드 크기이면서 부호는 없는 정수가 꼭 필요할 때만 사용하기 바랍니다. 이런 경우가 아니라면, 설령 값이 음수가 될 일이 없더라도 `Int` 를 쓰도록 합니다. 정수 값에 `Int` 를 일관되게 사용하는 것은 코드의 '상호 호환성 (interoperability)' 에 도움이 되고, 서로 다른 수치 타입들끼리 변환해야 할 필요성를 없애며, 정수 타입 추론기와도 어울리는 것으로, 이는 [Type Safety and Type Inference (타입 안전기와 타입 추론기)](https://docs.swift.org/swift-book/LanguageGuide/TheBasics.html#ID322) 에서 설명하도록 합니다.
