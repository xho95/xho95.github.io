...
> 상수나 변수에 '스위프트에서 예약한 키워드 (reserved Swift keyword)' 와 같은 이름을 쓰고싶으면, 이름으로 쓸 키워드를 '역따옴표 (backticks; `` ` ``)'[^backticks] 로 감싸면 됩니다. 하지만 선택의 여지가 없는 것이 아니라면 키워드를 사용하지 않는 것이 좋습니다.
