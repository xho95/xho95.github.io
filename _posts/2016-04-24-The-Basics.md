...
다른 정수 타입은 특히, 외부 소스에서 크기를 명시적으로 정한 자료이거나, 성능이나, 메모리 사용량, 또는 다른 필요한 최적화 때문에, 머지 않아 그런 임무가 필요할 때만 사용합니다. 이 상황에서 명시적인 크기의 타입을 사용하는 것은 예기치 않은 어떤 '값 넘침 (overflows)' 을 잡아내도록 도와주며 사용 중인 자료의 '본성 (nature)' 을 암시적으로 '문서화 (documents)' [^documents] 합니다.
