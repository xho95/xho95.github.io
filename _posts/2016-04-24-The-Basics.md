...
코드에 있는 모든 일반적인-용도의 정수 상수와 변수에 대해서, 설령 음수가 아님을 알고 있다 하더라도, `Int` 타입을 사용하기 바랍니다. 언제 어디서나 기본 정수 타입을 사용하는 것은 코드에 있는 정수 상수와 변수가 그 즉시 '상호 호환 가능 (interoperable)' 하며 '정수 글자 값' 에 대해 추론된 타입과도 잘 들어 맞을 것임을 의미합니다.

지금 당장 꼭 필요한 작업일 때만 다른 정수 타입을 사용해야 하는데, 이는 외부 소스에서 데이터의 크기가 명시적으로 정해졌기 때문이거나, 또는 성능이나, 메모리 사용량을 위해서거나, 아니면 다른 최적화가 필요하기 때문이어야 합니다. 이러한 상황에서는 명시적인 크기의 타입을 사용해야 돌발적인 값 넘침도 잡아 낼 수 있고 사용중인 데이터의 본질을 암시적으로 문서화할 수 있게 됩니다.
