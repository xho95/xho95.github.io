...
단언문과 선행 조건문의 차이는 검사 시점에 있는데: 단언문은 '디버그 제작 (debug builds) 일 때만 검사' 하지만, 선행 조건문은 '디버그와 제품 제작 (debug and production builds) 둘 다에서 검사' 합니다. '제품 제작' 일 때는, 단언문 안의 조건을 평가하지 않습니다. 이는 개발 과정 동안, 제품 성능의 손실없이, 원하는 만큼 많은 단언문을 사용할 수 있음을 의미합니다.
