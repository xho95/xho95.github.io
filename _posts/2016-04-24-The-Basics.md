...
스위프트는 '8, 16, 32, 및 64 비트 형식의 부호 있는 그리고 부호 없는 정수' 를 제공합니다. 이 정수들은, '8-비트 부호 없는 정수' 가 `UInt8` 타입이고, '32-비트 부호 있는 정수' 는 `Int32` 타입이라는 점에서, C 와 비슷한 '작명법 (naming convention)' 을 따릅니다. 스위프트의 모든 타입 같이, 이 정수 타입의 이름은 대문자로 시작합니다.
