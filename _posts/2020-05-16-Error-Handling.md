...
에러를 던질 땐, 어떠한 주위 코드가 반드시-예를 들어, 문제를 바로 잡거나, 대안을 시도하거나, 또는 사용자에게 실패를 알림으로써-에러 처리를 책임져야 합니다.

스위프트엔 네 가지 에러 처리 방법이 있습니다. 함수에서 그 함수를 호출한 코드로 에러를 전파 (propagate) 하거나, `do-catch` 문으로 에러를 처리하거나, 옵셔널 값으로 에러를 처리하거나, 또는 에러가 일어나지 않을 거라고 단언 (assert) 할 수 있습니다. 각각의 접근법은 아래 부분에서 설명합니다.
