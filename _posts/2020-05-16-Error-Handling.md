...
`eat(item:)` 함수는 자신이 잡아낼 자판기 에러를 나열하는데, 에러 텍스트는 그 목록의 항목에 해당합니다. 나열된 세 개의 에러 중 어떤 걸 던지면, 이 `catch` 절이 메시지를 인쇄하는 걸로 이들을 처리합니다. 어떤 에러든 다른 거면 주위 영역으로 퍼뜨리는데, 여기엔 나중에 추가될지도 모를 자판기 에러도 포함합니다.
