...
`someThrowingFunction()` 이 에러를 던지면, `x` 와 `y` 값은 `nil` 입니다. 그 외 경우, `x` 와 `y` 값은 함수의 반환 값입니다. `x` 와 `y` 는 `someThrowingFunction()` 의 반환 타입이 뭐든 옵셔널임을 기억하기 바랍니다. 여기선 함수가 정수를 반환하므로, `x` 와 `y` 는 옵셔널 정수입니다.

`try?` 를 사용하면 간결한 에러 처리 코드로 모든 에러를 똑같은 방식으로 처리하게 합니다.[^error-to-optional] 예를 들어, 다음 코드는 여러가지 접근법을 사용하여 자료를 가져오거나, 아니면 모든 접근법이 실패하여 `nil` 을 반환합니다.
...
사실, 던지는 함수나 메소드가 실행 시간에 에러를 던지지 않음을 알 때가 있습니다. 그럴 땐, 표현식 앞에 `try!` 를 작성하면 에러 전파를 못하게 하고 아무 에러도 던지지 않는다는 실행시간 단언문 (runtime assertion) 으로 호출을 포장할 수 있습니다. 실제로 에러를 던지면, 실행시간 에러를 가지게 됩니다.[^runtime-error]

예를 들어, 다음 코드는 `loadImage(atPath:)` 함수를 사용하여, 주어진 경로의 이미지를 불러오거나 이미지를 불러올 수 없으면 에러를 던집니다. 이 경우, 이미지는 응용 프로그램과 같이 출하하기 때문에, 실행 시간에 에러를 던지지 않으므로, 에러 전파를 못하게 하는 게 적절합니다.
...
[^runtime-error]: 본문에서 말하는 '실행 시간에 에러를 던지지 않음을 알 때' 라는 건, 결국 '실행 시간에 에러가 나면 안되는 상황' 을 말합니다. 즉, `try!` 는 실행 시간에 에러가 나면 안되는 걸 개발 과정에서 미리 파악하여 조치하기 위해 사용합니다.
