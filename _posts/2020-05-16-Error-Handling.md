...
`catch` 절은 `do` 절 코드가 던질 수 있는 모든 에러를 처리하지 않아도 됩니다. 아무런 `catch` 절도 에러를 처리하지 않으면, 주위 영역으로 에러를 퍼뜨립니다. 하지만, 퍼뜨린 에러는 반드시 _어떠한 (some)_ 주위 영역에서든 처리해야 합니다. 던지지 않는 함수라면, 테두리 친 `do`-`catch` 문이 반드시 에러를 처리해야 합니다. 던지는 함수라면, 테두리 친 `do-catch` 문이나 호출한 쪽 중에서 어느 한 곳이 반드시 에러를 처리해야 합니다. 처리 안된 에러를 최상단 시야 범위로 퍼뜨리면, 실행시간 에러를 가질겁니다.

예를 들어, 위에 있는 예제를 다시 작성하여 `VendingMachineError` 가 아닌 어떤 에러든 호출하는 쪽 함수가 대신 잡아내게 할 수 있습니다:
...
`nourish(with:)` 함수에서, `vend(itemNamed:)` 이 던진 에러가 `VendingMachineError` 열거체의 한 case 라면, `nourish(with:)` 가 메시지를 인쇄하는 걸로 에러를 처리합니다. 그 외의 경우, `nourish(with:)` 는 자신을 호출한 쪽으로 에러를 퍼뜨립니다. 그러면 일반적인 `catch` 절이 에러를 잡아냅니다.

여러가지 에러를 잡아내는 또 다른 방법은 `catch` 뒤에 이들을 나열하면서, 쉼표로 구분하는 겁니다. 예를 들면 다음과 같습니다:
...
`eat(item:)` 함수는 자신이 잡아낼 자판기 에러를 나열하는데, 에러 텍스트는 그 목록의 항목에 해당합니다. 나열된 세 개의 에러 중 어떤 걸 던지면, 이 `catch` 절이 메시지를 인쇄하는 걸로 이들을 처리합니다. 다른 에러는 어떤 거든 주위 영역으로 퍼뜨리는데, 이는 나중에 추가될지도 모를 자판기 에러도 포함합니다.

#### Converting Errors to Optional Values (에러를 옵셔널 값으로 변환하기)

`try?` 를 쓰면 옵셔널 값으로 변환하는 걸로 에러를 처리합니다. `try?` 표현식을 평가하는 동안 에러를 던지면, 표현식의 값이 `nil` 이 됩니다. 예를 들어, 다음 코드에서 `x` 와 `y` 는 값과 동작이 똑같습니다:
...
`someThrowingFunction()` 이 에러를 던지면, `x` 와 `y` 값이 `nil` 이 됩니다. 그 외 경우, `x` 와 `y` 값은 함수가 반환한 값입니다. `x` 와 `y` 는 `someThrowingFunction()` 이 무슨 타입을 반환하던 옵셔널임을 알아두기 바랍니다. 여기선 함수가 정수를 반환하므로, `x` 와 `y` 는 옵셔널 정수입니다.

`try?` 를 사용하면 모든 에러를 똑같은 방식으로 처리하고 싶을 때 간결한 에러 처리 코드를 쓸 수 있게 합니다.[^error-to-optional] 예를 들어, 다음 코드는 여러가지 접근 방법을 써서 데이터를 가져오거나, 모든 접근 방법이 실패하면 `nil` 을 반환합니다.
...
`defer` 문은 현재 시야 범위를 빠져나가기 전까지 실행을 미룹니다. 이 구문은 `defer` 키워드와 나중에 실행할 구문으로 이루어집니다. 미뤄둔 구문은, `break` 나 `return` 문, 또는 에러 던지기 같이, 구문 밖으로 제어를 옮길 어떤 코드도 담지 않을 수 있습니다. 미룬 행동은 소스 코드에 쓰여진 역순으로 실행됩니다. 즉, 첫 번째 `defer` 문 안의 코드는 마지막에 실행하고, 두 번째 `defer` 문 안의 코드는 마지막에서 두 번째로 실행하는, 식으로 계속됩니다. 소스 코드의 마지막 `defer` 문을 첫 번째로 실행합니다.
