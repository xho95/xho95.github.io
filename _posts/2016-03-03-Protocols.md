...
`FullyNamed` 프로토콜은 '준수 타입' 이 온전히 '규명된 (qualified)'[^qualified] 이름을 필수로 제공하도록 요구합니다. 이 프로토콜은 준수 타입의 본질에 대하여 어떤 다른 것도 지정하지 않습니다-오직 타입이 반드시 스스로 전체 이름을 제공할 수 있어야 한다는 것만 지정합니다. 이 프로토콜은 어떤 `FullyNamed` 타입이든, 타입이 `String` 인, `fullName` 이라는 획득 가능한 인스턴스 속성을 반드시 가져야 한다고 알립니다.
