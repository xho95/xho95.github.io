...
프로토콜은 클래스, 구조체, 및 열거체와 아주 비슷한 식으로 정의합니다:
...
사용자 정의 타입은, 자신의 정의 부분에서, 타입 이름 뒤에 프로토콜 이름을, 콜론으로 구분하여, 둠으로써 특별한 프로토콜을 채택한다고 알립니다. 쉼표로 구분하여, 여러 개의 프로토콜을 나열할 수도 있습니다:
...
### Property Requirements (속성 요구 조건)

프로토콜은 어떤 준수 타입이든 특별한 이름과 타입을 가진 인스턴스 속성 및 타입 속성을 제공하길 요구할 수 있습니다. 속성이 저장 속성이어야 하는지 계산 속성이어야 하는지는 프로토콜이 지정하지 않습니다-필요한 속성 이름과 타입만 지정합니다. 프로토콜은 각각의 속성이 반드시 획득 가능해야 하는지 아니면 획득 가능 _하면서 (and)_ 설정 가능해야 하는 지도 지정합니다.

프로토콜이 획득 가능하면서 설정 가능한 속성을 요구할 경우, 상수 저장 속성이나 읽기-전용 계산 속성으론 그 속성 요구 조건을 충족할 수 없습니다. 프로토콜이 획득 가능한 속성만을 요구할 경우, 어떤 종류의 속성이든 요구 조건을 만족할 수 있으며, 속성이 설정 가능한 게 자신의 코드에 유용할 경우 이것도 유효합니다.

속성 요구 조건은, `var` 키워드 접두사의, 변수 속성으로 항상 선언합니다. 획득 가능하면서 설정 가능한 속성은 자신의 타입 선언 뒤에 `{ get set }` 을 작성하여 지시하며, 획득 가능한 속성은 `{ get }` 을 작성하여 지시합니다.
...
프로토콜에서 타입 속성 요구 조건을 정의할 땐 항상 `static` 키워드 접두사를 붙입니다. 클래스가 구현할 땐 타입 속성 요구 조건에 `class` 나 `static` 키워드 접두사가 붙을 수 있을지라도 이 규칙은 그대로 적용됩니다[^type-property-requirements]:
...
프로토콜은 준수 타입이 특정 인스턴스 메소드 및 타입 메소드를 구현하길 요구할 수 있습니다. 이 메소드들을 프로토콜 정의 부분에 작성하는 방식은 보통의 인스턴스 및 타입 메소드와 정확히 똑같지만, 중괄호나 메소드 본문은 없습니다. 가변 매개 변수는, 보통의 메소드에서와 규칙이 동일하다는 전제로, 허용합니다. 하지만, 프로토콜 정의 안의 메소드 매개 변수에 기본 값을 지정할 순 없습니다.

타입 속성 요구 조건 처럼, 프로토콜에서 타입 메소드 요구 조건을 정의할 땐 항상 `static` 키워드 접두사를 붙입니다. 클래스가 구현할 땐 타입 메소드 요구 조건에 `class` 나 `static` 키워드 접두사가 붙을지라도 그렇습니다:
...
### Mutating Method Requirements (변경 메소드 요구 조건)

메소드가 자신이 속한 인스턴스를 수정-또는 _변경 (mutate)_-하는게 필요할 때가 있습니다. 값 타입 (즉, 구조체 및 열거체) 의 인스턴스 메소드는 메소드 `func` 키워드 앞에 `mutating` 키워드를 붙여서 메소드가 자신이 속한 인스턴스 및 그 인스턴스의 어떤 속성도 수정하도록 허용함을 지시합니다. 이 과정은 [Modifying Value Types from Within Instance Methods (인스턴스 메소드 안에서 값 타입 수정하기)]({% post_url 2020-05-03-Methods %}#modifying-value-types-from-within-instance-methods-인스턴스-메소드-안에서-값-타입-수정하기) 에서 설명합니다.

프로토콜을 채택한 어떤 타입의 인스턴스든 변경할 의도로 프로토콜 인스턴스 메소드 요구 조건을 정의한다면, 프로토콜 정의 부분에서 메소드에 `mutating` 키워드를 표시합니다. 이는 구조체와 열거체가 프로토콜을 채택하고 그 메소드 요구 조건을 만족할 수 있게 합니다.

> 프로토콜 인스턴스 메소드 요구 조건을 `mutating` 으로 표시한 경우, 클래스에서 그 메소드 구현을 작성할 땐 `mutating` 키워드를 작성할 필요가 없습니다. `mutating` 키워드는 구조체와 열거체만 사용합니다.

아래 예제는 `Togglable` 이라는 프로토콜을 정의하는데, 이는 `toggle` 이라는 단일 인스턴스 메소드 요구 조건을 정의합니다. 이름이 제시하듯이, `toggle()` 메소드는, 전형적으로 그 타입의 속성을 변경함으로써, 어떤 준수 타입의 상태를 전환 (toggle) 또는 반전 (invert) 할 의도입니다.

`Togglable` 프로토콜 정의 부분에서 `toggle()` 메소드에 `mutating` 키워드를 표시하여, 메소드를 호출할 때 준수 인스턴스[^conforming-instance] 의 상태를 변경할 걸로 예상한다고 지시합니다:
...
구조체나 열거체가 `Togglable` 프로토콜을 구현하는 경우, 그 구조체나 열거체는 `mutating` 으로도 표시한 `toggle()` 메소드를 구현함으로써 프로토콜을 준수할 수 있습니다.
