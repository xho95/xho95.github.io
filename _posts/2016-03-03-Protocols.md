...
프로토콜 준수성을 검사해서, 특정 프로토콜로 '변환 (cast)' 하기 위해, [Type Casting (타입 변환)]({% post_url 2020-04-01-Type-Casting %}) 에서 설명한 `is` 와 `as` 연산자를 사용할 수 있습니다. '프로토콜 검사 및 변환' 은 '타입 검사 및 변환' 과 정확하게 똑같은 구문을 따릅니다:
