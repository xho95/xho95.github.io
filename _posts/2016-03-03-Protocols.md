...
프로토콜은 특정 인스턴스 메소드 및 타입 메소드를 준수 타입이 구현하길 요구할 수 있습니다. 이 메소드들을 프로토콜 정의 부분에 작성하는 방식은 보통의 인스턴스 및 타입 메소드와 정확히 똑같지만, 중괄호 또는 메소드 본문이 없습니다. 가변 매개 변수는, 보통의 메소드에서와 규칙이 동일하다는 전제로, 허용합니다. 하지만, 프로토콜 정의 안의 메소드 매개 변수에 기본 값을 지정할 순 없습니다.
