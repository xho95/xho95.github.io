...
`FullyNamed` 프로토콜은 준수 타입이 완전히 규명된 이름[^qualified] 을 제공하길 요구합니다. 프로토콜은 준수 타입의 어떤 다른 본성도 지정하지 않습니다-반드시 타입 그 자체가 전체 이름 제공할 수 있어야 한다는 것만 지정합니다. 프로토콜은 어떤 `FullyNamed` 타입에든, `fullName` 이라는, `String` 타입의, 획득 가능 인스턴스 속성이 있어야 함을 알립니다.
