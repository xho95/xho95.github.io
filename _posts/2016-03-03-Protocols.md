...
'프로토콜' 은 어떤 '준수 타입' 이든 특별한 이름과 타입을 가진 인스턴스 속성이나 타입 속성을 필수로 제공하도록 할 수 있습니다. 프로토콜은 속성이 저장 속성인지 계산 속성인지는 지정하지 않습니다-필수인 속성 이름과 타입만을 지정합니다. 프로토콜은 각각의 속성이 반드시 '획득 가능 (gettable)' 해야 하는지 아니면 반드시 '획득 가능 (gettable)' _하면서 (and)_ '설정 가능 (settable)' 해야 하는 지도 지정합니다.
