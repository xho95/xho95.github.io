...
> 배열과, 딕셔너리, 및 문자열 같이 표준 라이브러리에서 정의한 집합체[^collections] 는 최적화를 써서 복사하기 위한 성능 비용을 감소시킵니다. 곧바로 복사하는 대신, 이러한 집합체는 원본 인스턴스와 복사본이 원소를 저장한 곳의 메모리를 공유합니다. 집합체의 한 복사본을 수정하면, 수정 직전에 원소를 복사합니다. 코드의 동작은 마치 항상 곧바로 복사하는 듯이 보입니다.
