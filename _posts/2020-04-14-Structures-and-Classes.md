...
_구조체 (structures)_ 와 _클래스 (classes)_ 는 '프로그램 코드의 건축 자재 역할을 하는 범용적이고, 유연한 구조물 (constructs)' 입니다. 상수, 변수, 및 함수 정의에 사용하는 것과 똑같은 구문을 사용하여 구조체와 클래스에 기능을 추가하는 속성과 메소드를 정의합니다.

다른 프로그래밍 언어와 달리, 스위프트는 사용자 정의 구조체와 클래스를 인터페이스와 구현 파일로 분리 생성할 것을 요구하지 않습니다. 스위프트에서, 구조체나 클래스를 단일 파일로 정의하면, 다른 코드가 그 클래스나 구조체의 외부 인터페이스를 사용하는 게 자동으로 가능해집니다.

> 전통적으로 클래스 인스턴스만 _객체 (object)_ 라고 합니다. 하지만, 스위프트 구조체와 클래스는 다른 언어에서보다 그 기능이 훨씬 더 가까우며, 이 장 많은 곳에서 설명할 기능은 클래스나 구조체 _어느 (either)_ 타입의 인스턴트에든 적용되는 것입니다. 이 때문에, 좀 더 일반적인 용어인 _인스턴스 (instance)_ 를 사용합니다.[^object-instance]
