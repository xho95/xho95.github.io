...
읽기 및 쓰기 접근의 차이는 대체로 명백한데: 쓰기 접근은 그 장소의 메모리를 바꾸지만, 읽기 접근은 아니라는 겁니다. 메모리 장소란 접근하는게 뭔지-예를 틀어, 변수나, 상수, 또는 속성-인지를 말합니다. 메모리 접근의 지속 시간은 순간 (instantaneous) 이거나 장-기간 (long-term) 입니다.

연산이 _원자적 (atomic)_ 이란 건 C-언어의 원자적 (atomic) 연산만 사용하는 경우이며; 그 외 경우엔 원자적이 아닙니다 (nonatomic). 이러한 함수의 목록은, `stdatomic(3)` 매뉴얼 페이지 (man page)[^man-page] 를 참고하기 바랍니다.

접근이 _순간적 (instantaneous)_ 이란 건 그 접근을 시작한 후엔 끝나기 전까지 다른 코드의 실행이 불가능한 경우입니다. 태생적으로, 두 개의 '순간적인 접근' 이 동시에 발생할 수는 없습니다. 대부분의 메모리 접근은 '순간적' 입니다. 예를 들어, 아래 나열한 코드에 있는 모든 읽기 접근과 쓰기 접근은 '순간적' 입니다:
