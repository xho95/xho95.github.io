...
위 예제에서, **Oscar** 의 `shareHealth(with:)` 메소드를 호출하여 **Maria** 와 체력을 공유하는 건 충돌을 일으키지 않습니다. 메소드 호출 중에 `oscar` 로의 쓰기 접근이 계속되는 건 변경 메소드에선 `self` 의 값이 `oscar` 이기 때문이며, 같이 기간에 `maria` 로의 쓰기 접근이 계속되는 건 `maria` 가 입-출력 매개 변수로 전달됐기 때문입니다. 아래 그림에 보듯, 이들은 서로 다른 장소의 메모리에 접근합니다. 두 쓰기 접근의 시간이 겹치더라도, 충돌하진 않습니다.

![access different locations in memory](/assets/Swift/Swift-Programming-Language/Memory-Safety-self-different-memory.jpg)

하지만, `oscar` 를 `shareHealth(with:)` 의 인자로 전달하면, 충돌합니다:
...
변경 메소드는 메소드가 계속되는 동안 `self` 로 쓰기 접근을 할 필요가 있고, 입-출력 매개 변수는 같은 기간 동안 `teammate` 로 쓰기 접근을 할 필요가 있습니다. 메소드 안에서, `self` 와 `teammate` 는 둘 다 같은 장소의 메모리를 참조하는데-이는 아래 모형에서 보는 것과 같습니다. 두 개의 쓰기 접근이 똑같은 메모리를 참조하면서 서로 겹치므로, 충돌을 만듭니다.
...
구조체와, 튜플, 및 열거체 같은 타입은, 구조체의 속성이나 튜플의 원소 같이, 개별 구성 요소 값들로 이루어집니다. 이들은 값 타입이기 때문에, 어떤 부분 값을 변경하든 전체 값을 변경하는 것으로, 이는 한 속성으로의 읽기나 쓰기 접근이라도 전체 값으로의 읽기나 쓰기 접근을 요구한다는 걸 의미합니다. 예를 들어, 튜플 원소로의 쓰기 접근이 겹치는 건 충돌을 만듭니다:
...
구조체 속성으로의 접근이 겹치는 걸 제약하는게 메모리 안전성의 보존에 항상 필요한 건 아닙니다. 메모리 안전성이 보증하길 원하는 것이지만, 독점적 접근은 메모리 안전성보다 더 엄격한 필수 요구 조건입니다[^stricter-requirement]-이는 어떤 코드에서 메모리 안전성은, 심지어 메모리로의 독점 접근을 위반하더라도 보존한다는 의미입니다. 스위프트는 컴파일러가 여전히 메모리로의 비독점 접근이 안전하다는 걸 증명할 수 있다면 이러한 메모리-안전 코드도 허용합니다. 구체적으로, 다음의 조건이 적용되면 구조체 속성으로의 접근 겹침이 안전하다는 걸 증명할 수 있습니다:

* 인스턴스의 저장 속성에만 접근하지, 계산 속성이나 클래스 속성엔 안합니다.
* 구조체가 지역 변수의 값이지, 전역 변수가 아닙니다.
* 구조체가 어떤 클로저로도 붙잡히지 않았거나, 벗어나지 않는 클로저로만 붙잡힌 겁니다.

컴파일러는 접근이 안전하다는 걸 증명하지 못하면, 접근을 허용하지 않습니다.
...
[^stricter-requirement]: 메모리 접근을 독점하면 메모리 안전성은 무조건 보장되지만, 독점 접근이 아닌 경우에도 메모리 안정성을 보장하는 경우가 있다는 의미입니다.
