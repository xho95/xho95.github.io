...
`switch` 문을 사용하여 `is` 및 `as` 패턴과 값의 일치 여부를 맞춰보는 예제는, [Type Casting for Any and AnyObject ('Any' 와 'AnyObject' 에 대한 타입 변환)]({% post_url 2020-04-01-Type-Casting %}#type-casting-for-any-and-anyobject-any-와-anybbject-에-대한-타입-변환) 을 참고하기 바랍니다.
