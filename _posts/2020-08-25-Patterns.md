...
### Identifier Pattern (식별자 유형)

_식별자 유형 (identifer pattern)_ 은 어떤 값과도 일치하며 '일치한 값을 변수나 상수 이름에 연결' 합니다. 예를 들어, 다음 상수 선언의, `someValue` 는 `Int` 타입의 값 `42` 와 일치하는 '식별자 유형' 입니다:
...
### Value-Binding Pattern (값-연결 유형)

_값-연결 유형 (value-binding pattern)_ 은 '일치 값' 을 '변수나 상수 이름에 연결' 합니다. 값-연결 유형에서 일치 값을 상수 이름에 연결하는 건 `let` 키워드로 시작하고; 변수 이름에 연결하는 건 `var` 키워드로 시작합니다.
...
_튜플 유형 (tuple pattern)_ 은 '0개 이상의 패턴을 쉼표로 구분하고, 괄호로 테두리 친, 목록' 입니다. '튜플 유형' 은 '관련 튜플 타입 값' 과 일치합니다.
