...
_패턴 (pattern)_ 은 '단일 값 또는 합성 값의 구조 (structure)' 를 표현합니다. 예를 들어, '`(1, 2)` 라는 튜플의 구조' 는 '두 원소를 쉼표로 구분한 목록' 입니다. '패턴' 은 어떤 하나의 특별한 값이라기 보다는 '값의 구조' 를 표현하기 때문에, 이는 다양한 값과 일치할 수 있습니다. 구체적인 사례를 보면, '`(x, y)` 라는 유형' 은 '`(1, 2)` 라는 튜플' 및 '어떤 다른 두-원소의 튜플' 과도 일치합니다. 유형과 값의 일치에 더하여, 합성 값의 일부나 전체를 추출하여 각 부분을 상수나 변수 이름에 연결할 수도 있습니다.
...
두 번째 종류의 유형은, 맞춰보려는 값이 실행 시간에 없을 수도 있는, '전체 패턴 맞춤 (full pattern matching)' 에 사용합니다. 이는 '열거체 case(enumeration case) 유형', '옵셔널 (optional) 유형', '표현식 (expression) 유형', 및 '타입-변환 (type-casting) 유형' 을 포함합니다. 이 유형들은 '`switch` 문의 `case` 이름표' 나, '`do` 문의 `catch` 절', 또는 '`if`, `while`, `guard`, 및 `for`-`in` 문의 조건' 에 사용합니다.

> GRAMMAR OF A PATTERN 부분 생략 - [링크](https://docs.swift.org/swift-book/ReferenceManual/Patterns.html)

### Wildcard Pattern (와일드카드 유형)
...
_식별자 패턴 (identifer pattern)_ 은 어떤 값과도 일치하며 '일치한 값을 변수나 상수 이름에 연결' 합니다. 예를 들어, 다음 상수 선언의, `someValue` 는 `Int` 타입의 값 `42` 와 일치하는 '식별자 유형' 입니다:
...
변수나 상수 선언 왼쪽의 유형이 식별자 유형일 때는, 암시적으로 '식별자 유형' 이 '값-연결 유형의 하위 유형' 입니다.
...
_값-연결 패턴 (value-binding pattern)_ 은 '일치 값' 을 '변수나 상수 이름에 연결' 합니다. 값-연결 유형에서 일치 값을 상수 이름에 연결하는 건 `let` 키워드로 시작하고; 변수 이름에 연결하는 건 `var` 키워드로 시작합니다.

값-연결 유형 안의 '식별자 유형' 은 '새로 이름 붙인 변수나 상수를 자신과 일치하는 값에 연결' 합니다. 예를 들어, 튜플의 원소를 분해해서 '각 원소 값을 관련 식별자 유형에 연결' 할 수 있습니다.
...
위 예제에서는, `let` 을 `(x, y)` 튜플 유형의 각 식별자 유형으로 배포합니다.[^distribute] 이 동작 때문에, `case let (x, y):` 와 `case (let x, let y):` 라는 `switch` 문 case 절' 들이 일치하는 값은 똑같습니다.

> GRAMMAR OF A VALUE-BINDING PATTERN 부분 생략 - [링크](https://docs.swift.org/swift-book/ReferenceManual/Patterns.html#ID422)

### Tuple Pattern (튜플 패턴)

_튜플 패턴 (tuple pattern)_ 은 '쉼표로 구분한 0개 이상의 유형을, 괄호로 테두리 친, 목록' 입니다. '튜플 유형' 은 '관련 튜플 타입 값' 과 일치합니다.

튜플 타입이 '정해진 종류의 튜플 타입과 일치하도록 구속' 하기 위해 '타입 보조 설명' 을 사용할 수 있습니다. 예를 들어, '`let (x, y): (Int, Int) = (1, 2)` 라는 상수 선언' 에 있는 '`(x, y): (Int, Int)` 라는 튜플 유형' 은 '원소 둘 다 `Int` 타입인 튜플 타입' 과만 일치합니다.

튜플 유형을 `for`-`in` 구문에 있는 유형이나 변수 또는 상수 선언에 있는 유형으로 사용할 때는, 와일드카드 유형, 식별자 유형, 옵셔널 패턴, 또는 이들을 가지고 있는 다른 튜플 패턴만을 가질 수 있습니다. 예를 들어, 다음의 코드는 유효하지 않는데 튜플 패턴 `(x, 0)` 에 있는 `0` 이라는 원소가 '표현식 패턴 (expression pattern)' 이기 때문입니다:
...
'열거체 case 유형' 은 '해당 case 값을 옵셔널로 포장한 값' 과도 일치합니다. 이 단순화 구문은 '옵셔널 패턴' 을 생략하게 해줍니다. `Optional` 은 열거체로 구현하기 때문에, `.none` 과 `.some` 이 '동일한 swift 문' 에서 열거체 타입의 'case 값' 과 같이 있을 수 있음을 기억하기 바랍니다.
...
[^distribute]: '튜플 유형의 각 식별자 유형으로 `let` 을 배포한다' 는 것은 뒤에서 설명하는 것처럼 `let (x, y)` 를 `(let x, let y)` 로 자동으로 만들어 준다는 의미입니다.