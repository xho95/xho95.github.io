...
_열거체 case 패턴 (enumeration case pattern)_ 은 '기존 열거체 타입의 case 값' 과 일치합니다. '열거체 case 패턴' 은 '`switch` 문의 case 이름표' 와 `if`, `while`, `guard`, 및 `for`-`in` 문의 'case 조건' 에 있습니다.

맞춰보려는 '열거체 case 값' 이 어떤 '결합 값' 이든 가지고 있으면, 관련된 '열거체 case 패턴' 은 반드시 '각 결합 값마다 하나의 원소를 담은 튜플 패턴' 을 지정해야 합니다. '`switch` 문' 으로 '결합 값을 담고 있는 열거체 case 값' 을 맞춰보는 예제는, [Associated Values (결합 값)]({% post_url 2020-06-13-Enumerations %}#associated-values-결합-값) 부분을 참고하기 바랍니다.
