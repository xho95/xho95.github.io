...
_열거체 case 유형 (enumeration case pattern)_ 은 기존 열거체 타입의 'case 값' 과 일치합니다. '열거체 case 유형' 은 '`switch` 문의 case 이름표' 에서 그리고 `if`, `while`, `guard`, 및 `for`-`in` 문의 'case 조건' 에서 나타납니다.

일치하는지 맞춰보려는 열거체 case 값이 '결합 값' 을 가지고 있는 경우, 연관된 '열거체 case 유형' 은 각 결합 값에 대해 하나의 원소를 가지는 튜플 패턴을 반드시 지정해야 합니다. `switch` 문을 사용하여 '결합 값' 을 가지는 '열거체 case 값' 의 일치 여부를 맞춰보는 예제는, [Associated Values (결합 값)]({% post_url 2020-06-13-Enumerations %}#associated-values-결합-값) 을 참고하기 바랍니다.

열거체 case 유형은 해당 'case 값' 을 옵셔널로 포장한 값과도 일치합니다. 이렇게 간소화한 구문 표현은 '옵셔널 패턴' 을 생략하도록 해줍니다. `Optional` 이 열거체로 구현되어 있기 때문에, `.none` 과 `.some` 이 같은 'switch 문' 에서 열거체 타입의 'case 값' 으로 나타날 수 있음에 주목하기 바랍니다.
