...
튜플 타입이 '정해진 종류의 튜플 타입과 일치하도록 구속' 하기 위해 '타입 보조 설명' 을 사용할 수 있습니다. 예를 들어, '`let (x, y): (Int, Int) = (1, 2)` 라는 상수 선언' 에 있는 '`(x, y): (Int, Int)` 라는 튜플 패턴' 은 '원소 둘 다 `Int` 타입인 튜플 타입' 과만 일치합니다.
