...
is 패턴은 런타임에 해당 값의 유형이 is 패턴의 오른쪽에 지정된 유형 또는 해당 유형의 하위 클래스에 지정된 유형과 동일한 경우 값과 일치합니다. is 패턴은 둘 다 형식 캐스트를 수행하지만 반환 된 형식을 버린다는 점에서 is 연산자처럼 동작합니다.

as 패턴은 런타임에 해당 값의 유형이 as 패턴의 오른쪽에 지정된 유형 또는 해당 유형의 하위 클래스에 지정된 유형과 동일한 경우 값과 일치합니다. 일치가 성공하면 일치 된 값의 유형이 as 패턴의 오른쪽에 지정된 패턴으로 캐스트됩니다.

switch 문을 사용하여 값을 is 및 as 패턴과 일치시키는 예제는 Any 및 AnyObject에 대한 유형 캐스팅을 참조하세요.
