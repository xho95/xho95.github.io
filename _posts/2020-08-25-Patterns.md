...
맞춰보려는 열거체 case 값에 어떤 결합 값이 있으면, 해당하는 열거체 case 패턴에도 반드시 각각의 결합 값마다 한 원소씩 담은 튜플 패턴을 지정해야 합니다. `switch` 문을 써서 결합 값을 담은 열거체 case 와 맞춰보는 예제는, [Associated Values (결합 값)]({% link docs/books/swift-programming-language/enumerations.md %}#associated-values-결합-값) 부분을 보기 바랍니다.
...
`switch` 문을 써서 `is` 와 `as` 패턴으로 값을 맞추는 예제는, [Type Casting for Any and AnyObject (Any 와 AnyObject 의 타입 변환)]({% link docs/books/swift-programming-language/type-casting.md %}#type-casting-for-any-and-anyobject-any-와-anyobject-의-타입-변환) 부분을 보기 바랍니다.
...
[Generic Parameters and Arguments (일반화 매개 변수와 인자)]({% link docs/books/swift-programming-language/generic-parameters-and-arguments.md %})
