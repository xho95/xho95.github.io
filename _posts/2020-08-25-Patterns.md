...
옵셔널 패턴은 '`Optional` 열거체 case 값 패턴' 에 대한 '구문 표현을 수월하게 한 것 (syntatic sugar)' 이므로, 다음은 '동치 (equivalent)' 입니다:
...
식 패턴은 식의 값을 나타냅니다. 식 패턴은 switch 문 케이스 레이블에만 나타납니다.

표현식 패턴이 나타내는 표현식은 Swift 표준 라이브러리 ~ = 연산자를 사용하여 입력 표현식의 값과 비교됩니다. ~ = 연산자가 true를 반환하면 일치가 성공합니다. 기본적으로 ~ = 연산자는 == 연산자를 사용하여 동일한 유형의 두 값을 비교합니다. 다음 예와 같이 값이 범위 내에 포함되어 있는지 확인하여 값 범위와 값을 일치시킬 수도 있습니다.
...
~ = 연산자를 오버로드하여 사용자 지정 식 일치 동작을 제공 할 수 있습니다. 예를 들어 위의 예를 다시 작성하여 포인트 표현식을 포인트의 문자열 표현과 비교할 수 있습니다.
