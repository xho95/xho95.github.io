...
'약한 참조' 와 마찬가지로, _소유되지 않은 참조 (unowned reference)_ 는 참조하는 인스턴스를 강하게 쥐고 있지 않습니다. 하지만, 약한 참조와는 다르게, '소유되지 않은' 는 다른 인스턴스가 같은 수명 또는 더 긴 수명을 가지고 있을 때 사용합니다. '소유되지 않은' 를 지시하려면 `unowned` 키워드를 속성이나 변수 선언 앞에 붙이면 됩니다.
