...
하지만, 코드를 작성하다 보면 클래스 인스턴스에 대한 '강한 참조' 를 _절대로 (never)_ 완전히 없애지 못하는 경우도 있습니다. 이는 두 클래스 인스턴스가 서로에 대한 '강한 참조' 를 쥐고 있을 때 발생하며, 그로 인해 각 인스턴스가 서로를 계속 살아있게 만듭니다. 이를 '_강한 참조 순환 (strong reference cycle)_' 이라고 합니다.

'강한 참조 순환' 을 해결하려면 클래스 사이의 일부 관계를 강한 참조가 아니라 '약한 참조 (weak reference)' 나 '소유되지 않은 참조 (unowned rerference)' 로 정의해야 합니다. 이 과정은 [Resolving Strong Reference Cycles Between Class Instances (클래스 인스턴스 사이의 강한 참조 순환 해결하기)](#resolving-strong-reference-cycles-between-class-instances-클래스-인스턴스-사이의-강한-참조-순환-해결하기) 에서 설명합니다. 하지만, 강한 참조 순환을 해결하는 방법을 배우기 전에, 어떻게 해서 그런 순환이 일어나는지를 먼저 이해하는 것이 좋을 겁니다.
...
불행하게도, 이 두 인스턴스를 연결하는 것은 이들 사이에 '강한 참조 순환 (strong reference cycle)' 을 생성합니다. `Person` 인스턴스는 이제 `Apartment` 인스턴스에 대한 '강한 참조' 를 가지며, `Apartment` 인스턴스는 `Person` 인스턴스에 대한 '강한 참조' 를 가지게 됩니다. 따라서, `john` 과 `unit4A` 변수가 쥐고 있던 '강한 참조' 를 끊더라도, '참조 카운트 (reference counts)' 는 0 으로 떨어지지 않으며, 인스턴스의 할당은 ARC 에 의해 해제되지 않습니다:
...
스위프트는 클래스 타입의 속성과 작업할 때 '강한 참조 순환 (strong reference cycle)' 을 해결하는 방법으로 두 가지를 제공합니다: '약한 참조 (weak reference)' 와 '소유되지 않은 (unowned reference)' 입니다.

'약한 참조' 와 '소유되지 않은' 는 '참조 순환' 에 있는 인스턴스 중 하나가 다른 인스턴스를 강하게 쥐지 _않고도 (without)_ 참조할 수 있도록 해줍니다. 그러면 이 인스턴스들은 '강한 참조 순환' 을 생성하지 않고도 서로를 참조할 수 있습니다.

'약한 참조' 는 다른 인스턴스의 일생이 더 짧을 때 사용합니다-다시 말해서, 다른 인스턴스의 할당이 먼저 해제되는 경우 사용합니다. 위의 `Apartment` 예제에서, 일생 중 어느 시점에 입주자가 없을 수 있다는 것은 아파트 입장에서 적절한 것이므로, '약한 참조' 로 '참조 순환' 을 끊는 것은 이 경우 적절한 방법에 해당합니다. 이와는 대조적으로, '소유되지 않은' 는 다른 인스턴스와 일생이 같거나 더 길 때 사용합니다.

#### Week References (약한 참조)

_약한 참조 (weak reference)_ 는 참조하는 인스턴스를 강하게 쥐지 않는 참조라서, ARC 가 참조된 인스턴스를 처리하는 것을 중지하지 않습니다. 이런 동작 방식은 참조가 '강한 참조 순환' 의 일부가 되는 것을 막아줍니다. '약한 참조' 를 지시하려면 `weak` 키워드를 속성이나 변수의 선언 앞에 붙이면 됩니다.

약한 참조는 참조하고 있는 인스턴스를 강하게 쥐지 않기 때문에, 약한 참조가 아직 참조하고 있는 중에도 해당 인스턴스의 할당을 해제하는 것이 가능합니다. 따라서, ARC 는 참조하고 있는 해당 인스턴스의 할당을 해체할 때 자동적으로 '약한 참조' 를 `nil` 로 설정합니다. 그리고, 약한 참조는 실행 시간에 값을 `nil` 로 바꿀 수 있어야 하므로, 항상 옵셔널 타입이며, 상수가 아니라, 변수로 선언되어야 합니다.
...
`Person` 인스턴스는 여전히 `Apartment` 인스턴스에 대한 '강한 참조' 를 가지고 있지만, `Apartment` 인스턴스는 이제 `Person` 인스턴스에 대한 '_약한 (weak)_ 참조' 를 가지고 있습니다. 이것의 의미는 `john` 변수를 `nil` 로 설정하여 이 변수가 강하게 쥐고 있던 참조를 깨뜨릴 경우, 이제 `Person` 인스턴스에 대한 '강한 참조' 는 더 이상 없다는 것입니다.
...
#### Unowned References (소유되지 않은)

'약한 참조' 와 마찬가지로, _소유되지 않은 (unowned reference)_ 는 참조하는 인스턴스를 강하게 쥐고 있지 않습니다. 하지만, 약한 참조와는 다르게, '소유되지 않은' 는 다른 인스턴스가 같은 수명 또는 더 긴 수명을 가지고 있을 때 사용합니다. '소유되지 않은' 를 지시하려면 `unowned` 키워드를 속성이나 변수 선언 앞에 붙이면 됩니다.

'소유되지 않은' 는 항상 값이 있다고 예상합니다. 그 결과로, ARC 는 '소유되지 않은' 의 값을 절대로 `nil` 로 설정하지 않으며, 이는 '소유되지 않은' 는 옵셔널이-아닌 타입을 사용하여 정의된다는 것을 의미합니다.

> 해당 참조가 할당이 해제되지 않은 인스턴스만 _항상 (always)_ 참조할 것임을 확신할 수 있을 때만 '소유되지 않은' 를 사용하기 바랍니다.
>
> 인스턴스의 할당이 해제된 후 '소유되지 않은' 의 값에 접근하려고 하면, '실행시간 에러 (runtime error)' 가 발생할 것입니다.
...
신용 카드는 항상 고객을 가지고 있기 때문에, `customer` 인스턴스를 '소유되지 않은' 로 정의하여, '강한 참조 순환' 을 피할 수 있습니다:
...
#### Weak and Unowned References (약한 참조와 소유되지 않은)
