...
스위프트는 비동기 코드와 병렬 코드를 구조적으로 작성하는 내장 기능을 지원합니다. _비동기 코드 (asynchronous code)_ 는, 한번에 한 조각의 프로그램만 실행하긴 하지만, 이를 잠시 매달았다가 나중에 다시 할 수 있습니다. 프로그램에서 코드를 잠시 매달았다가 다시 하는 것은 'UI 갱신' 같은 '단기 연산' 을 함과 동시에 네트워크 너머로 자료를 가져오거나 파일 구문을 해석하는 것 같은 '장기 연산' 도 계속 진행할 수 있게 해줍니다. _병렬 코드 (parallel code)_ 는 여러 코드 조각을 동시에 실행한다는-예를 들어, 4-개의 코어 프로세서를 가진 컴퓨터는, 각 코어마다 하나의 임무를 실시하여, 똑같은 시간에 네 코드 조각을 실행할 수 있다는-의미입니다. 병렬 및 비동기 코드를 사용하는 프로그램은 여러 개의 연산을 한번에 실시하며; 외부 시스템을 기다리는 연산은 잠시 매달고, 이 코드를 메모리-안전한 방식으로 작성하는 것을 더 쉽게 해줍니다.
...
[^sequence]: '나열 값 (sequence)' 에 대한 더 자세한 정보는, 위키피디아의 [Sequential access](https://en.wikipedia.org/wiki/Sequential_access) 항목과 [순차 접근](https://ko.wikipedia.org/wiki/순차_접근) 항목을 참고하기 바랍니다. 
