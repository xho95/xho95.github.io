...
지연 저장 속성
게으른 저장 속성은 처음 사용될 때까지 초기 값이 계산되지 않는 속성입니다. 선언하기 전에 지연 수정자를 작성하여 지연 저장 속성을 나타냅니다.

노트

인스턴스 초기화가 완료 될 때까지 초기 값을 검색하지 못할 수 있으므로 항상 lazy 속성을 var 키워드를 사용하여 변수로 선언해야합니다. 상수 속성은 초기화가 완료되기 전에 항상 값을 가져야하므로 지연으로 선언 될 수 없습니다.
게으른 속성은 속성의 초기 값이 인스턴스 초기화가 완료 될 때까지 값을 알 수없는 외부 요인에 의존 할 때 유용합니다. 게으른 속성은 속성의 초기 값에 복잡하거나 계산이 비싼 설정이 필요한 경우 나 필요하지 않을 때까지 수행하지 않아야하는 경우에도 유용합니다.

아래 예제는 지연 저장 속성을 사용하여 복잡한 클래스의 불필요한 초기화를 피합니다. 이 예제는 DataImporter 및 DataManager라는 두 클래스를 정의하며이 클래스 중 어느 것도 전체로 표시되지는 않습니다.
